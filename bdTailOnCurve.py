...
class TailIk():
    def __init__(self, *args, **kargs):
        ...
        self.direction = kargs.setdefault('direction', 'z')
        # Extras'''
        self.startJnt = ''
        self.endJnt = ''
    ...
    def createIkChain(self):
        ...
        for i in range(self.numJnt):
            jntPos = self.buildCrv.getShape().getCV(i)
    ...
    def createMainCtrl(self):
        ...
        ctrl = pm.circle(name=self.name + '_main_ctrl', nr=nr, radius=self.width * 2)[0]
        pm.delete(ctrl, ch=1)
    ...
    def createRbnSrf(self):
        ...
        loftSrf = pm.loft(crv1, crv2, ch=0, u=1, c=0, ar=1, d=1, ss=1, rn=0, po=0, rsn=1)[0]
        loftSrf = \
    ...
    def createCtrls(self):
        ...
        for i in range(self.numCtrl):
            ...
            pm.addAttr(ctrl, longName='extras', nn='------', attributeType='enum', en='Extras:', keyable=True)
            pm.setAttr(ctrl.name() + '.extras', lock=1)
            pm.addAttr(ctrl, longName='scaleRadius', attributeType='float', keyable=True, min=0.0001, defaultValue=0.3)
    ...
    def createFlcs(self, direction, ends):
        ...
        for i in range(0, self.numJnt):
            ...
            tempLoc = pm.spaceLocator()
            tempNpc = pm.createNode('nearestPointOnCurve')

            cvPos = self.buildCrv.getShape().getCV(i)
            tempLoc.translate.set(cvPos)

            self.buildCrv.worldSpace[0] >> tempNpc.inputCurve
            tempLoc.translate >> tempNpc.inPosition

            flcShape.parameterU.set(tempNpc.result.parameter.get())
            # flcShape.parameterU.set((i * 1.0 )/self.numJnt)
            flcShape.parameterV.set(0.5)

            pm.delete(tempLoc, tempNpc)
    ...
    def addCtrl(self, num):
        ...
        if self.direction == 'z':
            ...
            nr = [0, 1, 0]
        ctrl = pm.circle(name=self.name + '_ik_' + str(num).zfill(2) + '_ctrl', nr=nr, radius=self.width * 0.8)[0]
        pm.delete(ctrl, ch=1)
    ...
    def offsetCrv(self, crv, offset):
        ...
        if self.direction == 'x':
            ...
            tmpLoc.setTranslation([self.start[0] + offset, self.start[1], self.start[2]])
    ...
    def addScale(self):
        ctrlCpos = []
        for ctrl in self.ctrlList:
            pm.setAttr(ctrl.name() + '.scaleX', lock=1)

            vp = pm.shadingNode('vectorProduct', asUtility=1, name=ctrl.name() + '_ws_pmm')
            # use a closestPointOnSurface to get the coordinates on the surface of the closesest point from the ik joint
            cps = pm.shadingNode('closestPointOnSurface', asUtility=1, name=ctrl.name() + '_cps')

            vp.operation.set(4)
            ctrl.worldMatrix >> vp.matrix
            ctrl.rotatePivot >> vp.input1
        ...
        for jnt in self.rigJntList:
            layerTxt = pm.shadingNode('layeredTexture', asUtility=1, name=ctrl.name() + '_scale_lt')
            i = 0
            for ctrl in self.ctrlList:
                distancePma = pm.shadingNode('plusMinusAverage', asUtility=1, name=ctrl + '_to_' + jnt + '_dist_pma')
                distancePma.operation.set(2)
                powMd = pm.shadingNode('multiplyDivide', asUtility=1, name=ctrl + '_to_' + jnt + '_pow_pma')
                powMd.operation.set(3)
                powMd.input2X.set(2)
                sqrtMd = pm.shadingNode('multiplyDivide', asUtility=1, name=ctrl + '_to_' + jnt + '_sqrt_pma')
                sqrtMd.operation.set(3)
                sqrtMd.input2X.set(0.5)
                falloffScaleRv = pm.shadingNode('remapValue', asUtility=1,
                                                name=ctrl + '_to_' + jnt + '_scale_falloff_rv')
                falloffScaleRv.value[0].value_Position.set(0)
                falloffScaleRv.value[0].value_FloatValue.set(1)
                falloffScaleRv.value[0].value_Interp.set(2)
                falloffScaleRv.value[1].value_FloatValue.set(0)

                flcParent = pm.listRelatives(jnt, parent=1)[0]
                flcShape = flcParent.getShape()

                ctrlCpos[i].parameterU >> distancePma.input1D[0]
                flcShape.parameterU >> distancePma.input1D[1]

                distancePma.output1D >> powMd.input1X
                powMd.outputX >> sqrtMd.input1X

                sqrtMd.outputX >> falloffScaleRv.inputValue
                ctrl.scaleRadius >> falloffScaleRv.value[1].value_Position

                ctrl.scale >> layerTxt.attr('inputs[' + str(i) + '].color')
                falloffScaleRv.outValue >> layerTxt.attr('inputs[' + str(i) + '].alpha')
                layerTxt.attr('inputs[' + str(i) + '].blendMode').set(6)

                i += 1

            layerTxt.attr('inputs[' + str(i + 1) + '].color').set((1, 1, 1))
            layerTxt.attr('inputs[' + str(i + 1) + '].alpha').set(1)
            layerTxt.attr('inputs[' + str(i + 1) + '].blendMode').set(0)

            layerTxt.outColor >> jnt.scale
