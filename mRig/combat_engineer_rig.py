...
def rig():
    aya_char = char.Character(name='Combat', root='Bip001')
    aya_char.create()

    # Create upper body rig ( includes the hip rig )
    hips_jnt = ['Bip001xxxPelvis']
    upper_body_rig = ubr.UpperBodyRig(name='UpperBody', side=['', 0], bnd=hips_jnt)
    upper_body_rig.rig()
    ...
    #
    # Create arms rig
    bind_jnt = ['Bip001_SIDE_UpperArm', 'Bip001_SIDE_Forearm', 'Bip001_SIDE_Hand']
    clavicle = 'Bip001_SIDE_Clavicle'

    left_arm_rig = ar.ArmRig(side=['xxxLxxx', 1], bnd=bind_jnt, clav=clavicle)
    left_arm_rig.rig()
    #
    right_arm_rig = ar.ArmRig(side=['xxxRxxx', -1], bnd=bind_jnt, clav=clavicle)
    right_arm_rig.rig()
    #
    aya_char.add_rig(left_arm_rig)
    aya_char.add_rig(right_arm_rig)
    #Create legs rig
    bind_jnt = ['Bip001_SIDE_Thigh', 'Bip001_SIDE_Calf', 'Bip001_SIDE_Foot', 'Bip001_SIDE_Toe0', 'Bip001_SIDE_Toe0Nub']
    hips = 'Bip001xxxPelvis'

    left_leg_rig = lr.LegRig(side=['xxxLxxx', 1], bnd=bind_jnt, hips=hips)
    left_leg_rig.rig()

    right_leg_rig = lr.LegRig(side=['xxxRxxx', -1], bnd=bind_jnt, hips=hips)
    right_leg_rig.rig()
