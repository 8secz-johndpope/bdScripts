...
kPluginCmdName = "zooSurgeonCommand"
def maya_useNewAPI():
    """
...
class zooSurgeonCommand(om.MPxCommand):
    ...
    def doIt(self,argList):
        ...
        if mSelList.length() > 0:
            for i in range(mSelList.length()):
                mSelList.getDagPath(i,mDagPath)
    ...
    def createProxies(self,node,shape,skinNode):
        ...
        weightsDict = {}
        try:
            skinNode.influenceObjects(mSkinInf)
        except:
            sys.stderr.write('Object has no influences')
            return

        numFaces = shape.numPolygons()

        polyIter = om.MItMeshPolygon(node)
        while not polyIter.isDone():
            self.averageWeights = [0] * mSkinInf.length()
            currentPoly = polyIter.currentItem()
            polyIter.getVertices(mFaceVerts)
            ...
            while not vertIter.isDone():
                ...
                skinNode.getWeights(node,vertIter.currentItem(),weights,infCountPtr)
    ...
    def getSkincluster(self,fnMesh):
        ...
        '''
        skinClustersIt = om.MItDependencyNodes(om.MFn.kSkinClusterFilter)
        while not skinClustersIt.isDone():
            oItem = skinClustersIt.item()
            fnSkinCluster = oma.MFnSkinCluster(oItem)
...
def cmdCreator():
    return OpenMayaMpx.asMPxPtr(zooSurgeonCommand())
