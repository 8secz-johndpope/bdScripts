...
class TailIk():
    ...
    def skinSrf(self):
        ...
        skinCls = pm.skinCluster(self.srf, self.drvJntList, tsb=1, ih=1, bindMethod=0, maximumInfluences=1,
                                 dropoffRate=10.0)
        crvSkinCls = pm.skinCluster(self.ikCrv, self.drvJntList, tsb=1, ih=1, bindMethod=0, maximumInfluences=1,
                                    dropoffRate=10.0)
        for i in range(len(self.drvJntList)):
            if i == 0:
                ...
                pm.skinPercent(skinCls.name(), self.srf.name() + '.cv[' + str(i + 1) + '][0:3]',
                               tv=[(self.drvJntList[i], 1)])
                pm.skinPercent(crvSkinCls.name(), self.ikCrv.name() + '.cv[' + str(i + 1) + ']',
                               tv=[(self.drvJntList[i], 1)])
            elif i == self.numJnt - 1:
                pm.skinPercent(skinCls.name(), self.srf.name() + '.cv[' + str(i + 1) + ':' + str(i + 2) + '][0:3]',
                               tv=[(self.drvJntList[i], 1)])
                pm.skinPercent(crvSkinCls.name(), self.ikCrv.name() + '.cv[' + str(i + 1) + ':' + str(i + 2) + ']',
                               tv=[(self.drvJntList[i], 1)])
    ...
    def createIk(self):
        ...
        ikCrv = \
        pm.rebuildCurve(ikCrv[0], ch=0, rpo=1, rt=0, end=1, kr=0, kcp=0, kep=1, kt=0, s=self.numCtrl - 1, d=3, tol=0)[0]
        ikCrv.rename(self.name + '_ik_crv')
        self.ikCrv = ikCrv

        # create the ik
        self.ikHandle = \
        pm.ikHandle(sj=self.ikJntList[0], ee=self.ikJntList[-1], c=ikCrv, ccv=False, sol='ikSplineSolver',
