...
class VarFk:
    def __init__(self, *args, **kargs):
        ...
        self.crv = pm.ls(sl=1)[0]  # kargs.setdefault('curve')
        self.numCtrl = kargs.setdefault('numCtrl', 4)
        self.numJnt = kargs.setdefault('numJnt', 12)
    ...
    def skinSrf(self):
        skinCls = pm.skinCluster(self.srf, self.jntList, tsb=1, ih=1, skinMethod=0, maximumInfluences=1,
                                 dropoffRate=10.0)
        print skinCls
        for i in range(self.numJnt):
            if i == 0:
                pm.skinPercent(skinCls, self.srf + '.cv[0:1][0:3]', tv=[(self.jntList[i], 1)])
            elif i > 0 and i < self.numJnt - 1:
                pm.skinPercent(skinCls, self.srf + '.cv[' + str(i + 1) + '][0:3]', tv=[(self.jntList[i], 1)])
            elif i == self.numJnt - 1:
                pm.skinPercent(skinCls, self.srf + '.cv[' + str(i + 1) + ':' + str(i + 2) + '][0:3]',
                               tv=[(self.jntList[i], 1)])
    ...
    def createSrf(self):
        ...
        pm.select(cl=1)
        loftSrf = pm.loft(crv1, crv2, ch=1, u=1, c=0, ar=1, d=3, ss=1, rn=0, po=0, rsn=1, n=self.name + "_srf")[0]
        rebuiltLoftSrf = \
        pm.rebuildSurface(loftSrf, ch=1, rpo=1, rt=0, end=1, kr=0, kcp=0, kc=0, su=0, du=3, sv=0, dv=3, tol=0, fr=0,
                          dir=2)[0]
    ...
    def createFlcAndCtrl(self, direction, ends):
        ...
        for i in range(self.numCtrl):
            pm.select(cl=1)
            flcShape = pm.createNode('follicle', name=self.srf + '_flcShape_' + str(i).zfill(2))
        ...
        flcGrp = pm.group(folicles, n=self.srf + '_flc_grp')
        pm.select(cl=1)
    ...
    def addNullGroups(jntList,ctrlList):
        ...
        for jnt in jntList:import jo_varFk
#	reload(jo_varFk)
# 	jo_varFk.UI()
    ...
    def riggVarfk(self):
        ...
        for ctrl in self.ctrlList:
            for jnt in self.jntList:
                ctrlJntDistPma = pm.shadingNode('plusMinusAverage',
                                                name=jnt.name() + '_' + ctrl.name() + '_jnt_pos_minus_ctrl_pos_pma',
                                                asUtility=True)
                ...
                powMd1 = pm.shadingNode('multiplyDivide', name=jnt.name() + '_' + ctrl.name() + '_pow2_md',
                                        asUtility=True)
                ...
                powMd2 = pm.shadingNode('multiplyDivide', name=jnt.name() + '_' + ctrl.name() + '_powhalf_md',
                                        asUtility=True)
                ...
                falloffRv = pm.shadingNode('remapValue', name=jnt.name() + '_' + ctrl.name() + '_falloff_rv',
                                           asUtility=True)
                ...
                rotMultMd = pm.shadingNode('multiplyDivide', name=jnt.name() + '_' + ctrl.name() + '_rotMult_md',
                                           asUtility=True)
                ...
                if not allRotAddPma:
                    ...
                    # rotZRev = pm.shadingNode('multiplyDivide',name = jnt.name() + '_' + ctrl.name() + '_rotMult_ZRev_md',asUtility = True)
                    # rotZRev.input2Z.set(-1)
                    # allRotAddPma[0].output3D >> rotZRev.input1
                    # rotZRev.output >> jnt.rotate
                ...
                zeroScalePma = pm.shadingNode('plusMinusAverage',
                                              name=jnt.name() + '_' + ctrl.name() + '_zero_scale_pma', asUtility=True)
                zeroScalePma.operation.set(2)
                scaleMultMd = pm.shadingNode('multiplyDivide', name=jnt.name() + '_' + ctrl.name() + '_scaleMult_md',
                                             asUtility=True)
                ...
                if not allScaleAddPma:
                    allScaleAddPma = pm.shadingNode('plusMinusAverage', name=jnt.name() + '_scale_add_pma',
                                                    asUtility=True)
