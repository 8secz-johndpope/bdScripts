...
def bdRigLegBones(side):
    ...
    legBonesNames = ['Leg_1_RIG', 'Leg_2_RIG', 'Leg_3_RIG', 'LegToeBase_RIG', 'ToeEnd']
    legBones = []
...
'''
def bdCreateBlend(bindJnt,fkJnt, ikJnt,ikfkCon):
    blendColorPos = pm.createNode('blendColors',name = bindJnt.name().replace('RIG','POS_BC'))
    blendColorRot = pm.createNode('blendColors',name = bindJnt.name().replace('RIG','ROT_BC'))
    blendColorScl = pm.createNode('blendColors',name = bindJnt.name().replace('RIG','SCL_BC'))
...
for i in range(1,6):
    rigJnt = pm.ls('LeftLeg_' + str(i) + '_RIG',type='joint')[0]
    fkJnt = pm.ls('LeftLeg_' + str(i) + '_FK',type='joint')[0]
    ikJnt = pm.ls('LeftLeg_' + str(i) + '_IK',type='joint')[0]

    ikfkCon = pm.ls('LeftLeg_ikfk_ctrl',type='transform')[0]

    bdCreateBlend(rigJnt,fkJnt, ikJnt,ikfkCon):
'''
