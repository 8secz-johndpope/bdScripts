...
class bdEyeTemplate(bdRigTemplate):
        ...
        for side in parts:
            ...
            for attr in eyeCenterUD:
                if attr.type() == 'message':
                    ...
                    pm.select(cl=True)
                    chainRoot = pm.joint(p=eyeCenterPos, radius=0.2,
                                         n=(side + '_' + eyeChild.name().split(':')[1].replace('guide', '01_jnt')))
                    endChain = pm.joint(p=eyeChildPos, radius=0.2,
                                        n=(side + '_' + eyeChild.name().split(':')[1].replace('guide', '02_bnd_jnt')))
            ...
            eyeAnim = pm.circle(n=(side + '_' + eyeAnimGuide.name().split(':')[1].replace('_guide', '')), nr=(0, 0, 1),
                                c=eyeAnimGuidePos, radius=1)[0]
    ...
    def bdCreateBlink(self, side):
        ...
        for i in range(len(upLidDrvJnt)):
            ...
            pm.addAttr(baseLidsJnt, ln=upLidDrvJnt[i].name() + '_angle', nn=upLidDrvJnt[i].name() + '_angle',
                       at='double')
            baseLidsJnt.attr(upLidDrvJnt[i].name() + '_angle').set(upAngle)

            lowAngle = self.bdGetEyelidJntAngle(lowLidDrvJnt[i])
            pm.addAttr(baseLidsJnt, ln=lowLidDrvJnt[i].name() + '_angle', nn=upLidDrvJnt[i].name() + '_angle',
                       at='double')
    ...
    def bdRigEye(self):
        ...
        for side in parts:
            ...
            pm.select(cl=True)
            pm.aimConstraint(eyeAnim[0], eyeJoint, offset=[0, 0, 0], weight=1, aimVector=[0, 0, 1], upVector=[0, 1, 0],
