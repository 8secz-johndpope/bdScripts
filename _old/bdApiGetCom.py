...
def bdCalculateVolume(vectorArray, pointInsideVec):
    volume = ((vectorArray[0] - pointInsideVec) * (
    (vectorArray[1] - pointInsideVec) ^ (vectorArray[2] - pointInsideVec))) / 6
    return volume


def bdCalculateCenter(vectorArray, pointInsideVec):
    center = ((vectorArray[0] - pointInsideVec) + (vectorArray[1] - pointInsideVec) + (
    vectorArray[2] - pointInsideVec)) / 4
    return center
