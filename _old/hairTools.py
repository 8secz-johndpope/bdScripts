...
''' % window)
    ...
    mc.columnLayout(adjustableColumn=True)
    densityField = mc.floatSliderGrp(label='Density', cw3=[80, 80, 120], precision=2, field=True, value=.4, fs=.01,
                                     minValue=.01, maxValue=1.0, fmx=200.0)
    layerField = mc.intSliderGrp(label='Layers', cw3=[80, 80, 120], field=True, value=5, minValue=1, maxValue=15,
                                 fmx=200)
    twistField = mc.floatSliderGrp(label='Twist', cw3=[80, 80, 120], precision=2, field=True, value=0, fs=.01,
                                   minValue=-1.0, maxValue=1.0, fmx=5.0, fmn=-5.0)
...
''' % (densityField, layerField, twistField))
    ...
    mc.columnLayout(adjustableColumn=True)
    rand1Field = mc.floatSliderGrp(label='Start', cw3=[80, 80, 120], precision=2, field=True, value=.1, fs=.1,
                                   minValue=0, maxValue=5.0, fmx=200.0)
    rand2Field = mc.floatSliderGrp(label='Middle', cw3=[80, 80, 120], precision=2, field=True, value=.4, fs=.1,
                                   minValue=0, maxValue=5.0, fmx=200.0)
    rand3Field = mc.floatSliderGrp(label='End', cw3=[80, 80, 120], precision=2, field=True, value=.6, fs=.1, minValue=0,
                                   maxValue=5.0, fmx=200.0)
...
hairTools.randomizeHair(mc.ls(sl=True, fl=True), [mc.floatSliderGrp("%s", q=True, value=True), mc.floatSliderGrp("%s", q=True, v=True), mc.floatSliderGrp("%s", q=True, v=True)])
    ...
    mc.columnLayout(adjustableColumn=True)
    minTrimField = mc.floatSliderGrp(label='Min Length', cw3=[80, 80, 120], precision=2, field=True, value=.3, fs=.1,
                                     minValue=0.1, maxValue=1.0)
    percentTrimField = mc.floatSliderGrp(label='Percent to trim', cw3=[80, 80, 120], precision=2, field=True, value=.5,
                                         fs=.1, minValue=0.1, maxValue=1.0)
...
def orderEdgeloopVerts(verts, start=None, direction=None):
    ...
    if direction and direction in verts:
        ...
        adjacentVerts = mc.ls(
            mc.polyListComponentConversion(mc.polyListComponentConversion(start, fv=True, te=True), fe=True, tv=True),
            fl=True)
    ...
    while len(verts) > 1:
        adjacentVerts = mc.ls(
            mc.polyListComponentConversion(mc.polyListComponentConversion(orderedVerts[-1], fv=True, te=True), fe=True,
                                           tv=True), fl=True)
        for vert in adjacentVerts:
            if vert in verts:
...
def getNeighboringEdgeloops(edgeLoop):
    ...
    '''
    expandedVerts = mc.ls(
        mc.polyListComponentConversion(mc.polyListComponentConversion(edgeLoop, fv=True, te=True), fe=True, tv=True),
        fl=True)
...
def findCorrespondingVertInLoop(vert, edgeLoop):
    ...
    '''
    nearestVerts = mc.ls(
        mc.polyListComponentConversion(mc.polyListComponentConversion(vert, fv=True, te=True), fe=True, tv=True),
        fl=True)
...
def averageCV(amount=1.0):
    for cv in mc.ls(sl=True, fl=True):
        ...
        average = [(pos1[0] + pos2[0] + pos3[0]) / 3, (pos1[1] + pos2[1] + pos3[1]) / 3,
...
def createInterpolatedCurve(curve1, curve2, v):
    ...
    mc.rebuildCurve(interpolatedCurve, ch=0, rpo=1, rt=0, end=1, kr=0, kcp=0, kep=1, kt=0, s=len(cvList) - 3, d=3,
                    tol=0)
    for i in range(len(cvList)):
        pos1 = mc.pointPosition('%s.cv[%i]' % (interpolatedCurve, i))
        pos2 = mc.pointPosition('%s.cv[%i]' % (curve2, i))
        newPos = (
        (pos2[0] - pos1[0]) * v + pos1[0], (pos2[1] - pos1[1]) * v + pos1[1], (pos2[2] - pos1[2]) * v + pos1[2])
