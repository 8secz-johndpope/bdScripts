        ...
        if self.FK_CTRL:
            ...
            if not self.FK_Joint:
                ...
                temp = mayac.orientConstraint(self.FK_Joint, self.Bind_Joint, mo=True,
                                              name="%s_FKIK_Constraint" % (self.nodeName))
                self.Constraint = temp[0]
                mayac.setAttr("%s.offsetX" % (self.Constraint), 0)
                mayac.setAttr("%s.offsetY" % (self.Constraint), 0)
                mayac.setAttr("%s.offsetZ" % (self.Constraint), 0)
...
class DJB_Character():
    ...
    def hookUpControls(self):
        ...
        if self.rigType == "AutoRig":
            ...
            if self.LeftHandThumb1.Bind_Joint:
                ...
                if self.fingerFlip:
                    mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandThumb1.FK_CTRL_inRig_CONST_GRP),
                                            currentDriver="%s.ThumbCurl" % (self.LeftHand.Options_CTRL),
                                            driverValue=-10.0, value=25.0)
                    mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandThumb1.FK_CTRL_inRig_CONST_GRP),
                                            currentDriver="%s.ThumbCurl" % (self.LeftHand.Options_CTRL),
                                            driverValue=10.0, value=-25.0)
                    mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandThumb2.FK_CTRL_inRig_CONST_GRP),
                                            currentDriver="%s.ThumbCurl" % (self.LeftHand.Options_CTRL),
                                            driverValue=-10.0, value=60.0)
                    mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandThumb2.FK_CTRL_inRig_CONST_GRP),
                                            currentDriver="%s.ThumbCurl" % (self.LeftHand.Options_CTRL),
                                            driverValue=10.0, value=-60.0)
                    mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandThumb3.FK_CTRL_inRig_CONST_GRP),
            ...
            if self.RightHandIndex1.Bind_Joint:
                ...
                if self.fingerFlip:
                    ...
                    mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandIndex1.FK_CTRL_inRig_CONST_GRP),
                                            ...
                                            value=-30.0)
                    mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandIndex1.FK_CTRL_inRig_CONST_GRP),
                                            currentDriver="%s.Spread" % (self.RightHand.Options_CTRL),
                                            driverValue=-10.0, value=12.0)
            else:
            ...
            if self.RightHandThumb1.Bind_Joint:
                ...
                if self.fingerFlip:
                    mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandThumb1.FK_CTRL_inRig_CONST_GRP),
                                            currentDriver="%s.ThumbCurl" % (self.RightHand.Options_CTRL),
                                            driverValue=-10.0, value=-25.0)
                    mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandThumb1.FK_CTRL_inRig_CONST_GRP),
                                            currentDriver="%s.ThumbCurl" % (self.RightHand.Options_CTRL),
                                            driverValue=10.0, value=25.0)
                    ...
                    mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandThumb1.FK_CTRL_inRig_CONST_GRP),
                                            currentDriver="%s.Spread" % (self.RightHand.Options_CTRL),
                                            driverValue=-10.0, value=15.0)
                    mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandThumb2.FK_CTRL_inRig_CONST_GRP),
                                            currentDriver="%s.Spread" % (self.RightHand.Options_CTRL),
                                            driverValue=-10.0, value=30.0)
            else:
                    ...
                    for child in bodyPart.children:
                        if child.Bind_Joint and "End" not in child.nodeName:
                            ...
                            if not mayac.objExists("%s%s" % (New_joint_Namespace, child.nodeName)):
                                ...
                                DataChildPOS = mayac.xform("%s%s" % (New_joint_Namespace, child.nodeName), query=True,
                                                           absolute=True, worldSpace=True, translation=True)
                                correctDistance = math.sqrt((selfPOS[0] - childPOS[0]) * (selfPOS[0] - childPOS[0]) + (
                                selfPOS[1] - childPOS[1]) * (selfPOS[1] - childPOS[1]) + (selfPOS[2] - childPOS[2]) * (
                                                            selfPOS[2] - childPOS[2])) / mayac.getAttr(
    ...
    def transferMotionToAnimDataJoints(self, incomingDataRootJoint, newStartTime=0,
                                       mixMethod="insert"):  # mixMethod - insert or merge
        mayac.currentTime(1)
        New_joint_Namespace = DJB_findBeforeSeparator(incomingDataRootJoint, ':')
        ...
        for bodyPart in self.bodyParts:
            ...
            if bodyPart.nodeName == "Root" and self.hulaOption:
                ...
                mayac.copyKey("%sHips" % (New_joint_Namespace), time=(0, lastFrame), hierarchy="none", controlPoints=0,
                              shape=1)
                mayac.pasteKey(bodyPart.AnimData_Joint, option=mixMethod, connect=1, timeOffset=newStartTime,
                               valueOffset=0)
            elif bodyPart.nodeName == "Hips" and not self.hulaOption:
                ...
                newAnimDataJoint = "%s%s" % (New_joint_Namespace, bodyPart.nodeName)
                if mayac.objExists(newAnimDataJoint):
                    numCurves = mayac.copyKey(newAnimDataJoint, time=(0, lastFrame), hierarchy="none", controlPoints=0,
                                              shape=1)
                    if numCurves:
                        mayac.pasteKey(bodyPart.AnimData_Joint, option=mixMethod, connect=1, timeOffset=newStartTime,
                                       valueOffset=0)
...
class MIXAMO_AutoControlRig_UI:
    def __init__(self):
        ...
        autoSkinnerText = mayac.text(
            label='  Step 1: Import your downloaded MIXAMO character and then press the button below.', align='left')
        ...
        self.hulaOptionCheckBox = mayac.checkBox(
            label='Add Pelvis ("hula") Control     WARNING!!!!: This changes the hierarchy.  Skeleton will no longer match original fbx.',
            align='left')

        self.setupControls_button = mayac.button(label='Rig Character', w=100, c=self.setupControls_function)
        mayac.text(label='', align='left')
        animationText = mayac.text(label='  Step 2: Import or reference your downloaded MIXAMO animation.',
                                   align='left')
        mayac.text(label='(Referencing is for use if you intend to update the source animation fbx file.)',
                   align='center')
        ...
        mayac.text(label='', align='left')
        mayac.text(
            label='  Step 3: Select the "Hips" joint of the imported motion and press copy or direct connection button.',
            align='left')
        mayac.button(label='Copy Animation to Rig', w=100, c=self.copyAnimationToRig_function)
        mayac.button(label='Direct Connect Animation to Rig (Use with referenced animation)', w=100,
        ...
        mayac.text(label='', align='left')
        self.bakeAnimation_button = mayac.button(
            label="Bake Animation to Controls (can't use with direct connect animation)", w=100,
            c=self.bakeAnimation_function)
        mayac.text(label='', align='left')
        self.bakeAnimation_button = mayac.button(
            label="Clear Animation Controls (can't use with direct connect animation)", w=100,
            c=self.clearAnimation_function)
        mayac.text(label='', align='left')
        mayac.text(label='', align='left')
        self.deleteOrigAnimation_button = mayac.button(
            label="Delete Original Animation (can't use with direct connect animation)", w=100,
