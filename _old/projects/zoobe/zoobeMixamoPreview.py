...
class MixamoPreview():
    ...
    def createCharXML(self):
        ...
        if os.path.isdir(animFolder):
            animationList = [f.split('.')[0] for f in os.listdir(animFolder) if
                             f.endswith('.skeleton') and 'speak' not in f]
        else:
            ...
            if charDir:
                for folder in charDir:
                    if self.name in folder:
                        try:
                            shutil.copytree(os.path.join(ASSETS_PATH, 'characters', folder),
                                            os.path.join(TEST_ASSETS_PATH, 'characters', folder))
                        except:
