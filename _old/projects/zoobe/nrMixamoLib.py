...
def hookupEyesAimRig():
    ...
    for klb in keyframesLoopborders:
        for ela in eyeLidAttrs:
            mc.setKeyframe(keyedNodes[0], attribute=ela, time=klb[0], value=klb[1], inTangentType='auto',
                           outTangentType='auto')
        for ba in browAttrs:
            mc.setKeyframe(keyedNodes[1], attribute=ba, time=klb[0], value=klb[1], inTangentType='auto',
                           outTangentType='auto')
    ...
    for i in range(0, len(blinkStartTimes)):
        for k in range(0, len(keyframesBlink)):
            for ela in eyeLidAttrs:  # offset the blink for each of the blink start times and key the attr
                mc.setKeyframe(keyedNodes[0], attribute=ela, time=keyframesBlink[k][0] + blinkStartTimes[i],
                               value=keyframesBlink[k][1], inTangentType='auto', outTangentType='auto')
            for ba in browAttrs:  # offset the blink and modify to use as fleshy animation on brows
                mc.setKeyframe(keyedNodes[1], attribute=ba, time=keyframesBlink[k][0] + blinkStartTimes[i] + 1,
                               value=keyframesBlink[k][1] * -0.1, inTangentType='auto', outTangentType='auto')
...
def buildEyeAimRig(eyeVerts, eyeGeo='Body', eyeJoint='mixamorig:LeftEye'):
    ...
    mc.aimConstraint(targetGrp, aimGrp, n=aimGrp.replace('_GRP', '_AIC'), aimVector=[0, 0, 1],
...
def buildFaceShapes(blsNode='Facial_Blends'):
   ...
   \code
    ...
    shapes.append(['speak_mouth_narrow', ['MouthOpen', 0.1], ['JawBackward', 0.2], ['MouthNarrow_Left', 0.5],
                   ['MouthNarrow_Right', 0.5]])
    shapes.append(['speak_mouth_wide', ['MouthOpen', 0.25], ['JawBackward', 0.2], ['MouthNarrow_Left', -0.4],
                   ['MouthNarrow_Right', -0.4], ['Smile_Left', 0.2], ['Smile_Right', 0.2]])
    shapes.append(['smile', ['Midmouth_Left', 0.15], ['MouthNarrow_Left', -0.1], ['MouthNarrow_Right', -0.1],
                   ['NoseScrunch_Left', 0.1], ['NoseScrunch_Right', 0.1], ['Smile_Left', 0.5], ['Smile_Right', 0.5],
                   ['Squint_Left', 0.5], ['Squint_Right', 0.5]])

    setBlsPose(blsNode, shapes)
    secondaryBlsNodes = ['Facial_Blends_ncl1_1', 'Facial_Blends_ncl1_2']
    setBlsPose(secondaryBlsNodes[0],
               shapes)  # secondary nodes need to have addl shapes so smile shape also moves eye lashes
...
def DJB_BrowserWindow(filter_=None, caption_="Browse", fileMode_="directory"):
    ...
    if version <= 2011.0:
        ...
        if fileMode_ == "directory":
            window = mayac.fileDialog2(fileFilter=multipleFilters, dialogStyle=2, caption=caption_, fileMode=3,
                                       okCaption="Select")
        else:
            window = mayac.fileDialog2(fileFilter=multipleFilters, dialogStyle=2, caption=caption_, fileMode=4,
                                       okCaption="Select")
    if window:
        return window[0]
    else:
...
def DJB_movePivotToObject(moveMe, toHere, posOnly=False):
    ...
    mayac.move(POS[0], POS[1], POS[2], (moveMe + ".rotatePivot"), (moveMe + ".scalePivot"), absolute=True,
               worldSpace=True)
    if not posOnly:
...
def DJB_CheckAngle(object1, object2, object3, axis="z", multiplier=1):  # axis can be "x", "y", or "z"
    ...
    distOrig = math.sqrt((obj3POS[0] - obj1POS[0]) * (obj3POS[0] - obj1POS[0]) + (obj3POS[1] - obj1POS[1]) * (
    obj3POS[1] - obj1POS[1]) + (obj3POS[2] - obj1POS[2]) * (obj3POS[2] - obj1POS[2]))
    mayac.setAttr("%s.rotate%s" % (object2, axis.upper()), rotOrig + .5 * multiplier)
    obj1POS = mayac.xform(object1, query=True, worldSpace=True, absolute=True, translation=True)
    obj3POS = mayac.xform(object3, query=True, worldSpace=True, absolute=True, translation=True)
    distBack = math.sqrt((obj3POS[0] - obj1POS[0]) * (obj3POS[0] - obj1POS[0]) + (obj3POS[1] - obj1POS[1]) * (
    obj3POS[1] - obj1POS[1]) + (obj3POS[2] - obj1POS[2]) * (obj3POS[2] - obj1POS[2]))
...
def _particleDyn(obj, weight, conserve, transfShapes, nucleus):
    ...
    mayac.makeIdentity(dyn,
                       apply=True)  # in questo modo il cubo assume le coordinate dell'oggetto pur essendo posizionato nel suo pivot
...
undoInfo -swf 1;""" % (dyn, partic, dynLocName, partShape, c, c, c)
    ...
    # locks
    [mayac.setAttr(partic + s, k=False, cb=True) for s in
     [".tx", ".ty", ".tz", ".rx", ".ry", ".rz", ".sx", ".sy", ".sz", ".v", ".startFrame"]]

    return dyn


def drawOct(name, r=1.0, pos=(0.0, 0.0, 0.0)):
    p = [(s[0] + pos[0], s[1] + pos[1], s[2] + pos[2]) for s in
         [(0, 0, r), (r, 0, 0), (0, 0, -r), (-r, 0, 0), (0, -r, 0), (r, 0, 0), (0, r, 0), (-r, 0, 0), (0, 0, r),
...
class blendShapeAttrTracker(object):
    ...
    def connectNewBlendShape(self, newBlendshapeNode):
        ...
        mayac.connectAttr("%s.%s" % (self.blendShapeNodeOrigTempName, self.attr),
                          "%s.%s" % (self.newBlendShapeNode, self.attr))
...
class blendShapeTracker(object):
    ...
    def duplicate(self, newMesh):
        ...
        self.blendShapeNodeNew = \
        mayac.blendShape(blendShapeCreationSelection, name=self.blendShapeNodeOrig, frontOfChain=True)[0]
        for attrTracker in self.blendShapeAttrTrackers:
...
class DJB_CharacterNode():
    def __init__(self, joint_name_, infoNode_=None, optional_=0, hasIK_=0, parent=None, nameSpace_="", actAsRoot_=0,
                 alias_=None, dynamic_=None, twistJoint_=False, translateOpen_=False):
        ...
        if not self.infoNode:
            ...
            if not self.Bind_Joint and not optional_:
                OpenMaya.MGlobal.displayError(
                    "ERROR: %s cannot be found and is necessary for the autorigger to complete process" % (
                    str(JOINT_NAMESPACE) + joint_name_))
                sys.exit()
        ...
        # recreate from an infoNode
            ...
            self.AnimData_Joint = DJB_addNameSpace(self.characterNameSpace,
                                                   attrToPy("%s.AnimData_Joint" % (self.infoNode)))
            ...
            self.IK_Dummy_Joint = DJB_addNameSpace(self.characterNameSpace,
                                                   attrToPy("%s.IK_Dummy_Joint" % (self.infoNode)))
            ...
            self.FK_CTRL_inRig_CONST_GRP = DJB_addNameSpace(self.characterNameSpace,
                                                            attrToPy("%s.FK_CTRL_inRig_CONST_GRP" % (self.infoNode)))
            self.FK_CTRL_animData_CONST_GRP = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.FK_CTRL_animData_CONST_GRP" % (self.infoNode)))
            self.FK_CTRL_animData_MultNode = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.FK_CTRL_animData_MultNode" % (self.infoNode)))
            self.FK_CTRL_animData_MultNode_Trans = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.FK_CTRL_animData_MultNode_Trans" % (self.infoNode)))
            self.FK_CTRL_POS_GRP = DJB_addNameSpace(self.characterNameSpace,
                                                    attrToPy("%s.FK_CTRL_POS_GRP" % (self.infoNode)))
            ...
            self.IK_CTRL_inRig_CONST_GRP = DJB_addNameSpace(self.characterNameSpace,
                                                            attrToPy("%s.IK_CTRL_inRig_CONST_GRP" % (self.infoNode)))
            self.IK_CTRL_animData_CONST_GRP = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.IK_CTRL_animData_CONST_GRP" % (self.infoNode)))
            self.IK_CTRL_animData_MultNode = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.IK_CTRL_animData_MultNode" % (self.infoNode)))
            self.IK_CTRL_POS_GRP = DJB_addNameSpace(self.characterNameSpace,
                                                    attrToPy("%s.IK_CTRL_POS_GRP" % (self.infoNode)))
            self.IK_CTRL_ReorientGRP = DJB_addNameSpace(self.characterNameSpace,
                                                        attrToPy("%s.IK_CTRL_ReorientGRP" % (self.infoNode)))

            self.IK_CTRL_parent_animData_CONST_GRP = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.IK_CTRL_parent_animData_CONST_GRP" % (self.infoNode)))
            self.IK_CTRL_parent_animData_MultNode = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.IK_CTRL_parent_animData_MultNode" % (self.infoNode)))
            self.IK_CTRL_parent_POS_GRP = DJB_addNameSpace(self.characterNameSpace,
                                                           attrToPy("%s.IK_CTRL_parent_POS_GRP" % (self.infoNode)))

            self.IK_CTRL_grandparent_inRig_CONST_GRP = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.IK_CTRL_parent_POS_GRP" % (self.infoNode)))
            self.IK_CTRL_grandparent_animData_CONST_GRP = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.IK_CTRL_grandparent_animData_CONST_GRP" % (self.infoNode)))
            self.IK_CTRL_grandparent_animData_MultNode = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.IK_CTRL_grandparent_animData_MultNode" % (self.infoNode)))
            self.IK_CTRL_grandparent_POS_GRP = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.IK_CTRL_grandparent_POS_GRP" % (self.infoNode)))

            self.Inherit_Rotation_GRP = DJB_addNameSpace(self.characterNameSpace,
                                                         attrToPy("%s.Inherit_Rotation_GRP" % (self.infoNode)))
            self.Inherit_Rotation_Constraint = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.Inherit_Rotation_Constraint" % (self.infoNode)))
            self.Inherit_Rotation_Reverse = DJB_addNameSpace(self.characterNameSpace,
                                                             attrToPy("%s.Inherit_Rotation_Reverse" % (self.infoNode)))
            self.Constraint = DJB_addNameSpace(self.characterNameSpace, attrToPy("%s.Constraint" % (self.infoNode)))
            self.FK_Constraint = DJB_addNameSpace(self.characterNameSpace,
                                                  attrToPy("%s.FK_Constraint" % (self.infoNode)))
            self.IK_Constraint = DJB_addNameSpace(self.characterNameSpace,
                                                  attrToPy("%s.IK_Constraint" % (self.infoNode)))
            self.IK_Handle = DJB_addNameSpace(self.characterNameSpace, attrToPy("%s.IK_Handle" % (self.infoNode)))
            self.IK_EndEffector = DJB_addNameSpace(self.characterNameSpace,
                                                   attrToPy("%s.IK_EndEffector" % (self.infoNode)))
            self.PV_Constraint = DJB_addNameSpace(self.characterNameSpace,
                                                  attrToPy("%s.PV_Constraint" % (self.infoNode)))
            self.Guide_Curve = DJB_addNameSpace(self.characterNameSpace, attrToPy("%s.Guide_Curve" % (self.infoNode)))
            self.Guide_Curve_Cluster1 = DJB_addNameSpace(self.characterNameSpace,
                                                         attrToPy("%s.Guide_Curve_Cluster1" % (self.infoNode)))
            self.Guide_Curve_Cluster2 = DJB_addNameSpace(self.characterNameSpace,
                                                         attrToPy("%s.Guide_Curve_Cluster2" % (self.infoNode)))
            ...
            self.IK_CTRL_parent_Global_POS_GRP = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.IK_CTRL_parent_Global_POS_GRP" % (self.infoNode)))
            self.IK_CTRL_grandparent_Global_POS_GRP = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.IK_CTRL_grandparent_Global_POS_GRP" % (self.infoNode)))
            self.grandparent_Global_Constraint = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.grandparent_Global_Constraint" % (self.infoNode)))
            self.grandparent_Global_Constraint_Reverse = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.grandparent_Global_Constraint_Reverse" % (self.infoNode)))
            self.parent_Global_Constraint = DJB_addNameSpace(self.characterNameSpace,
                                                             attrToPy("%s.parent_Global_Constraint" % (self.infoNode)))
            self.parent_Global_Constraint_Reverse = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.parent_Global_Constraint_Reverse" % (self.infoNode)))

            self.follow_extremity_Constraint = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.follow_extremity_Constraint" % (self.infoNode)))
            self.follow_extremity_Constraint_Reverse = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.follow_extremity_Constraint_Reverse" % (self.infoNode)))
            ...
            self.locatorConstraint1 = DJB_addNameSpace(self.characterNameSpace,
                                                       attrToPy("%s.locatorConstraint1" % (self.infoNode)))
            self.footRotateLOC = DJB_addNameSpace(self.characterNameSpace,
                                                  attrToPy("%s.footRotateLOC" % (self.infoNode)))
            self.Follow_Foot_GRP = DJB_addNameSpace(self.characterNameSpace,
                                                    attrToPy("%s.Follow_Foot_GRP" % (self.infoNode)))
            self.Follow_Knee_GRP = DJB_addNameSpace(self.characterNameSpace,
                                                    attrToPy("%s.Follow_Knee_GRP" % (self.infoNode)))
            self.Follow_Knee_Constraint = DJB_addNameSpace(self.characterNameSpace,
                                                           attrToPy("%s.Follow_Knee_Constraint" % (self.infoNode)))
            self.Follow_Foot_Constraint = DJB_addNameSpace(self.characterNameSpace,
                                                           attrToPy("%s.Follow_Knee_Constraint" % (self.infoNode)))
            ...
            self.Dyn_CTRL_COLOR = DJB_addNameSpace(self.characterNameSpace,
                                                   attrToPy("%s.Dyn_CTRL_COLOR" % (self.infoNode)))
    ...
    def writeInfoNode(self):
        ...
        pyToAttr("%s.IK_CTRL_grandparent_animData_CONST_GRP" % (self.infoNode),
                 self.IK_CTRL_grandparent_animData_CONST_GRP)
        pyToAttr("%s.IK_CTRL_grandparent_animData_MultNode" % (self.infoNode),
                 self.IK_CTRL_grandparent_animData_MultNode)
        ...
        pyToAttr("%s.grandparent_Global_Constraint_Reverse" % (self.infoNode),
                 self.grandparent_Global_Constraint_Reverse)
    ...
    def duplicateJoint(self, type, parent_="UseSelf", jointNamespace=None):
        if self.Bind_Joint:
            if type == "AnimData":
                self.AnimData_Joint = \
                mayac.duplicate(self.Bind_Joint, parentOnly=True, name="AnimData_" + self.nodeName)[0]
            elif type == "FK":
                ...
                self.IK_Dummy_Joint = \
                mayac.duplicate(self.Bind_Joint, parentOnly=True, name="IK_Dummy_" + self.nodeName)[0]
                ...
                if jointNamespace:
                    ...
                    self.Export_Joint = mayac.duplicate(self.Bind_Joint, parentOnly=True, inputConnections=False,
                                                        name=jointNamespace + self.nodeName)[0]
                else:
                ...
                self.DynMult_Joint = mayac.duplicate(self.Bind_Joint, parentOnly=True, name="DynMult_" + self.nodeName)[
                    0]
    ...
    def createControl(self, type, rigType="AutoRig", style="circle", partialConstraint=0, scale=(0.1, 0.1, 0.1),
                      rotate=(0, 0, 0), offset=(0, 0, 0), estimateSize=True, color_=None, name_=None,
                      flipFingers=False):
        control = 0
        if style == "circle":
            if estimateSize:
                ...
                if "Root" not in self.nodeName and "Spine" not in self.nodeName and "Hips" not in self.nodeName and rigType == "World":
                    mayac.rotate(rotate[0], rotate[1], rotate[2], control,
                                 absolute=True)  # override for world-oriented rigs
                if rigType == "Dyn":
            ...
            if estimateSize:
                ...
                mayac.move(0, -.2, 0, "%s.cv[0]" % (control), "%s.cv[7:8]" % (control), "%s.cv[15]" % (control),
                           relative=True)
                mayac.scale(1.3, 1.3, 1.3, "%s.cv[3:4]" % (control), "%s.cv[9:12]" % (control),
                            "%s.cv[13:14]" % (control))
            ...
            if estimateSize:
                ...
                mayac.move(0, .3, 0, "%s.cv[0]" % (control), "%s.cv[7:8]" % (control), "%s.cv[15]" % (control),
                           relative=True)
            ...
            if estimateSize:
                ...
                mayac.scale(scale[0], scale[1], scale[2])
                mayac.rotate(rotate[0], rotate[1], rotate[2], control,
                             relative=True)  # override for world-oriented rigs
                mayac.move(offset[0], offset[1], offset[2], "%s.cv[0:7]" % (control), relative=True)
                mayac.makeIdentity(control, apply=True, t=1, r=1, s=1, n=0)
            else:
                print "exactSizeNotFunctionalYet"
            ...
            if estimateSize:
                control = mayac.circle(constructionHistory=0)
                control = control[0]
                mayac.move(0, 0, -0.5, "%s.cv[0]" % (control), "%s.cv[2]" % (control), "%s.cv[4]" % (control),
                           "%s.cv[6]" % (control), relative=True)
                mayac.move(0, 0, 0.3, "%s.cv[1]" % (control), "%s.cv[3]" % (control), "%s.cv[5]" % (control),
                           "%s.cv[7]" % (control), relative=True)
        ...
        if not partialConstraint:
            ...
            mayac.delete(mayac.aimConstraint(self.children[0].Bind_Joint, control, skip=["x", "z"], weight=1,
                                             aimVector=(0, 0, 1), worldUpType="vector", upVector=(0, 1, 0)))
    ...
    def finalizeCTRLs(self, parent="UseSelf"):
        ...
        if self.FK_CTRL:
            ...
            self.FK_CTRL_inRig_CONST_GRP = DJB_createGroup(transform=self.FK_CTRL,
                                                           fullName=self.FK_CTRL + "_In_Rig_CONST_GRP")
            self.FK_CTRL_animData_CONST_GRP = DJB_createGroup(transform=self.FK_CTRL_inRig_CONST_GRP,
                                                              fullName=self.FK_CTRL + "_AnimData_CONST_GRP")
            self.FK_CTRL_animData_MultNode = mayac.createNode('multiplyDivide', n=self.FK_CTRL + "_AnimData_MultNode")
            self.FK_CTRL_POS_GRP = DJB_createGroup(transform=self.FK_CTRL_animData_CONST_GRP,
                                                   fullName=self.FK_CTRL + "_POS_GRP")
            ...
            if "Head" in self.nodeName:
                ...
                self.Inherit_Rotation_GRP = DJB_createGroup(transform=None,
                                                            fullName=self.FK_CTRL + "_Inherit_Rotation_GRP",
                                                            pivotFrom=self.FK_CTRL)
                ...
                temp = mayac.orientConstraint(self.FK_CTRL_animData_CONST_GRP, self.FK_CTRL_inRig_CONST_GRP,
                                              maintainOffset=True)
                ...
                self.Inherit_Rotation_Constraint_Reverse = mayac.createNode('reverse',
                                                                            n="Head_Inherit_Rotation_Constraint_Reverse")
                mayac.connectAttr("%s.InheritRotation" % (self.FK_CTRL),
                                  "%s.inputX" % (self.Inherit_Rotation_Constraint_Reverse))
                mayac.connectAttr("%s.InheritRotation" % (self.FK_CTRL),
                                  "%s.%sW0" % (self.Inherit_Rotation_Constraint, self.FK_CTRL_animData_CONST_GRP))
                mayac.connectAttr("%s.outputX" % (self.Inherit_Rotation_Constraint_Reverse),
                                  "%s.%sW1" % (self.Inherit_Rotation_Constraint, self.Inherit_Rotation_GRP))
                mayac.setAttr("%s.interpType" % (self.Inherit_Rotation_Constraint), 2)

            # hook up CTRLs
            print('#hook up CTRLs')
            mayac.connectAttr("%s.rotateX" % (self.AnimData_Joint), "%s.input1X" % (self.FK_CTRL_animData_MultNode),
                              force=True)
            mayac.connectAttr("%s.AnimDataMult" % (self.FK_CTRL), "%s.input2X" % (self.FK_CTRL_animData_MultNode),
                              force=True)
            mayac.connectAttr("%s.rotateY" % (self.AnimData_Joint), "%s.input1Y" % (self.FK_CTRL_animData_MultNode),
                              force=True)
            mayac.connectAttr("%s.AnimDataMult" % (self.FK_CTRL), "%s.input2Y" % (self.FK_CTRL_animData_MultNode),
                              force=True)
            mayac.connectAttr("%s.rotateZ" % (self.AnimData_Joint), "%s.input1Z" % (self.FK_CTRL_animData_MultNode),
                              force=True)
            mayac.connectAttr("%s.AnimDataMult" % (self.FK_CTRL), "%s.input2Z" % (self.FK_CTRL_animData_MultNode),
                              force=True)

            mayac.connectAttr("%s.outputX" % (self.FK_CTRL_animData_MultNode),
                              "%s.rotateX" % (self.FK_CTRL_animData_CONST_GRP), force=True)
            mayac.connectAttr("%s.outputY" % (self.FK_CTRL_animData_MultNode),
                              "%s.rotateY" % (self.FK_CTRL_animData_CONST_GRP), force=True)
            mayac.connectAttr("%s.outputZ" % (self.FK_CTRL_animData_MultNode),
                              "%s.rotateZ" % (self.FK_CTRL_animData_CONST_GRP), force=True)
            if not self.FK_Joint:
                if self.actAsRoot:
                    mayac.addAttr(self.FK_CTRL, longName='AnimDataMultTrans', defaultValue=1.0, keyable=True)
                    self.FK_CTRL_animData_MultNode_Trans = mayac.createNode('multiplyDivide',
                                                                            n=self.FK_CTRL + "_AnimData_MultNode_Trans")
                    temp = mayac.parentConstraint(self.FK_CTRL, self.Bind_Joint, mo=True,
                                                  name="%s_Constraint" % (self.nodeName))
                    self.Constraint = temp[0]

                    mayac.connectAttr("%s.translateX" % (self.AnimData_Joint),
                                      "%s.input1X" % (self.FK_CTRL_animData_MultNode_Trans), force=True)
                    mayac.connectAttr("%s.AnimDataMultTrans" % (self.FK_CTRL),
                                      "%s.input2X" % (self.FK_CTRL_animData_MultNode_Trans), force=True)
                    mayac.connectAttr("%s.translateY" % (self.AnimData_Joint),
                                      "%s.input1Y" % (self.FK_CTRL_animData_MultNode_Trans), force=True)
                    mayac.connectAttr("%s.AnimDataMultTrans" % (self.FK_CTRL),
                                      "%s.input2Y" % (self.FK_CTRL_animData_MultNode_Trans), force=True)
                    mayac.connectAttr("%s.translateZ" % (self.AnimData_Joint),
                                      "%s.input1Z" % (self.FK_CTRL_animData_MultNode_Trans), force=True)
                    mayac.connectAttr("%s.AnimDataMultTrans" % (self.FK_CTRL),
                                      "%s.input2Z" % (self.FK_CTRL_animData_MultNode_Trans), force=True)

                    mayac.connectAttr("%s.outputX" % (self.FK_CTRL_animData_MultNode_Trans),
                                      "%s.translateX" % (self.FK_CTRL_POS_GRP), force=True)
                    mayac.connectAttr("%s.outputY" % (self.FK_CTRL_animData_MultNode_Trans),
                                      "%s.translateY" % (self.FK_CTRL_POS_GRP), force=True)
                    mayac.connectAttr("%s.outputZ" % (self.FK_CTRL_animData_MultNode_Trans),
                                      "%s.translateZ" % (self.FK_CTRL_POS_GRP), force=True)
                    mayac.connectAttr("%s.outputX" % (self.FK_CTRL_animData_MultNode_Trans),
                                      "%s.translateX" % (self.IK_Dummy_Joint), force=True)
                    mayac.connectAttr("%s.outputY" % (self.FK_CTRL_animData_MultNode_Trans),
                                      "%s.translateY" % (self.IK_Dummy_Joint), force=True)
                    mayac.connectAttr("%s.outputZ" % (self.FK_CTRL_animData_MultNode_Trans),
                                      "%s.translateZ" % (self.IK_Dummy_Joint), force=True)
                    ...
                    temp = mayac.orientConstraint(self.FK_CTRL, self.Bind_Joint, mo=True,
                                                  name="%s_Constraint" % (self.nodeName))
                    self.Constraint = temp[0]
                    mayac.setAttr("%s.offsetX" % (self.Constraint), 0)
                    mayac.setAttr("%s.offsetY" % (self.Constraint), 0)
                ...
                temp = mayac.parentConstraint(self.FK_CTRL, self.FK_Joint, mo=True,
                                              name="%s_FK_Constraint" % (self.nodeName))
                self.FK_Constraint = temp[0]
                temp = mayac.parentConstraint(self.FK_Joint, self.Bind_Joint, mo=True,
                                              name="%s_%s_Constraint" % (self.nodeName, switchName))
                self.Constraint = temp[0]
            else:
                temp = mayac.orientConstraint(self.FK_CTRL, self.FK_Joint, mo=True,
                                              name="%s_FK_Constraint" % (self.nodeName))
                self.FK_Constraint = temp[0]
                temp = mayac.orientConstraint(self.FK_Joint, self.Bind_Joint, mo=True,
                                              name="%s_%s_Constraint" % (self.nodeName, switchName))
                self.Constraint = temp[0]
                mayac.setAttr("%s.offsetX" % (self.Constraint), 0)
                mayac.setAttr("%s.offsetY" % (self.Constraint), 0)
        ...
        if self.IK_CTRL:
            ...
            if "Foot" in self.nodeName:
                self.IK_CTRL_ReorientGRP = DJB_createGroup(transform=self.IK_CTRL,
                                                           fullName=self.IK_CTRL + "_Reorient_GRP")
                self.IK_CTRL_inRig_CONST_GRP = DJB_createGroup(transform=self.IK_CTRL_ReorientGRP,
                                                               fullName=self.IK_CTRL + "_In_Rig_CONST_GRP")
                ...
                self.IK_CTRL_inRig_CONST_GRP = DJB_createGroup(transform=self.IK_CTRL,
                                                               fullName=self.IK_CTRL + "_In_Rig_CONST_GRP")
            self.IK_CTRL_animData_CONST_GRP = DJB_createGroup(transform=self.IK_CTRL_inRig_CONST_GRP,
                                                              fullName=self.IK_CTRL + "_AnimData_CONST_GRP")
            self.IK_CTRL_animData_MultNode = mayac.createNode('multiplyDivide', n=self.IK_CTRL + "_AnimData_MultNode")
            self.IK_CTRL_POS_GRP = DJB_createGroup(transform=self.IK_CTRL_animData_CONST_GRP,
                                                   fullName=self.IK_CTRL + "_POS_GRP")
            ...
            mayac.connectAttr("%s.rotateX" % (self.AnimData_Joint), "%s.input1X" % (self.IK_CTRL_animData_MultNode),
                              force=True)
            mayac.connectAttr("%s.AnimDataMult" % (self.IK_CTRL), "%s.input2X" % (self.IK_CTRL_animData_MultNode),
                              force=True)
            mayac.connectAttr("%s.rotateY" % (self.AnimData_Joint), "%s.input1Y" % (self.IK_CTRL_animData_MultNode),
                              force=True)
            mayac.connectAttr("%s.AnimDataMult" % (self.IK_CTRL), "%s.input2Y" % (self.IK_CTRL_animData_MultNode),
                              force=True)
            mayac.connectAttr("%s.rotateZ" % (self.AnimData_Joint), "%s.input1Z" % (self.IK_CTRL_animData_MultNode),
                              force=True)
            mayac.connectAttr("%s.AnimDataMult" % (self.IK_CTRL), "%s.input2Z" % (self.IK_CTRL_animData_MultNode),
                              force=True)

            mayac.connectAttr("%s.outputX" % (self.IK_CTRL_animData_MultNode),
                              "%s.rotateX" % (self.IK_CTRL_animData_CONST_GRP), force=True)
            mayac.connectAttr("%s.outputY" % (self.IK_CTRL_animData_MultNode),
                              "%s.rotateY" % (self.IK_CTRL_animData_CONST_GRP), force=True)
            mayac.connectAttr("%s.outputZ" % (self.IK_CTRL_animData_MultNode),
                              "%s.rotateZ" % (self.IK_CTRL_animData_CONST_GRP), force=True)

            if "Hand" in self.nodeName or "ForeArm" in self.nodeName or "Foot" in self.nodeName or "Leg" in self.nodeName:
                self.IK_CTRL_parent_animData_CONST_GRP = DJB_createGroup(transform=self.IK_CTRL_POS_GRP,
                                                                         fullName=self.IK_CTRL + "_parent_AnimData_CONST_GRP")
                ...
                self.IK_CTRL_parent_animData_MultNode = mayac.createNode('multiplyDivide',
                                                                         n=self.IK_CTRL + "_parent_AnimData_MultNode")
                self.IK_CTRL_parent_Global_POS_GRP = DJB_createGroup(transform=self.IK_CTRL_parent_animData_CONST_GRP,
                                                                     fullName=self.IK_CTRL + "_parent_Global_POS_GRP")
                self.IK_CTRL_parent_POS_GRP = DJB_createGroup(transform=self.IK_CTRL_parent_Global_POS_GRP,
                                                              fullName=self.IK_CTRL + "_parent_POS_GRP")
                ...
                mayac.connectAttr("%s.rotateX" % (self.parent.AnimData_Joint),
                                  "%s.input1X" % (self.IK_CTRL_parent_animData_MultNode), force=True)
                mayac.connectAttr("%s.AnimDataMult" % (self.IK_CTRL),
                                  "%s.input2X" % (self.IK_CTRL_parent_animData_MultNode), force=True)
                mayac.connectAttr("%s.rotateY" % (self.parent.AnimData_Joint),
                                  "%s.input1Y" % (self.IK_CTRL_parent_animData_MultNode), force=True)
                mayac.connectAttr("%s.AnimDataMult" % (self.IK_CTRL),
                                  "%s.input2Y" % (self.IK_CTRL_parent_animData_MultNode), force=True)
                mayac.connectAttr("%s.rotateZ" % (self.parent.AnimData_Joint),
                                  "%s.input1Z" % (self.IK_CTRL_parent_animData_MultNode), force=True)
                mayac.connectAttr("%s.AnimDataMult" % (self.IK_CTRL),
                                  "%s.input2Z" % (self.IK_CTRL_parent_animData_MultNode), force=True)

                mayac.connectAttr("%s.outputX" % (self.IK_CTRL_parent_animData_MultNode),
                                  "%s.rotateX" % (self.IK_CTRL_parent_animData_CONST_GRP), force=True)
                mayac.connectAttr("%s.outputY" % (self.IK_CTRL_parent_animData_MultNode),
                                  "%s.rotateY" % (self.IK_CTRL_parent_animData_CONST_GRP), force=True)
                mayac.connectAttr("%s.outputZ" % (self.IK_CTRL_parent_animData_MultNode),
                                  "%s.rotateZ" % (self.IK_CTRL_parent_animData_CONST_GRP), force=True)

                mayac.addAttr(self.IK_CTRL, longName='ParentToGlobal', defaultValue=0.0, minValue=0, maxValue=1.0,
                              keyable=True)

                if "ForeArm" in self.nodeName:
                    mayac.addAttr(self.IK_CTRL, longName='FollowHand', defaultValue=0.0, minValue=0, maxValue=1.0,
                                  keyable=True)
                ...
                if "Leg" in self.nodeName:
                    mayac.addAttr(self.IK_CTRL, longName='FollowFoot', defaultValue=0.0, minValue=0, maxValue=1.0,
                                  keyable=True)
                    ...
                    self.Follow_Knee_GRP = DJB_createGroup(transform=None, fullName=self.IK_CTRL + "_Follow_Knee_GRP",
                                                           pivotFrom=self.FK_CTRL)
                    self.Follow_Foot_GRP = DJB_createGroup(transform=self.Follow_Knee_GRP,
                                                           fullName=self.IK_CTRL + "_Follow_Foot_GRP",
                                                           pivotFrom=self.FK_CTRL)
                    # set rotation orders
                    print('#set rotation orders')
                    ...
                    parentPOS = mayac.xform(self.parent.Bind_Joint, query=True, absolute=True, worldSpace=True,
                                            translation=True)
                    tempDistance = math.sqrt(
                        (selfPOS[0] - parentPOS[0]) * (selfPOS[0] - parentPOS[0]) + (selfPOS[1] - parentPOS[1]) * (
                        selfPOS[1] - parentPOS[1]) + (selfPOS[2] - parentPOS[2]) * (selfPOS[2] - parentPOS[2]))
                ...
                if "Hand" in self.nodeName or "Foot" in self.nodeName:
                    self.IK_CTRL_grandparent_inRig_CONST_GRP = DJB_createGroup(transform=self.IK_CTRL_parent_POS_GRP,
                                                                               fullName=self.IK_CTRL + "_grandparent_inRig_CONST_GRP",
                                                                               pivotFrom=self.parent.parent.Bind_Joint)

                    # place parent GRP
                    print('#place parent GRP')
                    temp = mayac.duplicate(self.parent.parent.Bind_Joint, parentOnly=True,
                                           name="UnRotate" + self.nodeName)
                    ...
                    self.IK_CTRL_grandparent_animData_CONST_GRP = DJB_createGroup(
                        transform=self.IK_CTRL_grandparent_inRig_CONST_GRP,
                        fullName=self.IK_CTRL + "_grandparent_AnimData_CONST_GRP")
                    self.IK_CTRL_grandparent_animData_MultNode = mayac.createNode('multiplyDivide',
                                                                                  n=self.IK_CTRL + "_grandparent_AnimData_MultNode")
                    self.IK_CTRL_grandparent_Global_POS_GRP = DJB_createGroup(
                        transform=self.IK_CTRL_grandparent_animData_CONST_GRP,
                        fullName=self.IK_CTRL + "_grandparent_Global_POS_GRP")
                    self.IK_CTRL_grandparent_POS_GRP = DJB_createGroup(
                        transform=self.IK_CTRL_grandparent_Global_POS_GRP,
                        fullName=self.IK_CTRL + "_grandparent_POS_GRP")

                    # set rotation orders
                    mayac.setAttr("%s.rotateOrder" % (self.IK_CTRL_grandparent_inRig_CONST_GRP),
                                  self.parent.parent.rotOrder)
                    mayac.setAttr("%s.rotateOrder" % (self.IK_CTRL_grandparent_animData_CONST_GRP),
                                  self.parent.parent.rotOrder)
                    mayac.setAttr("%s.rotateOrder" % (self.IK_CTRL_grandparent_Global_POS_GRP),
                                  self.parent.parent.rotOrder)
                    mayac.setAttr("%s.rotateOrder" % (self.IK_CTRL_grandparent_POS_GRP), self.parent.parent.rotOrder)

                    mayac.connectAttr("%s.rotateX" % (self.parent.parent.AnimData_Joint),
                                      "%s.input1X" % (self.IK_CTRL_grandparent_animData_MultNode), force=True)
                    mayac.connectAttr("%s.AnimDataMult" % (self.IK_CTRL),
                                      "%s.input2X" % (self.IK_CTRL_grandparent_animData_MultNode), force=True)
                    mayac.connectAttr("%s.rotateY" % (self.parent.parent.AnimData_Joint),
                                      "%s.input1Y" % (self.IK_CTRL_grandparent_animData_MultNode), force=True)
                    mayac.connectAttr("%s.AnimDataMult" % (self.IK_CTRL),
                                      "%s.input2Y" % (self.IK_CTRL_grandparent_animData_MultNode), force=True)
                    mayac.connectAttr("%s.rotateZ" % (self.parent.parent.AnimData_Joint),
                                      "%s.input1Z" % (self.IK_CTRL_grandparent_animData_MultNode), force=True)
                    mayac.connectAttr("%s.AnimDataMult" % (self.IK_CTRL),
                                      "%s.input2Z" % (self.IK_CTRL_grandparent_animData_MultNode), force=True)

                    mayac.connectAttr("%s.outputX" % (self.IK_CTRL_grandparent_animData_MultNode),
                                      "%s.rotateX" % (self.IK_CTRL_grandparent_animData_CONST_GRP), force=True)
                    mayac.connectAttr("%s.outputY" % (self.IK_CTRL_grandparent_animData_MultNode),
                                      "%s.rotateY" % (self.IK_CTRL_grandparent_animData_CONST_GRP), force=True)
                    mayac.connectAttr("%s.outputZ" % (self.IK_CTRL_grandparent_animData_MultNode),
                                      "%s.rotateZ" % (self.IK_CTRL_grandparent_animData_CONST_GRP), force=True)

                    temp = mayac.ikHandle(n="%s_IK_Handle" % (self.nodeName), sj=self.parent.parent.IK_Joint,
                                          ee=self.IK_Joint, solver="ikRPsolver", weight=1)
                    ...
                    if "Foot" in self.nodeName:
                        mayac.addAttr(self.IK_CTRL, longName='FootControls', defaultValue=0.0, hidden=False,
                                      keyable=True)
                        ...
                        mayac.addAttr(self.IK_CTRL, longName='ToeSideToSide', defaultValue=0.0, hidden=False,
                                      keyable=True)
                        ...
                        mayac.addAttr(self.IK_CTRL, longName='HipSideToSide', defaultValue=0.0, hidden=False,
                                      keyable=True)
                        mayac.addAttr(self.IK_CTRL, longName='HipBackToFront', defaultValue=0.0, hidden=False,
                                      keyable=True)
        ...
        if self.Options_CTRL:
            ...
            mayac.parentConstraint(self.Bind_Joint, self.Options_CTRL, mo=True,
                                   name="%s_Constraint" % (self.Options_CTRL))
            # add attributes
            ...
            if "Hand" in self.nodeName:
                mayac.addAttr(self.Options_CTRL, longName='FingerControls', defaultValue=0.0, hidden=False,
                              keyable=True)
                mayac.setAttr("%s.FingerControls" % (self.Options_CTRL), lock=True)
                mayac.addAttr(self.Options_CTRL, longName='ThumbCurl', defaultValue=0.0, min=-10.0, max=10.0,
                              hidden=False, keyable=True)
                mayac.addAttr(self.Options_CTRL, longName='IndexCurl', defaultValue=0.0, min=-10.0, max=10.0,
                              hidden=False, keyable=True)
                mayac.addAttr(self.Options_CTRL, longName='MiddleCurl', defaultValue=0.0, min=-10.0, max=10.0,
                              hidden=False, keyable=True)
                mayac.addAttr(self.Options_CTRL, longName='RingCurl', defaultValue=0.0, min=-10.0, max=10.0,
                              hidden=False, keyable=True)
                mayac.addAttr(self.Options_CTRL, longName='PinkyCurl', defaultValue=0.0, min=-10.0, max=10.0,
                              hidden=False, keyable=True)
                mayac.addAttr(self.Options_CTRL, longName='Sway', defaultValue=0.0, min=-10.0, max=10.0, hidden=False,
                              keyable=True)
                mayac.addAttr(self.Options_CTRL, longName='Spread', defaultValue=0.0, min=-10.0, max=10.0, hidden=False,
                              keyable=True)
...
class DJB_Character():
    def __init__(self, infoNode_=None, hulaOption_=0, name_="Character"):
        ...
        if not self.infoNode:
            ...
            self.jointNamespace = JOINT_NAMESPACE = DJB_findBeforeSeparator(self.joints[1],
                                                                            ':')  # changed from 0 for one special case - zombie lores
            ...
            self.LeftToe_End = DJB_CharacterNode("LeftToe_End", parent=self.LeftToeBase,
                                                 alias_=["toe_L", "LeftFootToeBase_End"])
            ...
            self.RightToe_End = DJB_CharacterNode("RightToe_End", parent=self.RightToeBase,
                                                  alias_=["toe_R", "RightFootToeBase_End"])

            # educated guess with 2 samples for rig type
            if mayac.getAttr("%s.jointOrient" % self.LeftUpLeg.Bind_Joint)[0] == (0, 0, 0) and \
                            mayac.getAttr("%s.jointOrient" % self.RightArm.Bind_Joint)[0] == (0, 0, 0):
                self.rigType = "World"
            else:
        ...
        # there is an infoNode for this Character
            ...
            self.Root = DJB_CharacterNode("Root", infoNode_=attrToPy("%s.Root" % (self.infoNode)),
                                          nameSpace_=self.characterNameSpace)
            if self.hulaOption:
                if not self.Root.Bind_Joint:
                    return None
                self.Hips = DJB_CharacterNode("Hips", parent=self.Root, infoNode_=attrToPy("%s.Hips" % (self.infoNode)),
                                              nameSpace_=self.characterNameSpace)
                self.Spine = DJB_CharacterNode("Spine", parent=self.Root,
                                               infoNode_=attrToPy("%s.Spine" % (self.infoNode)),
                                               nameSpace_=self.characterNameSpace)
            else:
                self.Hips = DJB_CharacterNode("Hips", infoNode_=attrToPy("%s.Hips" % (self.infoNode)),
                                              nameSpace_=self.characterNameSpace)
                self.Spine = DJB_CharacterNode("Spine", parent=self.Hips,
                                               infoNode_=attrToPy("%s.Spine" % (self.infoNode)),
                                               nameSpace_=self.characterNameSpace)
            self.Spine1 = DJB_CharacterNode("Spine1", parent=self.Spine,
                                            infoNode_=attrToPy("%s.Spine1" % (self.infoNode)),
                                            nameSpace_=self.characterNameSpace)
            self.Spine2 = DJB_CharacterNode("Spine2", parent=self.Spine1, optional_=1,
                                            infoNode_=attrToPy("%s.Spine2" % (self.infoNode)),
                                            nameSpace_=self.characterNameSpace)
            if self.Spine2.Bind_Joint:
                self.Spine3 = DJB_CharacterNode("Spine3", parent=self.Spine2, optional_=1,
                                                infoNode_=attrToPy("%s.Spine3" % (self.infoNode)),
                                                nameSpace_=self.characterNameSpace)
                if self.Spine3.Bind_Joint:
                    self.Neck = DJB_CharacterNode("Neck", parent=self.Spine3,
                                                  infoNode_=attrToPy("%s.Neck" % (self.infoNode)),
                                                  nameSpace_=self.characterNameSpace)
                else:
                    self.Neck = DJB_CharacterNode("Neck", parent=self.Spine2,
                                                  infoNode_=attrToPy("%s.Neck" % (self.infoNode)),
                ...
                self.Neck = DJB_CharacterNode("Neck", parent=self.Spine1,
                                              ...
                                              nameSpace_=self.characterNameSpace)
            self.Neck1 = DJB_CharacterNode("Neck1", parent=self.Neck, optional_=1,
                                           infoNode_=attrToPy("%s.Neck1" % (self.infoNode)),
                                           nameSpace_=self.characterNameSpace)
            if self.Neck1.Bind_Joint:
                self.Head = DJB_CharacterNode("Head", parent=self.Neck1,
                                              infoNode_=attrToPy("%s.Head" % (self.infoNode)),
                                              nameSpace_=self.characterNameSpace)
            else:
                self.Head = DJB_CharacterNode("Head", parent=self.Neck, infoNode_=attrToPy("%s.Head" % (self.infoNode)),
                                              nameSpace_=self.characterNameSpace)
            self.HeadTop_End = DJB_CharacterNode("HeadTop_End", parent=self.Head,
                                                 infoNode_=attrToPy("%s.HeadTop_End" % (self.infoNode)),
                                                 nameSpace_=self.characterNameSpace)
            if self.Spine2.Bind_Joint:
                if self.Spine3.Bind_Joint:
                    self.LeftShoulder = DJB_CharacterNode("LeftShoulder", parent=self.Spine3,
                                                          infoNode_=attrToPy("%s.LeftShoulder" % (self.infoNode)),
                                                          nameSpace_=self.characterNameSpace)
                else:
                    self.LeftShoulder = DJB_CharacterNode("LeftShoulder", parent=self.Spine2,
                                                          infoNode_=attrToPy("%s.LeftShoulder" % (self.infoNode)),
                ...
                self.LeftShoulder = DJB_CharacterNode("LeftShoulder", parent=self.Spine1,
                                                      ...
                                                      nameSpace_=self.characterNameSpace)
            self.LeftArm = DJB_CharacterNode("LeftArm", parent=self.LeftShoulder,
                                             infoNode_=attrToPy("%s.LeftArm" % (self.infoNode)),
                                             nameSpace_=self.characterNameSpace)
            self.LeftForeArm = DJB_CharacterNode("LeftForeArm", parent=self.LeftArm,
                                                 infoNode_=attrToPy("%s.LeftForeArm" % (self.infoNode)),
                                                 nameSpace_=self.characterNameSpace)
            self.LeftHand = DJB_CharacterNode("LeftHand", parent=self.LeftForeArm,
                                              infoNode_=attrToPy("%s.LeftHand" % (self.infoNode)),
                                              nameSpace_=self.characterNameSpace)
            self.LeftHandThumb1 = DJB_CharacterNode("LeftHandThumb1", optional_=1, parent=self.LeftHand,
                                                    infoNode_=attrToPy("%s.LeftHandThumb1" % (self.infoNode)),
                                                    nameSpace_=self.characterNameSpace)
            self.LeftHandThumb2 = DJB_CharacterNode("LeftHandThumb2", optional_=1, parent=self.LeftHandThumb1,
                                                    infoNode_=attrToPy("%s.LeftHandThumb2" % (self.infoNode)),
                                                    nameSpace_=self.characterNameSpace)
            self.LeftHandThumb3 = DJB_CharacterNode("LeftHandThumb3", optional_=1, parent=self.LeftHandThumb2,
                                                    infoNode_=attrToPy("%s.LeftHandThumb3" % (self.infoNode)),
                                                    nameSpace_=self.characterNameSpace)
            self.LeftHandThumb4 = DJB_CharacterNode("LeftHandThumb4", optional_=1, parent=self.LeftHandThumb3,
                                                    infoNode_=attrToPy("%s.LeftHandThumb4" % (self.infoNode)),
                                                    nameSpace_=self.characterNameSpace)
            self.LeftHandIndex1 = DJB_CharacterNode("LeftHandIndex1", optional_=1, parent=self.LeftHand,
                                                    infoNode_=attrToPy("%s.LeftHandIndex1" % (self.infoNode)),
                                                    nameSpace_=self.characterNameSpace)
            self.LeftHandIndex2 = DJB_CharacterNode("LeftHandIndex2", optional_=1, parent=self.LeftHandIndex1,
                                                    infoNode_=attrToPy("%s.LeftHandIndex2" % (self.infoNode)),
                                                    nameSpace_=self.characterNameSpace)
            self.LeftHandIndex3 = DJB_CharacterNode("LeftHandIndex3", optional_=1, parent=self.LeftHandIndex2,
                                                    infoNode_=attrToPy("%s.LeftHandIndex3" % (self.infoNode)),
                                                    nameSpace_=self.characterNameSpace)
            self.LeftHandIndex4 = DJB_CharacterNode("LeftHandIndex4", optional_=1, parent=self.LeftHandIndex3,
                                                    infoNode_=attrToPy("%s.LeftHandIndex4" % (self.infoNode)),
                                                    nameSpace_=self.characterNameSpace)
            self.LeftHandMiddle1 = DJB_CharacterNode("LeftHandMiddle1", optional_=1, parent=self.LeftHand,
                                                     infoNode_=attrToPy("%s.LeftHandMiddle1" % (self.infoNode)),
                                                     nameSpace_=self.characterNameSpace)
            self.LeftHandMiddle2 = DJB_CharacterNode("LeftHandMiddle2", optional_=1, parent=self.LeftHandMiddle1,
                                                     infoNode_=attrToPy("%s.LeftHandMiddle2" % (self.infoNode)),
                                                     nameSpace_=self.characterNameSpace)
            self.LeftHandMiddle3 = DJB_CharacterNode("LeftHandMiddle3", optional_=1, parent=self.LeftHandMiddle2,
                                                     infoNode_=attrToPy("%s.LeftHandMiddle3" % (self.infoNode)),
                                                     nameSpace_=self.characterNameSpace)
            self.LeftHandMiddle4 = DJB_CharacterNode("LeftHandMiddle4", optional_=1, parent=self.LeftHandMiddle3,
                                                     infoNode_=attrToPy("%s.LeftHandMiddle4" % (self.infoNode)),
                                                     nameSpace_=self.characterNameSpace)
            self.LeftHandRing1 = DJB_CharacterNode("LeftHandRing1", optional_=1, parent=self.LeftHand,
                                                   infoNode_=attrToPy("%s.LeftHandRing1" % (self.infoNode)),
                                                   nameSpace_=self.characterNameSpace)
            self.LeftHandRing2 = DJB_CharacterNode("LeftHandRing2", optional_=1, parent=self.LeftHandRing1,
                                                   infoNode_=attrToPy("%s.LeftHandRing2" % (self.infoNode)),
                                                   nameSpace_=self.characterNameSpace)
            self.LeftHandRing3 = DJB_CharacterNode("LeftHandRing3", optional_=1, parent=self.LeftHandRing2,
                                                   infoNode_=attrToPy("%s.LeftHandRing3" % (self.infoNode)),
                                                   nameSpace_=self.characterNameSpace)
            self.LeftHandRing4 = DJB_CharacterNode("LeftHandRing4", optional_=1, parent=self.LeftHandRing3,
                                                   infoNode_=attrToPy("%s.LeftHandRing4" % (self.infoNode)),
                                                   nameSpace_=self.characterNameSpace)
            self.LeftHandPinky1 = DJB_CharacterNode("LeftHandPinky1", optional_=1, parent=self.LeftHand,
                                                    infoNode_=attrToPy("%s.LeftHandPinky1" % (self.infoNode)),
                                                    nameSpace_=self.characterNameSpace)
            self.LeftHandPinky2 = DJB_CharacterNode("LeftHandPinky2", optional_=1, parent=self.LeftHandPinky1,
                                                    infoNode_=attrToPy("%s.LeftHandPinky2" % (self.infoNode)),
                                                    nameSpace_=self.characterNameSpace)
            self.LeftHandPinky3 = DJB_CharacterNode("LeftHandPinky3", optional_=1, parent=self.LeftHandPinky2,
                                                    infoNode_=attrToPy("%s.LeftHandPinky3" % (self.infoNode)),
                                                    nameSpace_=self.characterNameSpace)
            self.LeftHandPinky4 = DJB_CharacterNode("LeftHandPinky4", optional_=1, parent=self.LeftHandPinky3,
                                                    infoNode_=attrToPy("%s.LeftHandPinky4" % (self.infoNode)),
                                                    nameSpace_=self.characterNameSpace)
            if self.Spine2.Bind_Joint:
                if self.Spine3.Bind_Joint:
                    self.RightShoulder = DJB_CharacterNode("RightShoulder", parent=self.Spine3,
                                                           infoNode_=attrToPy("%s.RightShoulder" % (self.infoNode)),
                                                           nameSpace_=self.characterNameSpace)
                else:
                    self.RightShoulder = DJB_CharacterNode("RightShoulder", parent=self.Spine2,
                                                           infoNode_=attrToPy("%s.RightShoulder" % (self.infoNode)),
                ...
                self.RightShoulder = DJB_CharacterNode("RightShoulder", parent=self.Spine1,
                                                       ...
                                                       nameSpace_=self.characterNameSpace)
            self.RightArm = DJB_CharacterNode("RightArm", parent=self.RightShoulder,
                                              infoNode_=attrToPy("%s.RightArm" % (self.infoNode)),
                                              nameSpace_=self.characterNameSpace)
            self.RightForeArm = DJB_CharacterNode("RightForeArm", parent=self.RightArm,
                                                  infoNode_=attrToPy("%s.RightForeArm" % (self.infoNode)),
                                                  nameSpace_=self.characterNameSpace)
            self.RightHand = DJB_CharacterNode("RightHand", parent=self.RightForeArm,
                                               infoNode_=attrToPy("%s.RightHand" % (self.infoNode)),
                                               nameSpace_=self.characterNameSpace)
            self.RightHandThumb1 = DJB_CharacterNode("RightHandThumb1", optional_=1, parent=self.RightHand,
                                                     infoNode_=attrToPy("%s.RightHandThumb1" % (self.infoNode)),
                                                     nameSpace_=self.characterNameSpace)
            self.RightHandThumb2 = DJB_CharacterNode("RightHandThumb2", optional_=1, parent=self.RightHandThumb1,
                                                     infoNode_=attrToPy("%s.RightHandThumb2" % (self.infoNode)),
                                                     nameSpace_=self.characterNameSpace)
            self.RightHandThumb3 = DJB_CharacterNode("RightHandThumb3", optional_=1, parent=self.RightHandThumb2,
                                                     infoNode_=attrToPy("%s.RightHandThumb3" % (self.infoNode)),
                                                     nameSpace_=self.characterNameSpace)
            self.RightHandThumb4 = DJB_CharacterNode("RightHandThumb4", optional_=1, parent=self.RightHandThumb3,
                                                     infoNode_=attrToPy("%s.RightHandThumb4" % (self.infoNode)),
                                                     nameSpace_=self.characterNameSpace)
            self.RightHandIndex1 = DJB_CharacterNode("RightHandIndex1", optional_=1, parent=self.RightHand,
                                                     infoNode_=attrToPy("%s.RightHandIndex1" % (self.infoNode)),
                                                     nameSpace_=self.characterNameSpace)
            self.RightHandIndex2 = DJB_CharacterNode("RightHandIndex2", optional_=1, parent=self.RightHandIndex1,
                                                     infoNode_=attrToPy("%s.RightHandIndex2" % (self.infoNode)),
                                                     nameSpace_=self.characterNameSpace)
            self.RightHandIndex3 = DJB_CharacterNode("RightHandIndex3", optional_=1, parent=self.RightHandIndex2,
                                                     infoNode_=attrToPy("%s.RightHandIndex3" % (self.infoNode)),
                                                     nameSpace_=self.characterNameSpace)
            self.RightHandIndex4 = DJB_CharacterNode("RightHandIndex4", optional_=1, parent=self.RightHandIndex3,
                                                     infoNode_=attrToPy("%s.RightHandIndex4" % (self.infoNode)),
                                                     nameSpace_=self.characterNameSpace)
            self.RightHandMiddle1 = DJB_CharacterNode("RightHandMiddle1", optional_=1, parent=self.RightHand,
                                                      infoNode_=attrToPy("%s.RightHandMiddle1" % (self.infoNode)),
                                                      nameSpace_=self.characterNameSpace)
            self.RightHandMiddle2 = DJB_CharacterNode("RightHandMiddle2", optional_=1, parent=self.RightHandMiddle1,
                                                      infoNode_=attrToPy("%s.RightHandMiddle2" % (self.infoNode)),
                                                      nameSpace_=self.characterNameSpace)
            self.RightHandMiddle3 = DJB_CharacterNode("RightHandMiddle3", optional_=1, parent=self.RightHandMiddle2,
                                                      infoNode_=attrToPy("%s.RightHandMiddle3" % (self.infoNode)),
                                                      nameSpace_=self.characterNameSpace)
            self.RightHandMiddle4 = DJB_CharacterNode("RightHandMiddle4", optional_=1, parent=self.RightHandMiddle3,
                                                      infoNode_=attrToPy("%s.RightHandMiddle4" % (self.infoNode)),
                                                      nameSpace_=self.characterNameSpace)
            self.RightHandRing1 = DJB_CharacterNode("RightHandRing1", optional_=1, parent=self.RightHand,
                                                    infoNode_=attrToPy("%s.RightHandRing1" % (self.infoNode)),
                                                    nameSpace_=self.characterNameSpace)
            self.RightHandRing2 = DJB_CharacterNode("RightHandRing2", optional_=1, parent=self.RightHandRing1,
                                                    infoNode_=attrToPy("%s.RightHandRing2" % (self.infoNode)),
                                                    nameSpace_=self.characterNameSpace)
            self.RightHandRing3 = DJB_CharacterNode("RightHandRing3", optional_=1, parent=self.RightHandRing2,
                                                    infoNode_=attrToPy("%s.RightHandRing3" % (self.infoNode)),
                                                    nameSpace_=self.characterNameSpace)
            self.RightHandRing4 = DJB_CharacterNode("RightHandRing4", optional_=1, parent=self.RightHandRing3,
                                                    infoNode_=attrToPy("%s.RightHandRing4" % (self.infoNode)),
                                                    nameSpace_=self.characterNameSpace)
            self.RightHandPinky1 = DJB_CharacterNode("RightHandPinky1", optional_=1, parent=self.RightHand,
                                                     infoNode_=attrToPy("%s.RightHandPinky1" % (self.infoNode)),
                                                     nameSpace_=self.characterNameSpace)
            self.RightHandPinky2 = DJB_CharacterNode("RightHandPinky2", optional_=1, parent=self.RightHandPinky1,
                                                     infoNode_=attrToPy("%s.RightHandPinky2" % (self.infoNode)),
                                                     nameSpace_=self.characterNameSpace)
            self.RightHandPinky3 = DJB_CharacterNode("RightHandPinky3", optional_=1, parent=self.RightHandPinky2,
                                                     infoNode_=attrToPy("%s.RightHandPinky3" % (self.infoNode)),
                                                     nameSpace_=self.characterNameSpace)
            self.RightHandPinky4 = DJB_CharacterNode("RightHandPinky4", optional_=1, parent=self.RightHandPinky3,
                                                     infoNode_=attrToPy("%s.RightHandPinky4" % (self.infoNode)),
                                                     nameSpace_=self.characterNameSpace)
            self.LeftUpLeg = DJB_CharacterNode("LeftUpLeg", parent=self.Hips,
                                               infoNode_=attrToPy("%s.LeftUpLeg" % (self.infoNode)),
                                               nameSpace_=self.characterNameSpace)
            self.LeftLeg = DJB_CharacterNode("LeftLeg", parent=self.LeftUpLeg,
                                             infoNode_=attrToPy("%s.LeftLeg" % (self.infoNode)),
                                             nameSpace_=self.characterNameSpace)
            self.LeftFoot = DJB_CharacterNode("LeftFoot", parent=self.LeftLeg,
                                              infoNode_=attrToPy("%s.LeftFoot" % (self.infoNode)),
                                              nameSpace_=self.characterNameSpace)
            self.LeftToeBase = DJB_CharacterNode("LeftToeBase", parent=self.LeftFoot,
                                                 infoNode_=attrToPy("%s.LeftToeBase" % (self.infoNode)),
                                                 nameSpace_=self.characterNameSpace)
            self.LeftToe_End = DJB_CharacterNode("LeftToe_End", parent=self.LeftToeBase,
                                                 infoNode_=attrToPy("%s.LeftToe_End" % (self.infoNode)),
                                                 nameSpace_=self.characterNameSpace)
            self.RightUpLeg = DJB_CharacterNode("RightUpLeg", parent=self.Hips,
                                                infoNode_=attrToPy("%s.RightUpLeg" % (self.infoNode)),
                                                nameSpace_=self.characterNameSpace)
            self.RightLeg = DJB_CharacterNode("RightLeg", parent=self.RightUpLeg,
                                              infoNode_=attrToPy("%s.RightLeg" % (self.infoNode)),
                                              nameSpace_=self.characterNameSpace)
            self.RightFoot = DJB_CharacterNode("RightFoot", parent=self.RightLeg,
                                               infoNode_=attrToPy("%s.RightFoot" % (self.infoNode)),
                                               nameSpace_=self.characterNameSpace)
            self.RightToeBase = DJB_CharacterNode("RightToeBase", parent=self.RightFoot,
                                                  infoNode_=attrToPy("%s.RightToeBase" % (self.infoNode)),
                                                  nameSpace_=self.characterNameSpace)
            self.RightToe_End = DJB_CharacterNode("RightToe_End", parent=self.RightToeBase,
                                                  infoNode_=attrToPy("%s.RightToe_End" % (self.infoNode)),
                                                  nameSpace_=self.characterNameSpace)
            ...
            self.Character_GRP = DJB_addNameSpace(self.characterNameSpace,
                                                  attrToPy("%s.Character_GRP" % (self.infoNode)))
            ...
            self.AnimData_Joint_GRP = DJB_addNameSpace(self.characterNameSpace,
                                                       attrToPy("%s.AnimData_Joint_GRP" % (self.infoNode)))
            self.Bind_Joint_GRP = DJB_addNameSpace(self.characterNameSpace,
                                                   attrToPy("%s.Bind_Joint_GRP" % (self.infoNode)))
            ...
            self.LeftArm_Switch_Reverse = DJB_addNameSpace(self.characterNameSpace,
                                                           attrToPy("%s.Misc_GRP" % (self.infoNode)))
            self.RightArm_Switch_Reverse = DJB_addNameSpace(self.characterNameSpace,
                                                            attrToPy("%s.RightArm_Switch_Reverse" % (self.infoNode)))
            self.LeftLeg_Switch_Reverse = DJB_addNameSpace(self.characterNameSpace,
                                                           attrToPy("%s.LeftLeg_Switch_Reverse" % (self.infoNode)))
            self.RightLeg_Switch_Reverse = DJB_addNameSpace(self.characterNameSpace,
                                                            attrToPy("%s.RightLeg_Switch_Reverse" % (self.infoNode)))
            self.Bind_Joint_SelectSet = DJB_addNameSpace(self.characterNameSpace,
                                                         attrToPy("%s.Bind_Joint_SelectSet" % (self.infoNode)))
            self.AnimData_Joint_SelectSet = DJB_addNameSpace(self.characterNameSpace,
                                                             attrToPy("%s.AnimData_Joint_SelectSet" % (self.infoNode)))
            self.Controls_SelectSet = DJB_addNameSpace(self.characterNameSpace,
                                                       attrToPy("%s.Controls_SelectSet" % (self.infoNode)))
            self.Geo_SelectSet = DJB_addNameSpace(self.characterNameSpace,
                                                  attrToPy("%s.Geo_SelectSet" % (self.infoNode)))
            self.Left_Toe_IK_AnimData_GRP = DJB_addNameSpace(self.characterNameSpace,
                                                             attrToPy("%s.Left_Toe_IK_AnimData_GRP" % (self.infoNode)))
            self.Left_Toe_IK_CTRL = DJB_addNameSpace(self.characterNameSpace,
                                                     attrToPy("%s.Left_Toe_IK_CTRL" % (self.infoNode)))
            self.Left_ToeBase_IK_AnimData_GRP = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.Left_ToeBase_IK_AnimData_GRP" % (self.infoNode)))
            self.Left_IK_ToeBase_animData_MultNode = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.Left_IK_ToeBase_animData_MultNode" % (self.infoNode)))
            self.Left_ToeBase_IK_CTRL = DJB_addNameSpace(self.characterNameSpace,
                                                         attrToPy("%s.Left_ToeBase_IK_CTRL" % (self.infoNode)))
            self.Left_Ankle_IK_AnimData_GRP = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.Left_Ankle_IK_AnimData_GRP" % (self.infoNode)))
            self.Left_Ankle_IK_CTRL = DJB_addNameSpace(self.characterNameSpace,
                                                       attrToPy("%s.Left_Ankle_IK_CTRL" % (self.infoNode)))
            self.Left_ToeBase_IkHandle = DJB_addNameSpace(self.characterNameSpace,
                                                          attrToPy("%s.Left_ToeBase_IkHandle" % (self.infoNode)))
            self.Left_ToeEnd_IkHandle = DJB_addNameSpace(self.characterNameSpace,
                                                         attrToPy("%s.Left_ToeEnd_IkHandle" % (self.infoNode)))
            self.Right_Toe_IK_AnimData_GRP = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.Right_Toe_IK_AnimData_GRP" % (self.infoNode)))
            self.Right_Toe_IK_CTRL = DJB_addNameSpace(self.characterNameSpace,
                                                      attrToPy("%s.Right_Toe_IK_CTRL" % (self.infoNode)))
            self.Right_ToeBase_IK_AnimData_GRP = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.Right_ToeBase_IK_AnimData_GRP" % (self.infoNode)))
            self.Right_IK_ToeBase_animData_MultNode = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.Right_IK_ToeBase_animData_MultNode" % (self.infoNode)))
            self.Right_ToeBase_IK_CTRL = DJB_addNameSpace(self.characterNameSpace,
                                                          attrToPy("%s.Right_ToeBase_IK_CTRL" % (self.infoNode)))
            self.Right_Ankle_IK_AnimData_GRP = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.Right_Ankle_IK_AnimData_GRP" % (self.infoNode)))
            self.Right_Ankle_IK_CTRL = DJB_addNameSpace(self.characterNameSpace,
                                                        attrToPy("%s.Right_Ankle_IK_AnimData_GRP" % (self.infoNode)))
            self.Right_ToeBase_IkHandle = DJB_addNameSpace(self.characterNameSpace,
                                                           attrToPy("%s.Right_ToeBase_IkHandle" % (self.infoNode)))
            self.Right_ToeEnd_IkHandle = DJB_addNameSpace(self.characterNameSpace,
                                                          attrToPy("%s.Right_ToeEnd_IkHandle" % (self.infoNode)))
            self.LeftHand_CTRLs_GRP = DJB_addNameSpace(self.characterNameSpace,
                                                       attrToPy("%s.LeftHand_CTRLs_GRP" % (self.infoNode)))
            self.RightHand_CTRLs_GRP = DJB_addNameSpace(self.characterNameSpace,
                                                        attrToPy("%s.RightHand_CTRLs_GRP" % (self.infoNode)))
            self.LeftFoot_FootRoll_MultNode = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.LeftFoot_FootRoll_MultNode" % (self.infoNode)))
            self.LeftFoot_ToeRoll_MultNode = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.LeftFoot_ToeRoll_MultNode" % (self.infoNode)))
            self.RightFoot_FootRoll_MultNode = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.RightFoot_FootRoll_MultNode" % (self.infoNode)))
            self.RightFoot_ToeRoll_MultNode = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.RightFoot_ToeRoll_MultNode" % (self.infoNode)))
            self.RightFoot_HipPivot_MultNode = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.RightFoot_HipPivot_MultNode" % (self.infoNode)))
            self.RightFoot_BallPivot_MultNode = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.RightFoot_BallPivot_MultNode" % (self.infoNode)))
            self.RightFoot_ToePivot_MultNode = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.RightFoot_ToePivot_MultNode" % (self.infoNode)))
            self.RightFoot_HipSideToSide_MultNode = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.RightFoot_HipSideToSide_MultNode" % (self.infoNode)))
            self.RightFoot_ToeRotate_MultNode = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.RightFoot_ToeRotate_MultNode" % (self.infoNode)))
            self.IK_Dummy_Joint_GRP = DJB_addNameSpace(self.characterNameSpace,
                                                       attrToPy("%s.IK_Dummy_Joint_GRP" % (self.infoNode)))
            self.LeftHand_grandparent_Constraint = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.LeftHand_grandparent_Constraint" % (self.infoNode)))
            self.LeftHand_grandparent_Constraint_Reverse = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.LeftHand_grandparent_Constraint_Reverse" % (self.infoNode)))
            self.RightHand_grandparent_Constraint = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.RightHand_grandparent_Constraint" % (self.infoNode)))
            self.RightHand_grandparent_Constraint_Reverse = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.RightHand_grandparent_Constraint_Reverse" % (self.infoNode)))
            self.LeftForeArm_grandparent_Constraint = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.LeftForeArm_grandparent_Constraint" % (self.infoNode)))
            self.LeftForeArm_grandparent_Constraint_Reverse = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.LeftForeArm_grandparent_Constraint_Reverse" % (self.infoNode)))
            self.RightForeArm_grandparent_Constraint = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.RightForeArm_grandparent_Constraint" % (self.infoNode)))
            self.RightForeArm_grandparent_Constraint_Reverse = DJB_addNameSpace(self.characterNameSpace, attrToPy(
                "%s.RightForeArm_grandparent_Constraint_Reverse" % (self.infoNode)))
            self.exportList = attrToPy("%s.exportList" % (self.infoNode))

            if attrToPy("%s.origAnim" % (self.infoNode)):
                if mayac.objExists(
                        DJB_addNameSpace(self.characterNameSpace, attrToPy("%s.origAnim" % (self.infoNode)))):
                    self.origAnim = DJB_addNameSpace(self.characterNameSpace, attrToPy("%s.origAnim" % (self.infoNode)))
                    self.origAnimation_Layer = DJB_addNameSpace(self.characterNameSpace,
                                                                attrToPy("%s.origAnimation_Layer" % (self.infoNode)))
                else:
            ...
            self.Bind_Joint_Layer = DJB_addNameSpace(self.characterNameSpace,
                                                     attrToPy("%s.Bind_Joint_Layer" % (self.infoNode)))
            self.fingerFlip = attrToPy("%s.fingerFlip" % (self.infoNode))

        self.bodyParts = []
        for bodyPart in (
        self.Root, self.Hips, self.Spine, self.Spine1, self.Spine2, self.Spine3, self.Neck, self.Neck1, self.Head,
        self.HeadTop_End, self.LeftShoulder,
        ...
        if self.infoNode:
            ...
            if extraJointInfoNodes:
                for extraJointInfoNode in extraJointInfoNodes:
                    ...
                    extraJointName = DJB_addNameSpace(self.characterNameSpace,
                                                      attrToPy("%s.nodeName" % (extraJointInfoNode)))
                    ...
                    if not parentShouldBe:
                        parentShouldBe = mayac.listRelatives(
                            DJB_addNameSpace(self.characterNameSpace, attrToPy("%s.Bind_Joint" % (extraJointInfoNode))),
                            parent=True)[0]
                    ...
                    self.ExtraJoints.append(
                        DJB_CharacterNode(extraJointName, parent=nodesParent, infoNode_=extraJointInfoNode,
                                          nameSpace_=self.characterNameSpace))
            else:
                self.ExtraJoints = None
    ...
    def fixArmsAndLegs(self):
        LAnklePosStart = mayac.xform(self.LeftFoot.Bind_Joint, query=True, worldSpace=True, absolute=True,
                                     translation=True)
        RAnklePosStart = mayac.xform(self.RightFoot.Bind_Joint, query=True, worldSpace=True, absolute=True,
                                     translation=True)

        if self.rigType == "World":
            value = -1
            while not DJB_CheckAngle(self.LeftUpLeg.Bind_Joint, self.LeftLeg.Bind_Joint, self.LeftFoot.Bind_Joint,
                                     axis="x", multiplier=-1):
            ...
            value = -1
            while not DJB_CheckAngle(self.RightUpLeg.Bind_Joint, self.RightLeg.Bind_Joint, self.RightFoot.Bind_Joint,
                                     axis="x", multiplier=-1):
            ...
            value = -1
            while not DJB_CheckAngle(self.LeftArm.Bind_Joint, self.LeftForeArm.Bind_Joint, self.LeftHand.Bind_Joint,
                                     axis="y", multiplier=1):
            ...
            mayac.setAttr("%s.rotate" % (self.LeftArm.Bind_Joint), tempRotData[0][0], tempRotData[0][1],
                          tempRotData[0][2], type="double3")
            mayac.rotate(0, 90, 0, self.LeftForeArm.Bind_Joint, relative=True)

            value = 1
            while not DJB_CheckAngle(self.RightArm.Bind_Joint, self.RightForeArm.Bind_Joint, self.RightHand.Bind_Joint,
                                     axis="y", multiplier=-1):
            ...
            mayac.setAttr("%s.rotate" % (self.RightArm.Bind_Joint), tempRotData[0][0], tempRotData[0][1],
                          tempRotData[0][2], type="double3")
            mayac.rotate(0, -90, 0, self.RightForeArm.Bind_Joint, relative=True)
        ...
        LAnklePosEnd = mayac.xform(self.LeftFoot.Bind_Joint, query=True, worldSpace=True, absolute=True,
                                   translation=True)
        RAnklePosEnd = mayac.xform(self.RightFoot.Bind_Joint, query=True, worldSpace=True, absolute=True,
                                   translation=True)
    ...
    def makeControls(self, estimateSize=True):
        ...
        mayac.move(self.proportions["widthMidpoint"], self.proportions["lowPoint"], self.proportions["depthMidpoint"],
        ...
        if self.rigType == "AutoRig":
            ...
            if self.Root.Bind_Joint:
                self.Root.createControl(type="normal",
                                        style="circle",
                                        scale=(self.proportions["depth"] * 0.8, self.proportions["depth"] * 0.8,
                                               self.proportions["depth"] * 0.8),
                                        offset=(0, 0, 0),
                ...
                self.Hips.createControl(type="normal",
                                        style="hula",
                                        scale=(self.proportions["depth"] * 0.75, self.proportions["depth"] * 0.75,
                                               self.proportions["depth"] * 0.75),
            ...
            self.Spine.createControl(type="normal",
                                     style="circle",
                                     scale=(self.proportions["depth"] * 0.7, self.proportions["depth"] * 0.7,
                                            self.proportions["depth"] * 0.7),
            ...
            if self.Spine2.Bind_Joint:
                self.Spine1.createControl(type="normal",
                                          style="circle",
                                          scale=(self.proportions["depth"] * 0.6, self.proportions["depth"] * 0.6,
                                                 self.proportions["depth"] * 0.6),
                ...
                if self.Spine3.Bind_Joint:
                    ...
                    self.Spine3.createControl(type="normal",
                                              ...
                                              scale=(self.proportions["depth"] * 0.7, self.proportions["depth"] * 0.7,
                                                     (self.proportions["depth"]) * 0.8),
                                              offset=(
                                              0, self.proportions["depth"] * .2, self.proportions["depth"] * 0.1),
                                              estimateSize=estimateSize,
                                              color_="yellow")
                else:
                    self.Spine2.createControl(type="normal",
            ...
            self.Neck.createControl(type="normal",
                                    style="pin1",
                                    scale=(self.proportions["depth"] * -0.18, self.proportions["depth"] * 0.18,
                                           self.proportions["depth"] * 0.18),
            ...
            if self.Neck1.Bind_Joint:
                self.Neck1.createControl(type="normal",
                                         ...
                                         scale=(self.proportions["depth"] * -0.18, self.proportions["depth"] * 0.18,
                                                self.proportions["depth"] * 0.18),
                                         offset=(
                                         self.proportions["height"] * 0.033, 0, self.proportions["height"] * -0.04),
                                         estimateSize=estimateSize,
                                         color_="yellow")
            ...
            self.Head.createControl(type="normal",
                                    style="box",
                                    scale=(self.proportions["depth"] * 0.4, self.proportions["height"] * 0.13,
                                           (self.proportions["depth"]) * 0.5),
            ...
            self.LeftShoulder.createControl(type="normal",
                                            style="circleWrapped",
                                            scale=(self.proportions["depth"] * 0.4, self.proportions["depth"] * 0.15,
                                                   self.proportions["depth"] * 0.15),
                                            offset=(
                                            0, self.proportions["depth"] * 0.3, self.proportions["height"] * -0.04),
                                            estimateSize=estimateSize,
                                            color_="blue1")
            ...
            self.LeftArm.createControl(type="FK",
                                       style="circle",
                                       scale=(self.proportions["depth"] * 0.25, self.proportions["depth"] * 0.25,
                                              self.proportions["depth"] * 0.25),
                                       offset=(0, 0, 0),
                                       estimateSize=estimateSize,
                                       color_="blue1")
            ...
            self.LeftForeArm.createControl(type="IK",
                                           style="PoleVector",
                                           scale=(self.proportions["depth"] * 0.1, self.proportions["depth"] * 0.2,
                                                  self.proportions["depth"] * 0.1),
                                           offset=(0, 0, 0),
                                           rotate=(0, 90, 0),
                                           estimateSize=estimateSize,
                                           color_="blue2")
            ...
            self.LeftHand.createControl(type="FK",
                                        style="circle",
                                        scale=(self.proportions["depth"] * 0.2, self.proportions["depth"] * 0.2,
                                               self.proportions["depth"] * 0.2),
                                        offset=(0, 0, 0),
                                        estimateSize=estimateSize,
                                        color_="blue1")
            ...
            self.LeftHand.createControl(type="IK",
                                        style="box",
                                        scale=(self.proportions["depth"] * 0.2, self.proportions["depth"] * 0.3,
                                               self.proportions["depth"] * 0.2),
            ...
            self.LeftUpLeg.createControl(type="FK",
                                         style="pin",
                                         scale=(self.proportions["depth"] * 0.1, self.proportions["depth"] * 0.1,
                                                self.proportions["depth"] * 0.1),
                                         offset=(0, 0, 0),
                                         estimateSize=estimateSize,
                                         color_="blue1")
            ...
            self.LeftLeg.createControl(type="FK",
                                       style="pin",
                                       scale=(self.proportions["depth"] * 0.09, self.proportions["depth"] * 0.09,
                                              self.proportions["depth"] * 0.09),
                                       offset=(0, 0, 0),
                                       estimateSize=estimateSize,
                                       color_="blue1")
            ...
            self.LeftFoot.createControl(type="FK",
                                        style="pin",
                                        scale=(self.proportions["depth"] * 0.08, self.proportions["depth"] * 0.08,
                                               self.proportions["depth"] * 0.08),
                                        offset=(0, 0, 0),
                                        estimateSize=estimateSize,
                                        color_="blue1")
            ...
            self.LeftFoot.createControl(type="IK",
                                        style="footBox",
                                        scale=(self.proportions["depth"] * 0.4, self.proportions["depth"] * 0.7,
                                               self.proportions["depth"] * -0.4),
                                        offset=(0, self.proportions["depth"] * 0.1, self.proportions["depth"] * 0.1),
                                        rotate=(90, 0, 0),
                                        partialConstraint=1,
                                        estimateSize=estimateSize,
                                        color_="blue2")
            mayac.move(self.proportions["lowPoint"], "%s.scalePivot" % (self.LeftFoot.IK_CTRL),
                       "%s.rotatePivot" % (self.LeftFoot.IK_CTRL), y=True)

            # LeftToeBase
            self.LeftToeBase.createControl(type="FK",
                                           style="pin",
                                           scale=(self.proportions["depth"] * 0.07, self.proportions["depth"] * 0.07,
                                                  self.proportions["depth"] * 0.07),
                                           offset=(0, 0, 0),
                                           estimateSize=estimateSize,
                                           color_="blue1")
            ...
            self.RightFoot.createControl(type="IK",
                                         ...
                                         estimateSize=estimateSize,
                                         color_="red2")
            mayac.move(self.proportions["lowPoint"], "%s.scalePivot" % (self.RightFoot.IK_CTRL),
                       "%s.rotatePivot" % (self.RightFoot.IK_CTRL), y=True)
            ...
            if self.LeftHandThumb1.Bind_Joint:
                self.LeftHandThumb1.createControl(type="normal",
                                                  style="pin1",
                                                  scale=(
                                                  self.proportions["depth"] * 0.02, self.proportions["depth"] * 0.02,
                                                  self.proportions["depth"] * 0.02),
                                                  offset=(0, 0, 0),
                                                  estimateSize=estimateSize,
                ...
                self.LeftHandThumb2.createControl(type="normal",
                                                  style="pin1",
                                                  scale=(
                                                  self.proportions["depth"] * 0.016, self.proportions["depth"] * 0.016,
                                                  self.proportions["depth"] * 0.016),
                                                  offset=(0, 0, 0),
                                                  estimateSize=estimateSize,
                                                  flipFingers=self.fingerFlip,
                                                  color_="blue2")
                self.LeftHandThumb3.createControl(type="normal",
                                                  style="pin1",
                                                  scale=(
                                                  self.proportions["depth"] * 0.012, self.proportions["depth"] * 0.012,
                                                  self.proportions["depth"] * 0.012),
                                                  offset=(0, 0, 0),
                                                  estimateSize=estimateSize,
                                                  flipFingers=self.fingerFlip,
                                                  color_="blue2")
            ...
            if self.RightHandMiddle1.Bind_Joint:
                ...
                self.RightHandMiddle2.createControl(type="normal",
                                                    style="pin1",
                                                    scale=(self.proportions["depth"] * 0.016,
                                                           self.proportions["depth"] * 0.016,
                                                           self.proportions["depth"] * 0.016),
                                                    offset=(0, 0, 0),
                                                    estimateSize=estimateSize,
                ...
                self.RightHandMiddle3.createControl(type="normal",
                                                    style="pin1",
                                                    scale=(self.proportions["depth"] * 0.012,
                                                           self.proportions["depth"] * 0.012,
                                                           self.proportions["depth"] * 0.012),
                                                    offset=(0, 0, 0),
                                                    estimateSize=estimateSize,
            ...
            self.LeftFoot.createControl(type="options",
                                        style="options",
                                        scale=(self.proportions["depth"] * 0.12, self.proportions["depth"] * 0.12,
                                               self.proportions["depth"] * -0.12),
            ...
            self.LeftShoulder.createControl(type="normal",
                                            ...
                                            scale=(self.proportions["depth"] * 0.4, self.proportions["depth"] * 0.15,
                                                   self.proportions["depth"] * 0.15),
                                            offset=(
                                            self.proportions["height"] * 0.04, self.proportions["depth"] * 0.3, 0),
                                            rotate=(0, -90, 90),
                                            estimateSize=estimateSize,
                                            color_="blue1")
    ...
    def hookUpControls(self):
        ...
        parentPOS = mayac.xform(self.LeftForeArm.parent.Bind_Joint, query=True, absolute=True, worldSpace=True,
                                translation=True)
        tempDistance = math.sqrt(
            (selfPOS[0] - parentPOS[0]) * (selfPOS[0] - parentPOS[0]) + (selfPOS[1] - parentPOS[1]) * (
            selfPOS[1] - parentPOS[1]) + (selfPOS[2] - parentPOS[2]) * (selfPOS[2] - parentPOS[2]))
        ...
        selfPOS = mayac.xform(self.RightForeArm.Bind_Joint, query=True, absolute=True, worldSpace=True,
                              translation=True)
        parentPOS = mayac.xform(self.RightForeArm.parent.Bind_Joint, query=True, absolute=True, worldSpace=True,
                                translation=True)
        tempDistance = math.sqrt(
            (selfPOS[0] - parentPOS[0]) * (selfPOS[0] - parentPOS[0]) + (selfPOS[1] - parentPOS[1]) * (
            selfPOS[1] - parentPOS[1]) + (selfPOS[2] - parentPOS[2]) * (selfPOS[2] - parentPOS[2]))
        ...
        mayac.parentConstraint(self.LeftHand.Bind_Joint, self.LeftHand_CTRLs_GRP,
                               name="%s_Constraint" % (self.LeftHand_CTRLs_GRP))
        mayac.parentConstraint(self.RightHand.Bind_Joint, self.RightHand_CTRLs_GRP,
                               name="%s_Constraint" % (self.RightHand_CTRLs_GRP))
        DJB_LockNHide(self.LeftHand_CTRLs_GRP)
        DJB_LockNHide(self.RightHand_CTRLs_GRP)

        mayac.parent(self.LeftFoot.Options_CTRL, self.RightFoot.Options_CTRL, self.LeftHand.Options_CTRL,
                     self.RightHand.Options_CTRL, self.global_CTRL)
        if self.hulaOption:
        ...
        # IKFK follow body
        # arms
        temp = mayac.parentConstraint(self.LeftShoulder.IK_Dummy_Joint, self.LeftHand.IK_CTRL_grandparent_POS_GRP,
                                      maintainOffset=True)
        self.LeftHand_grandparent_Constraint = temp[0]
        mayac.parentConstraint(self.LeftShoulder.Bind_Joint, self.LeftHand.IK_CTRL_grandparent_POS_GRP,
                               maintainOffset=True)
        self.LeftHand_grandparent_Constraint_Reverse = mayac.createNode('reverse',
                                                                        n="LeftHand_grandparent_Constraint_Reverse")
        mayac.connectAttr("%s.FollowBody" % (self.LeftHand.IK_CTRL),
                          "%s.inputX" % (self.LeftHand_grandparent_Constraint_Reverse))
        mayac.connectAttr("%s.FollowBody" % (self.LeftHand.IK_CTRL),
                          "%s.%sW1" % (self.LeftHand_grandparent_Constraint, self.LeftShoulder.Bind_Joint))
        mayac.connectAttr("%s.outputX" % (self.LeftHand_grandparent_Constraint_Reverse),
                          "%s.%sW0" % (self.LeftHand_grandparent_Constraint, self.LeftShoulder.IK_Dummy_Joint))
        mayac.setAttr("%s.interpType" % (self.LeftHand_grandparent_Constraint), 2)

        temp = mayac.parentConstraint(self.RightShoulder.IK_Dummy_Joint, self.RightHand.IK_CTRL_grandparent_POS_GRP,
                                      maintainOffset=True)
        self.RightHand_grandparent_Constraint = temp[0]
        mayac.parentConstraint(self.RightShoulder.Bind_Joint, self.RightHand.IK_CTRL_grandparent_POS_GRP,
                               maintainOffset=True)
        self.RightHand_grandparent_Constraint_Reverse = mayac.createNode('reverse',
                                                                         n="RightHand_grandparent_Constraint_Reverse")
        mayac.connectAttr("%s.FollowBody" % (self.RightHand.IK_CTRL),
                          "%s.inputX" % (self.RightHand_grandparent_Constraint_Reverse))
        mayac.connectAttr("%s.FollowBody" % (self.RightHand.IK_CTRL),
                          "%s.%sW1" % (self.RightHand_grandparent_Constraint, self.RightShoulder.Bind_Joint))
        mayac.connectAttr("%s.outputX" % (self.RightHand_grandparent_Constraint_Reverse),
                          "%s.%sW0" % (self.RightHand_grandparent_Constraint, self.RightShoulder.IK_Dummy_Joint))
        mayac.setAttr("%s.interpType" % (self.RightHand_grandparent_Constraint), 2)

        temp = mayac.parentConstraint(self.LeftShoulder.IK_Dummy_Joint, self.LeftForeArm.IK_CTRL_parent_POS_GRP,
                                      maintainOffset=True)
        self.LeftForeArm_parent_Constraint = temp[0]
        mayac.parentConstraint(self.LeftShoulder.Bind_Joint, self.LeftForeArm.IK_CTRL_parent_POS_GRP,
                               maintainOffset=True)
        self.LeftForeArm_parent_Constraint_Reverse = mayac.createNode('reverse',
                                                                      n="LeftForeArm_parent_Constraint_Reverse")
        mayac.connectAttr("%s.FollowBody" % (self.LeftForeArm.IK_CTRL),
                          "%s.inputX" % (self.LeftForeArm_parent_Constraint_Reverse))
        mayac.connectAttr("%s.FollowBody" % (self.LeftForeArm.IK_CTRL),
                          "%s.%sW1" % (self.LeftForeArm_parent_Constraint, self.LeftShoulder.Bind_Joint))
        mayac.connectAttr("%s.outputX" % (self.LeftForeArm_parent_Constraint_Reverse),
                          "%s.%sW0" % (self.LeftForeArm_parent_Constraint, self.LeftShoulder.IK_Dummy_Joint))
        mayac.setAttr("%s.interpType" % (self.LeftForeArm_parent_Constraint), 2)

        temp = mayac.parentConstraint(self.RightShoulder.IK_Dummy_Joint, self.RightForeArm.IK_CTRL_parent_POS_GRP,
                                      maintainOffset=True)
        self.RightForeArm_parent_Constraint = temp[0]
        mayac.parentConstraint(self.RightShoulder.Bind_Joint, self.RightForeArm.IK_CTRL_parent_POS_GRP,
                               maintainOffset=True)
        self.RightForeArm_parent_Constraint_Reverse = mayac.createNode('reverse',
                                                                       n="RightForeArm_parent_Constraint_Reverse")
        mayac.connectAttr("%s.FollowBody" % (self.RightForeArm.IK_CTRL),
                          "%s.inputX" % (self.RightForeArm_parent_Constraint_Reverse))
        mayac.connectAttr("%s.FollowBody" % (self.RightForeArm.IK_CTRL),
                          "%s.%sW1" % (self.RightForeArm_parent_Constraint, self.RightShoulder.Bind_Joint))
        mayac.connectAttr("%s.outputX" % (self.RightForeArm_parent_Constraint_Reverse),
                          "%s.%sW0" % (self.RightForeArm_parent_Constraint, self.RightShoulder.IK_Dummy_Joint))
        mayac.setAttr("%s.interpType" % (self.RightForeArm_parent_Constraint), 2)

        # legs
        temp = mayac.parentConstraint(self.Hips.IK_Dummy_Joint, self.LeftFoot.IK_CTRL_grandparent_POS_GRP,
                                      maintainOffset=True)
        ...
        self.LeftFoot_grandparent_Constraint_Reverse = mayac.createNode('reverse',
                                                                        n="LeftFoot_grandparent_Constraint_Reverse")
        mayac.connectAttr("%s.FollowBody" % (self.LeftFoot.IK_CTRL),
                          "%s.inputX" % (self.LeftFoot_grandparent_Constraint_Reverse))
        mayac.connectAttr("%s.FollowBody" % (self.LeftFoot.IK_CTRL),
                          "%s.%sW1" % (self.LeftFoot_grandparent_Constraint, self.Hips.Bind_Joint))
        mayac.connectAttr("%s.outputX" % (self.LeftFoot_grandparent_Constraint_Reverse),
                          "%s.%sW0" % (self.LeftFoot_grandparent_Constraint, self.Hips.IK_Dummy_Joint))
        mayac.setAttr("%s.interpType" % (self.LeftFoot_grandparent_Constraint), 2)

        temp = mayac.parentConstraint(self.Hips.IK_Dummy_Joint, self.RightFoot.IK_CTRL_grandparent_POS_GRP,
                                      maintainOffset=True)
        ...
        self.RightFoot_grandparent_Constraint_Reverse = mayac.createNode('reverse',
                                                                         n="RightFoot_grandparent_Constraint_Reverse")
        mayac.connectAttr("%s.FollowBody" % (self.RightFoot.IK_CTRL),
                          "%s.inputX" % (self.RightFoot_grandparent_Constraint_Reverse))
        mayac.connectAttr("%s.FollowBody" % (self.RightFoot.IK_CTRL),
                          "%s.%sW1" % (self.RightFoot_grandparent_Constraint, self.Hips.Bind_Joint))
        mayac.connectAttr("%s.outputX" % (self.RightFoot_grandparent_Constraint_Reverse),
                          "%s.%sW0" % (self.RightFoot_grandparent_Constraint, self.Hips.IK_Dummy_Joint))
        mayac.setAttr("%s.interpType" % (self.RightFoot_grandparent_Constraint), 2)

        temp = mayac.parentConstraint(self.Hips.IK_Dummy_Joint, self.LeftLeg.IK_CTRL_parent_POS_GRP,
                                      maintainOffset=True)
        ...
        mayac.connectAttr("%s.FollowBody" % (self.LeftLeg.IK_CTRL),
                          "%s.inputX" % (self.LeftLeg_parent_Constraint_Reverse))
        mayac.connectAttr("%s.FollowBody" % (self.LeftLeg.IK_CTRL),
                          "%s.%sW1" % (self.LeftLeg_parent_Constraint, self.Hips.Bind_Joint))
        mayac.connectAttr("%s.outputX" % (self.LeftLeg_parent_Constraint_Reverse),
                          "%s.%sW0" % (self.LeftLeg_parent_Constraint, self.Hips.IK_Dummy_Joint))
        mayac.setAttr("%s.interpType" % (self.LeftLeg_parent_Constraint), 2)

        temp = mayac.parentConstraint(self.Hips.IK_Dummy_Joint, self.RightLeg.IK_CTRL_parent_POS_GRP,
                                      maintainOffset=True)
        ...
        mayac.connectAttr("%s.FollowBody" % (self.RightLeg.IK_CTRL),
                          "%s.inputX" % (self.RightLeg_parent_Constraint_Reverse))
        mayac.connectAttr("%s.FollowBody" % (self.RightLeg.IK_CTRL),
                          "%s.%sW1" % (self.RightLeg_parent_Constraint, self.Hips.Bind_Joint))
        mayac.connectAttr("%s.outputX" % (self.RightLeg_parent_Constraint_Reverse),
                          "%s.%sW0" % (self.RightLeg_parent_Constraint, self.Hips.IK_Dummy_Joint))
        mayac.setAttr("%s.interpType" % (self.RightLeg_parent_Constraint), 2)

        # IK Legs and Arms to Global
        temp = mayac.parentConstraint(self.LeftFoot.IK_CTRL_grandparent_POS_GRP,
                                      self.LeftFoot.IK_CTRL_grandparent_Global_POS_GRP, maintainOffset=True)
        ...
        self.LeftFoot.grandparent_Global_Constraint_Reverse = mayac.createNode('reverse',
                                                                               n="LeftFoot_grandparent_Global_Constraint_Reverse")
        mayac.connectAttr("%s.ParentToGlobal" % (self.LeftFoot.IK_CTRL),
                          "%s.inputX" % (self.LeftFoot.grandparent_Global_Constraint_Reverse))
        mayac.connectAttr("%s.ParentToGlobal" % (self.LeftFoot.IK_CTRL),
                          "%s.%sW1" % (self.LeftFoot.grandparent_Global_Constraint, self.global_CTRL))
        mayac.connectAttr("%s.outputX" % (self.LeftFoot.grandparent_Global_Constraint_Reverse), "%s.%sW0" % (
        self.LeftFoot.grandparent_Global_Constraint, self.LeftFoot.IK_CTRL_grandparent_POS_GRP))
        mayac.setAttr("%s.interpType" % (self.LeftFoot.grandparent_Global_Constraint), 0)

        temp = mayac.parentConstraint(self.RightFoot.IK_CTRL_grandparent_POS_GRP,
                                      self.RightFoot.IK_CTRL_grandparent_Global_POS_GRP, maintainOffset=True)
        ...
        self.RightFoot.grandparent_Global_Constraint_Reverse = mayac.createNode('reverse',
                                                                                n="RightFoot_grandparent_Global_Constraint_Reverse")
        mayac.connectAttr("%s.ParentToGlobal" % (self.RightFoot.IK_CTRL),
                          "%s.inputX" % (self.RightFoot.grandparent_Global_Constraint_Reverse))
        mayac.connectAttr("%s.ParentToGlobal" % (self.RightFoot.IK_CTRL),
                          "%s.%sW1" % (self.RightFoot.grandparent_Global_Constraint, self.global_CTRL))
        mayac.connectAttr("%s.outputX" % (self.RightFoot.grandparent_Global_Constraint_Reverse), "%s.%sW0" % (
        self.RightFoot.grandparent_Global_Constraint, self.RightFoot.IK_CTRL_grandparent_POS_GRP))
        mayac.setAttr("%s.interpType" % (self.RightFoot.grandparent_Global_Constraint), 2)

        temp = mayac.parentConstraint(self.LeftHand.IK_CTRL_grandparent_POS_GRP,
                                      self.LeftHand.IK_CTRL_grandparent_Global_POS_GRP, maintainOffset=True)
        ...
        self.LeftHand.grandparent_Global_Constraint_Reverse = mayac.createNode('reverse',
                                                                               n="LeftHand_grandparent_Global_Constraint_Reverse")
        mayac.connectAttr("%s.ParentToGlobal" % (self.LeftHand.IK_CTRL),
                          "%s.inputX" % (self.LeftHand.grandparent_Global_Constraint_Reverse))
        mayac.connectAttr("%s.ParentToGlobal" % (self.LeftHand.IK_CTRL),
                          "%s.%sW1" % (self.LeftHand.grandparent_Global_Constraint, self.global_CTRL))
        mayac.connectAttr("%s.outputX" % (self.LeftHand.grandparent_Global_Constraint_Reverse), "%s.%sW0" % (
        self.LeftHand.grandparent_Global_Constraint, self.LeftHand.IK_CTRL_grandparent_POS_GRP))
        mayac.setAttr("%s.interpType" % (self.LeftHand.grandparent_Global_Constraint), 2)

        temp = mayac.parentConstraint(self.RightHand.IK_CTRL_grandparent_POS_GRP,
                                      self.RightHand.IK_CTRL_grandparent_Global_POS_GRP, maintainOffset=True)
        ...
        self.RightHand.grandparent_Global_Constraint_Reverse = mayac.createNode('reverse',
                                                                                n="RightHand_grandparent_Global_Constraint_Reverse")
        mayac.connectAttr("%s.ParentToGlobal" % (self.RightHand.IK_CTRL),
                          "%s.inputX" % (self.RightHand.grandparent_Global_Constraint_Reverse))
        mayac.connectAttr("%s.ParentToGlobal" % (self.RightHand.IK_CTRL),
                          "%s.%sW1" % (self.RightHand.grandparent_Global_Constraint, self.global_CTRL))
        mayac.connectAttr("%s.outputX" % (self.RightHand.grandparent_Global_Constraint_Reverse), "%s.%sW0" % (
        self.RightHand.grandparent_Global_Constraint, self.RightHand.IK_CTRL_grandparent_POS_GRP))
        ...
        temp = mayac.parentConstraint(self.LeftLeg.IK_CTRL_parent_POS_GRP, self.LeftLeg.IK_CTRL_parent_Global_POS_GRP,
                                      maintainOffset=True)
        ...
        self.LeftLeg.parent_Global_Constraint_Reverse = mayac.createNode('reverse',
                                                                         n="LeftLeg_parent_Global_Constraint_Reverse")
        mayac.connectAttr("%s.ParentToGlobal" % (self.LeftLeg.IK_CTRL),
                          "%s.inputX" % (self.LeftLeg.parent_Global_Constraint_Reverse))
        mayac.connectAttr("%s.ParentToGlobal" % (self.LeftLeg.IK_CTRL),
                          "%s.%sW1" % (self.LeftLeg.parent_Global_Constraint, self.global_CTRL))
        mayac.connectAttr("%s.outputX" % (self.LeftLeg.parent_Global_Constraint_Reverse),
                          "%s.%sW0" % (self.LeftLeg.parent_Global_Constraint, self.LeftLeg.IK_CTRL_parent_POS_GRP))
        mayac.setAttr("%s.interpType" % (self.LeftLeg.parent_Global_Constraint), 2)

        temp = mayac.parentConstraint(self.RightLeg.IK_CTRL_parent_POS_GRP, self.RightLeg.IK_CTRL_parent_Global_POS_GRP,
                                      maintainOffset=True)
        ...
        self.RightLeg.parent_Global_Constraint_Reverse = mayac.createNode('reverse',
                                                                          n="RightLeg_parent_Global_Constraint_Reverse")
        mayac.connectAttr("%s.ParentToGlobal" % (self.RightLeg.IK_CTRL),
                          "%s.inputX" % (self.RightLeg.parent_Global_Constraint_Reverse))
        mayac.connectAttr("%s.ParentToGlobal" % (self.RightLeg.IK_CTRL),
                          "%s.%sW1" % (self.RightLeg.parent_Global_Constraint, self.global_CTRL))
        mayac.connectAttr("%s.outputX" % (self.RightLeg.parent_Global_Constraint_Reverse),
                          "%s.%sW0" % (self.RightLeg.parent_Global_Constraint, self.RightLeg.IK_CTRL_parent_POS_GRP))
        mayac.setAttr("%s.interpType" % (self.RightLeg.parent_Global_Constraint), 2)

        temp = mayac.parentConstraint(self.LeftForeArm.IK_CTRL_parent_POS_GRP,
                                      self.LeftForeArm.IK_CTRL_parent_Global_POS_GRP, maintainOffset=True)
        ...
        self.LeftForeArm.parent_Global_Constraint_Reverse = mayac.createNode('reverse',
                                                                             n="LeftForeArm_parent_Global_Constraint_Reverse")
        mayac.connectAttr("%s.ParentToGlobal" % (self.LeftForeArm.IK_CTRL),
                          "%s.inputX" % (self.LeftForeArm.parent_Global_Constraint_Reverse))
        mayac.connectAttr("%s.ParentToGlobal" % (self.LeftForeArm.IK_CTRL),
                          "%s.%sW1" % (self.LeftForeArm.parent_Global_Constraint, self.global_CTRL))
        mayac.connectAttr("%s.outputX" % (self.LeftForeArm.parent_Global_Constraint_Reverse), "%s.%sW0" % (
        self.LeftForeArm.parent_Global_Constraint, self.LeftForeArm.IK_CTRL_parent_POS_GRP))
        mayac.setAttr("%s.interpType" % (self.LeftForeArm.parent_Global_Constraint), 2)

        temp = mayac.parentConstraint(self.RightForeArm.IK_CTRL_parent_POS_GRP,
                                      self.RightForeArm.IK_CTRL_parent_Global_POS_GRP, maintainOffset=True)
        ...
        self.RightForeArm.parent_Global_Constraint_Reverse = mayac.createNode('reverse',
                                                                              n="RightForeArm_parent_Global_Constraint_Reverse")
        mayac.connectAttr("%s.ParentToGlobal" % (self.RightForeArm.IK_CTRL),
                          "%s.inputX" % (self.RightForeArm.parent_Global_Constraint_Reverse))
        mayac.connectAttr("%s.ParentToGlobal" % (self.RightForeArm.IK_CTRL),
                          "%s.%sW1" % (self.RightForeArm.parent_Global_Constraint, self.global_CTRL))
        mayac.connectAttr("%s.outputX" % (self.RightForeArm.parent_Global_Constraint_Reverse), "%s.%sW0" % (
        self.RightForeArm.parent_Global_Constraint, self.RightForeArm.IK_CTRL_parent_POS_GRP))
        ...
        temp = mayac.parentConstraint(self.LeftLeg.IK_CTRL_animData_CONST_GRP, self.LeftLeg.IK_CTRL_inRig_CONST_GRP,
        ...
        self.LeftLeg.follow_extremity_Constraint_Reverse = mayac.createNode('reverse',
                                                                            n="LeftLeg_follow_extremity_Constraint_Reverse")
        mayac.connectAttr("%s.FollowFoot" % (self.LeftLeg.IK_CTRL),
                          "%s.inputX" % (self.LeftLeg.follow_extremity_Constraint_Reverse))
        mayac.connectAttr("%s.FollowFoot" % (self.LeftLeg.IK_CTRL),
                          "%s.%sW1" % (self.LeftLeg.follow_extremity_Constraint, self.LeftLeg.Follow_Knee_GRP))
        mayac.connectAttr("%s.outputX" % (self.LeftLeg.follow_extremity_Constraint_Reverse), "%s.%sW0" % (
        self.LeftLeg.follow_extremity_Constraint, self.LeftLeg.IK_CTRL_animData_CONST_GRP))
        ...
        temp = mayac.parentConstraint(self.RightLeg.IK_CTRL_animData_CONST_GRP, self.RightLeg.IK_CTRL_inRig_CONST_GRP,
                                      maintainOffset=False)
        self.RightLeg.follow_extremity_Constraint = temp[0]
        mayac.parentConstraint(self.RightLeg.Follow_Knee_GRP, self.RightLeg.IK_CTRL_inRig_CONST_GRP,
                               maintainOffset=False)
        self.RightLeg.follow_extremity_Constraint_Reverse = mayac.createNode('reverse',
                                                                             n="RightLeg_follow_extremity_Constraint_Reverse")
        mayac.connectAttr("%s.FollowFoot" % (self.RightLeg.IK_CTRL),
                          "%s.inputX" % (self.RightLeg.follow_extremity_Constraint_Reverse))
        mayac.connectAttr("%s.FollowFoot" % (self.RightLeg.IK_CTRL),
                          "%s.%sW1" % (self.RightLeg.follow_extremity_Constraint, self.RightLeg.Follow_Knee_GRP))
        mayac.connectAttr("%s.outputX" % (self.RightLeg.follow_extremity_Constraint_Reverse), "%s.%sW0" % (
        self.RightLeg.follow_extremity_Constraint, self.RightLeg.IK_CTRL_animData_CONST_GRP))
        mayac.setAttr("%s.interpType" % (self.RightLeg.follow_extremity_Constraint), 2)

        temp = mayac.parentConstraint(self.LeftForeArm.IK_CTRL_animData_CONST_GRP,
                                      self.LeftForeArm.IK_CTRL_inRig_CONST_GRP, maintainOffset=True)
        ...
        self.LeftForeArm.follow_extremity_Constraint_Reverse = mayac.createNode('reverse',
                                                                                n="LeftForeArm_follow_extremity_Constraint_Reverse")
        mayac.connectAttr("%s.FollowHand" % (self.LeftForeArm.IK_CTRL),
                          "%s.inputX" % (self.LeftForeArm.follow_extremity_Constraint_Reverse))
        mayac.connectAttr("%s.FollowHand" % (self.LeftForeArm.IK_CTRL),
                          "%s.%sW1" % (self.LeftForeArm.follow_extremity_Constraint, self.LeftHand.IK_CTRL))
        mayac.connectAttr("%s.outputX" % (self.LeftForeArm.follow_extremity_Constraint_Reverse), "%s.%sW0" % (
        self.LeftForeArm.follow_extremity_Constraint, self.LeftForeArm.IK_CTRL_animData_CONST_GRP))
        mayac.setAttr("%s.interpType" % (self.LeftForeArm.follow_extremity_Constraint), 2)

        temp = mayac.parentConstraint(self.RightForeArm.IK_CTRL_animData_CONST_GRP,
                                      self.RightForeArm.IK_CTRL_inRig_CONST_GRP, maintainOffset=True)
        ...
        self.RightForeArm.follow_extremity_Constraint_Reverse = mayac.createNode('reverse',
                                                                                 n="RightForeArm_follow_extremity_Constraint_Reverse")
        mayac.connectAttr("%s.FollowHand" % (self.RightForeArm.IK_CTRL),
                          "%s.inputX" % (self.RightForeArm.follow_extremity_Constraint_Reverse))
        mayac.connectAttr("%s.FollowHand" % (self.RightForeArm.IK_CTRL),
                          "%s.%sW1" % (self.RightForeArm.follow_extremity_Constraint, self.RightHand.IK_CTRL))
        mayac.connectAttr("%s.outputX" % (self.RightForeArm.follow_extremity_Constraint_Reverse), "%s.%sW0" % (
        self.RightForeArm.follow_extremity_Constraint, self.RightForeArm.IK_CTRL_animData_CONST_GRP))
        mayac.setAttr("%s.interpType" % (self.RightForeArm.follow_extremity_Constraint), 2)

        # IK feet
        self.Left_Ankle_IK_CTRL = DJB_createGroup(transform=None, suffix=None, fullName="Left_Ankle_IK_CTRL",
                                                  pivotFrom=self.LeftToeBase.Bind_Joint)
        self.Left_ToeBase_IK_CTRL = DJB_createGroup(transform=None, suffix=None, fullName="Left_ToeBase_IK_CTRL",
                                                    pivotFrom=self.LeftToeBase.Bind_Joint)
        self.Left_ToeBase_IK_AnimData_GRP = DJB_createGroup(transform=self.Left_ToeBase_IK_CTRL, suffix=None,
                                                            fullName="Left_ToeBase_IK_AnimData_GRP")
        self.Left_Ankle_IK_AnimData_GRP = DJB_createGroup(transform=self.Left_Ankle_IK_CTRL, suffix=None,
                                                          fullName="Left_Ankle_IK_AnimData_GRP")
        self.Left_Toe_IK_CTRL = DJB_createGroup(transform=None, suffix=None, fullName="Left_Toe_End_IK_CTRL",
                                                pivotFrom=self.LeftToe_End.Bind_Joint)
        self.Left_Toe_IK_AnimData_GRP = DJB_createGroup(transform=self.Left_Toe_IK_CTRL, suffix=None,
                                                        fullName="Left_Toe_IK_AnimData_GRP")
        # set rotation orders
        ...
        # handle
        temp = mayac.ikHandle(n="Left_ToeBase_IkHandle", sj=self.LeftFoot.IK_Joint, ee=self.LeftToeBase.IK_Joint,
                              solver="ikSCsolver", weight=1)
        ...
        self.Left_IK_ToeBase_animData_MultNode = mayac.createNode('multiplyDivide',
                                                                  n="Left_IK_ToeBase_animData_MultNode")
        mayac.connectAttr("%s.rotateX" % (self.LeftToeBase.AnimData_Joint),
                          "%s.input1X" % (self.Left_IK_ToeBase_animData_MultNode), force=True)
        mayac.connectAttr("%s.AnimDataMult" % (self.LeftFoot.IK_CTRL),
                          "%s.input2X" % (self.Left_IK_ToeBase_animData_MultNode), force=True)
        mayac.connectAttr("%s.rotateY" % (self.LeftToeBase.AnimData_Joint),
                          "%s.input1Y" % (self.Left_IK_ToeBase_animData_MultNode), force=True)
        mayac.connectAttr("%s.AnimDataMult" % (self.LeftFoot.IK_CTRL),
                          "%s.input2Y" % (self.Left_IK_ToeBase_animData_MultNode), force=True)
        mayac.connectAttr("%s.rotateZ" % (self.LeftToeBase.AnimData_Joint),
                          "%s.input1Z" % (self.Left_IK_ToeBase_animData_MultNode), force=True)
        mayac.connectAttr("%s.AnimDataMult" % (self.LeftFoot.IK_CTRL),
                          "%s.input2Z" % (self.Left_IK_ToeBase_animData_MultNode), force=True)
        mayac.connectAttr("%s.outputX" % (self.Left_IK_ToeBase_animData_MultNode),
                          "%s.rotateX" % (self.Left_ToeBase_IK_AnimData_GRP), force=True)
        mayac.connectAttr("%s.outputY" % (self.Left_IK_ToeBase_animData_MultNode),
                          "%s.rotateY" % (self.Left_ToeBase_IK_AnimData_GRP), force=True)
        mayac.connectAttr("%s.outputZ" % (self.Left_IK_ToeBase_animData_MultNode),
                          "%s.rotateZ" % (self.Left_ToeBase_IK_AnimData_GRP), force=True)

        self.Right_Ankle_IK_CTRL = DJB_createGroup(transform=None, suffix=None, fullName="Right_Ankle_IK_CTRL",
                                                   pivotFrom=self.RightToeBase.Bind_Joint)
        self.Right_ToeBase_IK_CTRL = DJB_createGroup(transform=None, suffix=None, fullName="Right_ToeBase_IK_CTRL",
                                                     pivotFrom=self.RightToeBase.Bind_Joint)
        self.Right_ToeBase_IK_AnimData_GRP = DJB_createGroup(transform=self.Right_ToeBase_IK_CTRL, suffix=None,
                                                             fullName="Right_ToeBase_IK_AnimData_GRP")
        self.Right_Ankle_IK_AnimData_GRP = DJB_createGroup(transform=self.Right_Ankle_IK_CTRL, suffix=None,
                                                           fullName="Right_Ankle_IK_AnimData_GRP")
        self.Right_Toe_IK_CTRL = DJB_createGroup(transform=None, suffix=None, fullName="Right_Toe_End_IK_CTRL",
                                                 pivotFrom=self.RightToe_End.Bind_Joint)
        self.Right_Toe_IK_AnimData_GRP = DJB_createGroup(transform=self.Right_Toe_IK_CTRL, suffix=None,
                                                         fullName="Right_Toe_IK_AnimData_GRP")
        # set rotation orders
        ...
        # IK Handle
        temp = mayac.ikHandle(n="Right_ToeBase_IkHandle", sj=self.RightFoot.IK_Joint, ee=self.RightToeBase.IK_Joint,
                              solver="ikSCsolver", weight=1)
        ...
        self.Right_IK_ToeBase_animData_MultNode = mayac.createNode('multiplyDivide',
                                                                   n="Right_IK_ToeBase_animData_MultNode")
        mayac.connectAttr("%s.rotateX" % (self.RightToeBase.AnimData_Joint),
                          "%s.input1X" % (self.Right_IK_ToeBase_animData_MultNode), force=True)
        mayac.connectAttr("%s.AnimDataMult" % (self.RightFoot.IK_CTRL),
                          "%s.input2X" % (self.Right_IK_ToeBase_animData_MultNode), force=True)
        mayac.connectAttr("%s.rotateY" % (self.RightToeBase.AnimData_Joint),
                          "%s.input1Y" % (self.Right_IK_ToeBase_animData_MultNode), force=True)
        mayac.connectAttr("%s.AnimDataMult" % (self.RightFoot.IK_CTRL),
                          "%s.input2Y" % (self.Right_IK_ToeBase_animData_MultNode), force=True)
        mayac.connectAttr("%s.rotateZ" % (self.RightToeBase.AnimData_Joint),
                          "%s.input1Z" % (self.Right_IK_ToeBase_animData_MultNode), force=True)
        mayac.connectAttr("%s.AnimDataMult" % (self.RightFoot.IK_CTRL),
                          "%s.input2Z" % (self.Right_IK_ToeBase_animData_MultNode), force=True)
        mayac.connectAttr("%s.outputX" % (self.Right_IK_ToeBase_animData_MultNode),
                          "%s.rotateX" % (self.Right_ToeBase_IK_AnimData_GRP), force=True)
        mayac.connectAttr("%s.outputY" % (self.Right_IK_ToeBase_animData_MultNode),
                          "%s.rotateY" % (self.Right_ToeBase_IK_AnimData_GRP), force=True)
        mayac.connectAttr("%s.outputZ" % (self.Right_IK_ToeBase_animData_MultNode),
                          "%s.rotateZ" % (self.Right_ToeBase_IK_AnimData_GRP), force=True)
        ...
        if self.rigType == "AutoRig":
            ...
            mayac.connectAttr("%s.FootRoll" % (self.LeftFoot.IK_CTRL), "%s.input1X" % (self.LeftFoot_FootRoll_MultNode),
                              force=True)
            mayac.connectAttr("%s.outputX" % (self.LeftFoot_FootRoll_MultNode),
                              "%s.rotateX" % (self.Left_Ankle_IK_CTRL), force=True)

            mayac.connectAttr("%s.ToeTap" % (self.LeftFoot.IK_CTRL), "%s.rotateX" % (self.Left_ToeBase_IK_CTRL),
                              force=True)
            Left_ToeBase_ZAdd = mayac.shadingNode('plusMinusAverage', asUtility=True, n="Left_ToeBase_ZAdd")
            mayac.connectAttr("%s.ToeSideToSide" % (self.LeftFoot.IK_CTRL), "%s.input1D[0]" % (Left_ToeBase_ZAdd),
                              force=True)
            mayac.connectAttr("%s.output1D" % (Left_ToeBase_ZAdd), "%s.rotateZ" % (self.Left_ToeBase_IK_CTRL),
                              force=True)
            mayac.connectAttr("%s.ToeRotate" % (self.LeftFoot.IK_CTRL), "%s.rotateY" % (self.Left_ToeBase_IK_CTRL),
                              force=True)
            ...
            mayac.connectAttr("%s.ToeRoll" % (self.LeftFoot.IK_CTRL), "%s.input1X" % (self.LeftFoot_ToeRoll_MultNode),
                              force=True)
            mayac.connectAttr("%s.outputX" % (self.LeftFoot_ToeRoll_MultNode), "%s.rotateX" % (self.Left_Toe_IK_CTRL),
                              force=True)

            mayac.connectAttr("%s.HipPivot" % (self.LeftFoot.IK_CTRL),
                              "%s.rotateY" % (self.LeftFoot.IK_CTRL_grandparent_inRig_CONST_GRP), force=True)

            mayac.connectAttr("%s.BallPivot" % (self.LeftFoot.IK_CTRL), "%s.input1D[1]" % (Left_ToeBase_ZAdd),
                              force=True)
            mayac.connectAttr("%s.BallPivot" % (self.LeftFoot.IK_CTRL), "%s.rotateZ" % (self.Left_Ankle_IK_CTRL),
                              force=True)

            mayac.connectAttr("%s.ToePivot" % (self.LeftFoot.IK_CTRL), "%s.rotateZ" % (self.Left_Toe_IK_CTRL),
                              force=True)

            mayac.connectAttr("%s.HipSideToSide" % (self.LeftFoot.IK_CTRL),
                              "%s.rotateZ" % (self.LeftFoot.IK_CTRL_grandparent_inRig_CONST_GRP), force=True)

            mayac.connectAttr("%s.HipBackToFront" % (self.LeftFoot.IK_CTRL),
                              "%s.rotateX" % (self.LeftFoot.IK_CTRL_grandparent_inRig_CONST_GRP), force=True)
            ...
            mayac.connectAttr("%s.FootRoll" % (self.RightFoot.IK_CTRL),
                              "%s.input1X" % (self.RightFoot_FootRoll_MultNode), force=True)
            mayac.connectAttr("%s.outputX" % (self.RightFoot_FootRoll_MultNode),
                              "%s.rotateX" % (self.Right_Ankle_IK_CTRL), force=True)

            mayac.connectAttr("%s.ToeTap" % (self.RightFoot.IK_CTRL), "%s.rotateX" % (self.Right_ToeBase_IK_CTRL),
                              force=True)
            Right_ToeBase_ZAdd = mayac.shadingNode('plusMinusAverage', asUtility=True, n="Right_ToeBase_ZAdd")
            mayac.connectAttr("%s.ToeSideToSide" % (self.RightFoot.IK_CTRL), "%s.input1D[0]" % (Right_ToeBase_ZAdd),
                              force=True)
            mayac.connectAttr("%s.output1D" % (Right_ToeBase_ZAdd), "%s.rotateZ" % (self.Right_ToeBase_IK_CTRL),
                              force=True)
            mayac.connectAttr("%s.ToeRotate" % (self.RightFoot.IK_CTRL), "%s.rotateY" % (self.Right_ToeBase_IK_CTRL),
                              force=True)
            ...
            mayac.connectAttr("%s.ToeRoll" % (self.RightFoot.IK_CTRL), "%s.input1X" % (self.RightFoot_ToeRoll_MultNode),
                              force=True)
            mayac.connectAttr("%s.outputX" % (self.RightFoot_ToeRoll_MultNode), "%s.rotateX" % (self.Right_Toe_IK_CTRL),
                              force=True)
            ...
            mayac.connectAttr("%s.HipPivot" % (self.RightFoot.IK_CTRL),
                              "%s.input1X" % (self.RightFoot_HipPivot_MultNode), force=True)
            mayac.connectAttr("%s.outputX" % (self.RightFoot_HipPivot_MultNode),
                              "%s.rotateY" % (self.RightFoot.IK_CTRL_grandparent_inRig_CONST_GRP), force=True)
            ...
            mayac.connectAttr("%s.BallPivot" % (self.RightFoot.IK_CTRL),
                              "%s.input1X" % (self.RightFoot_BallPivot_MultNode), force=True)
            mayac.connectAttr("%s.outputX" % (self.RightFoot_BallPivot_MultNode),
                              "%s.input1D[1]" % (Right_ToeBase_ZAdd), force=True)
            mayac.connectAttr("%s.outputX" % (self.RightFoot_BallPivot_MultNode),
                              "%s.rotateZ" % (self.Right_Ankle_IK_CTRL), force=True)
            ...
            mayac.connectAttr("%s.ToePivot" % (self.RightFoot.IK_CTRL),
                              "%s.input1X" % (self.RightFoot_ToePivot_MultNode), force=True)
            mayac.connectAttr("%s.outputX" % (self.RightFoot_ToePivot_MultNode),
                              "%s.rotateZ" % (self.Right_Toe_IK_CTRL), force=True)

            self.RightFoot_HipSideToSide_MultNode = mayac.createNode('multiplyDivide',
                                                                     n="RightFoot_HipSideToSide_MultNode")
            mayac.setAttr("%s.input2X" % (self.RightFoot_HipSideToSide_MultNode), -1)
            mayac.connectAttr("%s.HipSideToSide" % (self.RightFoot.IK_CTRL),
                              "%s.input1X" % (self.RightFoot_HipSideToSide_MultNode), force=True)
            mayac.connectAttr("%s.outputX" % (self.RightFoot_HipSideToSide_MultNode),
                              "%s.rotateZ" % (self.RightFoot.IK_CTRL_grandparent_inRig_CONST_GRP), force=True)

            mayac.connectAttr("%s.HipBackToFront" % (self.RightFoot.IK_CTRL),
                              "%s.rotateX" % (self.RightFoot.IK_CTRL_grandparent_inRig_CONST_GRP), force=True)
            ...
            mayac.connectAttr("%s.ToeTap" % (self.LeftFoot.IK_CTRL), "%s.input1X" % (LeftFoot_ToeTap_MultNode),
                              force=True)
            mayac.connectAttr("%s.outputX" % (LeftFoot_ToeTap_MultNode), "%s.rotateX" % (self.Left_ToeBase_IK_CTRL),
                              force=True)

            Left_ToeBase_ZAdd = mayac.shadingNode('plusMinusAverage', asUtility=True, n="Left_ToeBase_ZAdd")
            mayac.connectAttr("%s.ToeSideToSide" % (self.LeftFoot.IK_CTRL), "%s.input1D[0]" % (Left_ToeBase_ZAdd),
                              force=True)
            mayac.connectAttr("%s.output1D" % (Left_ToeBase_ZAdd), "%s.rotateY" % (self.Left_ToeBase_IK_CTRL),
                              force=True)
            mayac.connectAttr("%s.ToeRotate" % (self.LeftFoot.IK_CTRL), "%s.rotateZ" % (self.Left_ToeBase_IK_CTRL),
                              force=True)
            ...
            mayac.connectAttr("%s.BallPivot" % (self.LeftFoot.IK_CTRL), "%s.input1D[1]" % (Left_ToeBase_ZAdd),
                              force=True)
            mayac.connectAttr("%s.BallPivot" % (self.LeftFoot.IK_CTRL), "%s.rotateY" % (self.Left_Ankle_IK_CTRL),
                              force=True)

            mayac.connectAttr("%s.ToePivot" % (self.LeftFoot.IK_CTRL), "%s.rotateY" % (self.Left_Toe_IK_CTRL),
                              force=True)
            ...
            mayac.connectAttr("%s.ToeTap" % (self.RightFoot.IK_CTRL), "%s.input1X" % (RightFoot_ToeTap_MultNode),
                              force=True)
            mayac.connectAttr("%s.outputX" % (RightFoot_ToeTap_MultNode), "%s.rotateX" % (self.Right_ToeBase_IK_CTRL),
                              force=True)

            Right_ToeBase_ZAdd = mayac.shadingNode('plusMinusAverage', asUtility=True, n="Right_ToeBase_ZAdd")
            mayac.connectAttr("%s.ToeSideToSide" % (self.RightFoot.IK_CTRL), "%s.input1D[0]" % (Right_ToeBase_ZAdd),
                              force=True)
            mayac.connectAttr("%s.output1D" % (Right_ToeBase_ZAdd), "%s.rotateY" % (self.Right_ToeBase_IK_CTRL),
                              force=True)
            mayac.connectAttr("%s.ToeRotate" % (self.RightFoot.IK_CTRL), "%s.rotateZ" % (self.Right_ToeBase_IK_CTRL),
                              force=True)
            ...
            mayac.connectAttr("%s.outputX" % (self.RightFoot_BallPivot_MultNode),
                              "%s.input1D[1]" % (Right_ToeBase_ZAdd), force=True)
            mayac.connectAttr("%s.outputX" % (self.RightFoot_BallPivot_MultNode),
                              "%s.rotateY" % (self.Right_Ankle_IK_CTRL), force=True)
            ...
            mayac.connectAttr("%s.ToePivot" % (self.RightFoot.IK_CTRL),
                              "%s.input1X" % (self.RightFoot_ToePivot_MultNode), force=True)
            mayac.connectAttr("%s.outputX" % (self.RightFoot_ToePivot_MultNode),
                              "%s.rotateY" % (self.Right_Toe_IK_CTRL), force=True)
        ...
        if self.rigType == "AutoRig":
            if self.LeftHandIndex1.Bind_Joint:
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandIndex1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandIndex1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.LeftHand.Options_CTRL), driverValue=10.0,
                                        value=90.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandIndex1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandIndex2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandIndex2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.LeftHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandIndex2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandIndex3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandIndex3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.LeftHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandIndex3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandIndex1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandIndex1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.LeftHand.Options_CTRL), driverValue=10.0,
                                        value=45.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandIndex1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=-45.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandIndex1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Spread" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandIndex1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Spread" % (self.LeftHand.Options_CTRL), driverValue=10.0,
                                        value=-30.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandIndex1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Spread" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=12.0)
            ...
            if self.LeftHandMiddle1.Bind_Joint:
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandMiddle1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandMiddle1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.LeftHand.Options_CTRL), driverValue=10.0,
                                        value=90.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandMiddle1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandMiddle2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandMiddle2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.LeftHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandMiddle2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandMiddle3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandMiddle3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.LeftHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandMiddle3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandMiddle1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandMiddle1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.LeftHand.Options_CTRL), driverValue=10.0,
            ...
            if self.LeftHandRing1.Bind_Joint:
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandRing1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandRing1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.LeftHand.Options_CTRL), driverValue=10.0,
                                        value=90.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandRing1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandRing2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandRing2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.LeftHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandRing2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandRing3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandRing3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.LeftHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandRing3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandRing1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandRing1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.LeftHand.Options_CTRL), driverValue=10.0,
            ...
            if self.LeftHandPinky1.Bind_Joint:
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandPinky1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandPinky1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.LeftHand.Options_CTRL), driverValue=10.0,
                                        value=90.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandPinky1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandPinky2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandPinky2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.LeftHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandPinky2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandPinky3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandPinky3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.LeftHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateX" % (self.LeftHandPinky3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandPinky1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandPinky1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.LeftHand.Options_CTRL), driverValue=10.0,
            ...
            if self.LeftHandThumb1.Bind_Joint:
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandThumb1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandThumb1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.LeftHand.Options_CTRL), driverValue=10.0,
                                        value=25.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandThumb1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=-25.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandThumb2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandThumb2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.LeftHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandThumb2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=-60.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandThumb3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandThumb3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.LeftHand.Options_CTRL), driverValue=10.0,
            ...
            if self.RightHandIndex1.Bind_Joint:
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandIndex1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandIndex1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.RightHand.Options_CTRL), driverValue=10.0,
                                        value=90.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandIndex1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.RightHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandIndex2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandIndex2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.RightHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandIndex2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.RightHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandIndex3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandIndex3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.RightHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandIndex3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.RightHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandIndex1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandIndex1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.RightHand.Options_CTRL), driverValue=10.0,
                                        value=-45.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandIndex1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.RightHand.Options_CTRL), driverValue=-10.0,
                                        value=45.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandIndex1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Spread" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandIndex1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Spread" % (self.RightHand.Options_CTRL), driverValue=10.0,
                                        value=30.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandIndex1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Spread" % (self.RightHand.Options_CTRL), driverValue=-10.0,
                                        value=-12.0)
            ...
            if self.RightHandMiddle1.Bind_Joint:
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandMiddle1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandMiddle1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.RightHand.Options_CTRL), driverValue=10.0,
                                        value=90.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandMiddle1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.RightHand.Options_CTRL),
                                        driverValue=-10.0, value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandMiddle2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandMiddle2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.RightHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandMiddle2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.RightHand.Options_CTRL),
                                        driverValue=-10.0, value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandMiddle3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandMiddle3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.RightHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandMiddle3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.RightHand.Options_CTRL),
                                        driverValue=-10.0, value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandMiddle1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandMiddle1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.RightHand.Options_CTRL), driverValue=10.0,
            ...
            if self.RightHandRing1.Bind_Joint:
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandRing1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandRing1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.RightHand.Options_CTRL), driverValue=10.0,
                                        value=90.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandRing1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.RightHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandRing2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandRing2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.RightHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandRing2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.RightHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandRing3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandRing3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.RightHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandRing3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.RightHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandRing1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandRing1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.RightHand.Options_CTRL), driverValue=10.0,
            ...
            if self.RightHandPinky1.Bind_Joint:
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandPinky1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandPinky1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.RightHand.Options_CTRL), driverValue=10.0,
                                        value=90.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandPinky1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.RightHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandPinky2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandPinky2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.RightHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandPinky2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.RightHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandPinky3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandPinky3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.RightHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateX" % (self.RightHandPinky3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.RightHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandPinky1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandPinky1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.RightHand.Options_CTRL), driverValue=10.0,
            ...
            if self.RightHandThumb1.Bind_Joint:
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandThumb1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandThumb1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.RightHand.Options_CTRL), driverValue=10.0,
                                        value=-25.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandThumb1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.RightHand.Options_CTRL), driverValue=-10.0,
                                        value=25.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandThumb2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandThumb2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.RightHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandThumb2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.RightHand.Options_CTRL), driverValue=-10.0,
                                        value=60.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandThumb3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandThumb3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.RightHand.Options_CTRL), driverValue=10.0,
            ...
            if self.LeftHandIndex1.Bind_Joint:
                ...
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandIndex1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=90.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandIndex2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandIndex2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.LeftHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandIndex2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=90.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandIndex3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandIndex3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.LeftHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandIndex3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=90.0)
                mayac.setDrivenKeyframe("%s.rotateY" % (self.LeftHandIndex1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateY" % (self.LeftHandIndex1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.LeftHand.Options_CTRL), driverValue=10.0,
            ...
            if self.LeftHandMiddle1.Bind_Joint:
                ...
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandMiddle1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=90.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandMiddle2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandMiddle2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.LeftHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandMiddle2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=90.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandMiddle3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandMiddle3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.LeftHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandMiddle3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=90.0)
                mayac.setDrivenKeyframe("%s.rotateY" % (self.LeftHandMiddle1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateY" % (self.LeftHandMiddle1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.LeftHand.Options_CTRL), driverValue=10.0,
            ...
            if self.LeftHandRing1.Bind_Joint:
                ...
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandRing1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=90.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandRing2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandRing2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.LeftHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandRing2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=90.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandRing3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandRing3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.LeftHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandRing3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=90.0)
                mayac.setDrivenKeyframe("%s.rotateY" % (self.LeftHandRing1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateY" % (self.LeftHandRing1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.LeftHand.Options_CTRL), driverValue=10.0,
            ...
            if self.LeftHandPinky1.Bind_Joint:
                ...
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandPinky1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=90.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandPinky2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandPinky2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.LeftHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandPinky2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=90.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandPinky3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandPinky3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.LeftHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.LeftHandPinky3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=90.0)
                mayac.setDrivenKeyframe("%s.rotateY" % (self.LeftHandPinky1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateY" % (self.LeftHandPinky1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.LeftHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.deleteAttr("%s.PinkyCurl" % (self.LeftHand.Options_CTRL))
            if self.LeftHandThumb1.Bind_Joint:
                mayac.setDrivenKeyframe("%s.rotateY" % (self.LeftHandThumb1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateY" % (self.LeftHandThumb1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.LeftHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateY" % (self.LeftHandThumb1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=-25.0)
                mayac.setDrivenKeyframe("%s.rotateY" % (self.LeftHandThumb2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateY" % (self.LeftHandThumb2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.LeftHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateY" % (self.LeftHandThumb2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.LeftHand.Options_CTRL), driverValue=-10.0,
                                        value=-60.0)
                mayac.setDrivenKeyframe("%s.rotateY" % (self.LeftHandThumb3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.LeftHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateY" % (self.LeftHandThumb3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.LeftHand.Options_CTRL), driverValue=10.0,
            ...
            if self.RightHandIndex1.Bind_Joint:
                ...
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandIndex1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.RightHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandIndex2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandIndex2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.RightHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandIndex2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.RightHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandIndex3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandIndex3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.RightHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandIndex3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.IndexCurl" % (self.RightHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateY" % (self.RightHandIndex1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateY" % (self.RightHandIndex1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.RightHand.Options_CTRL), driverValue=10.0,
            ...
            if self.RightHandMiddle1.Bind_Joint:
                ...
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandMiddle1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.RightHand.Options_CTRL),
                                        driverValue=-10.0, value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandMiddle2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandMiddle2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.RightHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandMiddle2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.RightHand.Options_CTRL),
                                        driverValue=-10.0, value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandMiddle3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandMiddle3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.RightHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandMiddle3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.MiddleCurl" % (self.RightHand.Options_CTRL),
                                        driverValue=-10.0, value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateY" % (self.RightHandMiddle1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateY" % (self.RightHandMiddle1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.RightHand.Options_CTRL), driverValue=10.0,
            ...
            if self.RightHandRing1.Bind_Joint:
                ...
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandRing1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.RightHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandRing2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandRing2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.RightHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandRing2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.RightHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandRing3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandRing3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.RightHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandRing3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.RingCurl" % (self.RightHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateY" % (self.RightHandRing1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateY" % (self.RightHandRing1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.RightHand.Options_CTRL), driverValue=10.0,
            ...
            if self.RightHandPinky1.Bind_Joint:
                ...
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandPinky1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.RightHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandPinky2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandPinky2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.RightHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandPinky2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.RightHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandPinky3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandPinky3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.RightHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateZ" % (self.RightHandPinky3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.PinkyCurl" % (self.RightHand.Options_CTRL), driverValue=-10.0,
                                        value=-90.0)
                mayac.setDrivenKeyframe("%s.rotateY" % (self.RightHandPinky1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateY" % (self.RightHandPinky1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.Sway" % (self.RightHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.deleteAttr("%s.PinkyCurl" % (self.RightHand.Options_CTRL))
            if self.RightHandThumb1.Bind_Joint:
                mayac.setDrivenKeyframe("%s.rotateY" % (self.RightHandThumb1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateY" % (self.RightHandThumb1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.RightHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateY" % (self.RightHandThumb1.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.RightHand.Options_CTRL), driverValue=-10.0,
                                        value=25.0)
                mayac.setDrivenKeyframe("%s.rotateY" % (self.RightHandThumb2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateY" % (self.RightHandThumb2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.RightHand.Options_CTRL), driverValue=10.0,
                ...
                mayac.setDrivenKeyframe("%s.rotateY" % (self.RightHandThumb2.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.RightHand.Options_CTRL), driverValue=-10.0,
                                        value=60.0)
                mayac.setDrivenKeyframe("%s.rotateY" % (self.RightHandThumb3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.RightHand.Options_CTRL), driverValue=0.0,
                                        value=0.0)
                mayac.setDrivenKeyframe("%s.rotateY" % (self.RightHandThumb3.FK_CTRL_inRig_CONST_GRP),
                                        currentDriver="%s.ThumbCurl" % (self.RightHand.Options_CTRL), driverValue=10.0,
        ...
        mayac.connectAttr("%s.FK_IK" % (self.LeftHand.Options_CTRL),
                          "%s.%sW1" % (self.LeftArm.Constraint, self.LeftArm.IK_Joint))
        mayac.connectAttr("%s.outputX" % (self.LeftArm_Switch_Reverse),
                          "%s.%sW0" % (self.LeftArm.Constraint, self.LeftArm.FK_Joint))
        mayac.connectAttr("%s.FK_IK" % (self.LeftHand.Options_CTRL),
                          "%s.%sW1" % (self.LeftForeArm.Constraint, self.LeftForeArm.IK_Joint))
        mayac.connectAttr("%s.outputX" % (self.LeftArm_Switch_Reverse),
                          "%s.%sW0" % (self.LeftForeArm.Constraint, self.LeftForeArm.FK_Joint))
        mayac.connectAttr("%s.FK_IK" % (self.LeftHand.Options_CTRL),
                          "%s.%sW1" % (self.LeftHand.Constraint, self.LeftHand.IK_Joint))
        mayac.connectAttr("%s.outputX" % (self.LeftArm_Switch_Reverse),
                          "%s.%sW0" % (self.LeftHand.Constraint, self.LeftHand.FK_Joint))
        mayac.connectAttr("%s.FK_IK" % (self.RightHand.Options_CTRL),
                          "%s.%sW1" % (self.RightArm.Constraint, self.RightArm.IK_Joint))
        mayac.connectAttr("%s.outputX" % (self.RightArm_Switch_Reverse),
                          "%s.%sW0" % (self.RightArm.Constraint, self.RightArm.FK_Joint))
        mayac.connectAttr("%s.FK_IK" % (self.RightHand.Options_CTRL),
                          "%s.%sW1" % (self.RightForeArm.Constraint, self.RightForeArm.IK_Joint))
        mayac.connectAttr("%s.outputX" % (self.RightArm_Switch_Reverse),
                          "%s.%sW0" % (self.RightForeArm.Constraint, self.RightForeArm.FK_Joint))
        mayac.connectAttr("%s.FK_IK" % (self.RightHand.Options_CTRL),
                          "%s.%sW1" % (self.RightHand.Constraint, self.RightHand.IK_Joint))
        mayac.connectAttr("%s.outputX" % (self.RightArm_Switch_Reverse),
                          "%s.%sW0" % (self.RightHand.Constraint, self.RightHand.FK_Joint))

        mayac.connectAttr("%s.FK_IK" % (self.LeftFoot.Options_CTRL),
                          "%s.%sW1" % (self.LeftUpLeg.Constraint, self.LeftUpLeg.IK_Joint))
        mayac.connectAttr("%s.outputX" % (self.LeftLeg_Switch_Reverse),
                          "%s.%sW0" % (self.LeftUpLeg.Constraint, self.LeftUpLeg.FK_Joint))
        mayac.connectAttr("%s.FK_IK" % (self.LeftFoot.Options_CTRL),
                          "%s.%sW1" % (self.LeftLeg.Constraint, self.LeftLeg.IK_Joint))
        mayac.connectAttr("%s.outputX" % (self.LeftLeg_Switch_Reverse),
                          "%s.%sW0" % (self.LeftLeg.Constraint, self.LeftLeg.FK_Joint))
        mayac.connectAttr("%s.FK_IK" % (self.LeftFoot.Options_CTRL),
                          "%s.%sW1" % (self.LeftFoot.Constraint, self.LeftFoot.IK_Joint))
        mayac.connectAttr("%s.outputX" % (self.LeftLeg_Switch_Reverse),
                          "%s.%sW0" % (self.LeftFoot.Constraint, self.LeftFoot.FK_Joint))
        mayac.connectAttr("%s.FK_IK" % (self.LeftFoot.Options_CTRL),
                          "%s.%sW1" % (self.LeftToeBase.Constraint, self.LeftToeBase.IK_Joint))
        mayac.connectAttr("%s.outputX" % (self.LeftLeg_Switch_Reverse),
                          "%s.%sW0" % (self.LeftToeBase.Constraint, self.LeftToeBase.FK_Joint))
        mayac.connectAttr("%s.FK_IK" % (self.RightFoot.Options_CTRL),
                          "%s.%sW1" % (self.RightUpLeg.Constraint, self.RightUpLeg.IK_Joint))
        mayac.connectAttr("%s.outputX" % (self.RightLeg_Switch_Reverse),
                          "%s.%sW0" % (self.RightUpLeg.Constraint, self.RightUpLeg.FK_Joint))
        mayac.connectAttr("%s.FK_IK" % (self.RightFoot.Options_CTRL),
                          "%s.%sW1" % (self.RightLeg.Constraint, self.RightLeg.IK_Joint))
        mayac.connectAttr("%s.outputX" % (self.RightLeg_Switch_Reverse),
                          "%s.%sW0" % (self.RightLeg.Constraint, self.RightLeg.FK_Joint))
        mayac.connectAttr("%s.FK_IK" % (self.RightFoot.Options_CTRL),
                          "%s.%sW1" % (self.RightFoot.Constraint, self.RightFoot.IK_Joint))
        mayac.connectAttr("%s.outputX" % (self.RightLeg_Switch_Reverse),
                          "%s.%sW0" % (self.RightFoot.Constraint, self.RightFoot.FK_Joint))
        mayac.connectAttr("%s.FK_IK" % (self.RightFoot.Options_CTRL),
                          "%s.%sW1" % (self.RightToeBase.Constraint, self.RightToeBase.IK_Joint))
        mayac.connectAttr("%s.outputX" % (self.RightLeg_Switch_Reverse),
                          "%s.%sW0" % (self.RightToeBase.Constraint, self.RightToeBase.FK_Joint))

        # FKIK Visibilities
        DJB_Unlock_Connect_Lock("%s.FK_IK" % (self.LeftHand.Options_CTRL), "%s.visibility" % (self.LeftArm.IK_Joint))
        DJB_Unlock_Connect_Lock("%s.FK_IK" % (self.LeftHand.Options_CTRL),
                                "%s.visibility" % (self.LeftForeArm.IK_CTRL_POS_GRP))
        DJB_Unlock_Connect_Lock("%s.FK_IK" % (self.LeftHand.Options_CTRL),
                                "%s.visibility" % (self.LeftHand.IK_CTRL_POS_GRP))
        DJB_Unlock_Connect_Lock("%s.FK_IK" % (self.RightHand.Options_CTRL), "%s.visibility" % (self.RightArm.IK_Joint))
        DJB_Unlock_Connect_Lock("%s.FK_IK" % (self.RightHand.Options_CTRL),
                                "%s.visibility" % (self.RightForeArm.IK_CTRL_POS_GRP))
        DJB_Unlock_Connect_Lock("%s.FK_IK" % (self.RightHand.Options_CTRL),
                                "%s.visibility" % (self.RightHand.IK_CTRL_POS_GRP))

        DJB_Unlock_Connect_Lock("%s.FK_IK" % (self.LeftFoot.Options_CTRL), "%s.visibility" % (self.LeftUpLeg.IK_Joint))
        DJB_Unlock_Connect_Lock("%s.FK_IK" % (self.LeftFoot.Options_CTRL),
                                "%s.visibility" % (self.LeftLeg.IK_CTRL_POS_GRP))
        DJB_Unlock_Connect_Lock("%s.FK_IK" % (self.LeftFoot.Options_CTRL),
                                "%s.visibility" % (self.LeftFoot.IK_CTRL_POS_GRP))
        DJB_Unlock_Connect_Lock("%s.FK_IK" % (self.RightFoot.Options_CTRL),
                                "%s.visibility" % (self.RightUpLeg.IK_Joint))
        DJB_Unlock_Connect_Lock("%s.FK_IK" % (self.RightFoot.Options_CTRL),
                                "%s.visibility" % (self.RightLeg.IK_CTRL_POS_GRP))
        DJB_Unlock_Connect_Lock("%s.FK_IK" % (self.RightFoot.Options_CTRL),
                                "%s.visibility" % (self.RightFoot.IK_CTRL_POS_GRP))

        DJB_Unlock_Connect_Lock("%s.outputX" % (self.LeftArm_Switch_Reverse), "%s.visibility" % (self.LeftArm.FK_Joint))
        DJB_Unlock_Connect_Lock("%s.outputX" % (self.LeftArm_Switch_Reverse),
                                "%s.visibility" % (self.LeftArm.FK_CTRL_POS_GRP))
        DJB_Unlock_Connect_Lock("%s.outputX" % (self.RightArm_Switch_Reverse),
                                "%s.visibility" % (self.RightArm.FK_Joint))
        DJB_Unlock_Connect_Lock("%s.outputX" % (self.RightArm_Switch_Reverse),
                                "%s.visibility" % (self.RightArm.FK_CTRL_POS_GRP))

        DJB_Unlock_Connect_Lock("%s.outputX" % (self.LeftLeg_Switch_Reverse),
                                "%s.visibility" % (self.LeftUpLeg.FK_Joint))
        DJB_Unlock_Connect_Lock("%s.outputX" % (self.LeftLeg_Switch_Reverse),
                                "%s.visibility" % (self.LeftUpLeg.FK_CTRL_POS_GRP))
        DJB_Unlock_Connect_Lock("%s.outputX" % (self.RightLeg_Switch_Reverse),
                                "%s.visibility" % (self.RightUpLeg.FK_Joint))
        DJB_Unlock_Connect_Lock("%s.outputX" % (self.RightLeg_Switch_Reverse),
                                "%s.visibility" % (self.RightUpLeg.FK_CTRL_POS_GRP))
        ...
        parentPOS = mayac.xform(self.LeftLeg.parent.Bind_Joint, query=True, absolute=True, worldSpace=True,
                                translation=True)
        tempDistance = math.sqrt(
            (selfPOS[0] - parentPOS[0]) * (selfPOS[0] - parentPOS[0]) + (selfPOS[1] - parentPOS[1]) * (
            selfPOS[1] - parentPOS[1]) + (selfPOS[2] - parentPOS[2]) * (selfPOS[2] - parentPOS[2]))
        ...
        for bodyPart in self.bodyParts:
            if bodyPart.children and bodyPart.nodeName != "Root" and bodyPart.Bind_Joint:
                ...
                if not mayac.objExists("%s%s" % (New_joint_Namespace, bodyPart.nodeName)):
                    ...
                    DataSelfPOS = mayac.xform("%s%s" % (New_joint_Namespace, bodyPart.nodeName), query=True,
                                              absolute=True, worldSpace=True, translation=True)
                    for child in bodyPart.children:
                        if child in self.bodyParts:
                            if child.Bind_Joint and "End" not in child.nodeName:
                                childPOS = mayac.xform(child.Bind_Joint, query=True, absolute=True, worldSpace=True,
                                                       translation=True)
                                if not mayac.objExists("%s%s" % (New_joint_Namespace, child.nodeName)):
                                    ...
                                    DataChildPOS = mayac.xform("%s%s" % (New_joint_Namespace, child.nodeName),
                                                               query=True, absolute=True, worldSpace=True,
                                                               translation=True)
                                    correctDistance = math.sqrt(
                                        (selfPOS[0] - childPOS[0]) * (selfPOS[0] - childPOS[0]) + (
                                        selfPOS[1] - childPOS[1]) * (selfPOS[1] - childPOS[1]) + (
                                        selfPOS[2] - childPOS[2]) * (selfPOS[2] - childPOS[2])) / mayac.getAttr(
                                        "%s.scaleX" % (self.global_CTRL))
                                    distanceInQuestion = math.sqrt(
                                        (DataSelfPOS[0] - DataChildPOS[0]) * (DataSelfPOS[0] - DataChildPOS[0]) + (
                                        DataSelfPOS[1] - DataChildPOS[1]) * (DataSelfPOS[1] - DataChildPOS[1]) + (
                                        DataSelfPOS[2] - DataChildPOS[2]) * (DataSelfPOS[2] - DataChildPOS[2]))
                                    if not math.fabs(
                                                    distanceInQuestion / correctDistance) > 1 - proportionCheckTolerance or not math.fabs(
                                                    distanceInQuestion / correctDistance) < 1 + proportionCheckTolerance:
                                        print "Failing because proportions are incorrect for %s%s" % (
                                        New_joint_Namespace, bodyPart.nodeName)
                                        success = False
                                        break
                                if bodyPart.rotOrder != mayac.getAttr(
                                                "%s.rotateOrder" % (New_joint_Namespace + bodyPart.nodeName)):
                                    print "Failing because rotation Orders are incorrect for %s%s" % (
                                    New_joint_Namespace, bodyPart.nodeName)
                                    success = False
                                    break
        return success
    ...
    def connectMotionToAnimDataJoints(self, incomingDataRootJoint):
        ...
        for bodyPart in self.bodyParts:
            if bodyPart.nodeName == "Root" and self.hulaOption:
                ...
                objectsOfInterest.append("%sHips" % (New_joint_Namespace))
                DJB_ConnectAll("%sHips" % (New_joint_Namespace), bodyPart.AnimData_Joint)
            elif bodyPart.nodeName not in ["Hips", "HeadTop_End", "LeftHandThumb4", "LeftHandIndex4", "LeftHandMiddle4",
                                           "LeftHandRing4", "LeftHandPinky4", "LeftToe_End", "RightHandThumb4",
    ...
    def transferMotionToAnimDataJoints(self, incomingDataRootJoint, newStartTime=0, mixMethod="insert",
                                       directConnect_=False):  # mixMethod - insert or merge
        mayac.currentTime(1)
        New_joint_Namespace = DJB_findBeforeSeparator(incomingDataRootJoint, ':')
        ...
        if not directConnect_:
            ...
            for bodyPart in self.bodyParts:
                ...
                if bodyPart.nodeName == "Root" and self.hulaOption:
                    if mayac.objExists("%sRoot" % (New_joint_Namespace)):
                        mayac.copyKey("%sRoot" % (New_joint_Namespace), time=(0, lastFrame), hierarchy="none",
                                      controlPoints=0, shape=1)
                        mayac.pasteKey(bodyPart.AnimData_Joint, option=mixMethod, connect=1, timeOffset=newStartTime,
                                       valueOffset=0)
                    else:
                        mayac.copyKey("%sHips" % (New_joint_Namespace), time=(0, lastFrame), hierarchy="none",
                                      controlPoints=0, shape=1)
                        mayac.pasteKey(bodyPart.AnimData_Joint, option=mixMethod, connect=1, timeOffset=newStartTime,
                                       valueOffset=0)
                elif bodyPart.nodeName == "Hips" and self.hulaOption and mayac.objExists(
                                "%sRoot" % (New_joint_Namespace)):
                    mayac.copyKey("%sHips" % (New_joint_Namespace), time=(0, lastFrame), hierarchy="none",
                                  controlPoints=0, shape=1)
                    mayac.pasteKey(bodyPart.AnimData_Joint, option=mixMethod, connect=1, timeOffset=newStartTime,
                                   valueOffset=0)
                    ...
                    mayac.pasteKey(bodyPart.AnimData_Joint, option=mixMethod, connect=1, timeOffset=newStartTime,
                                   valueOffset=0)
                elif bodyPart.nodeName not in ["Hips", "HeadTop_End", "LeftHandThumb4", "LeftHandIndex4",
                                               "LeftHandMiddle4", "LeftHandRing4", "LeftHandPinky4", "LeftToe_End",
                                               "RightHandThumb4", "RightHandIndex4", "RightHandMiddle4",
                                               "RightHandRing4", "RightHandPinky4", "RightToe_End"]:
                    newAnimDataJoint = "%s%s" % (New_joint_Namespace, bodyPart.nodeName)
                    if mayac.objExists(newAnimDataJoint):
                        numCurves = mayac.copyKey(newAnimDataJoint, time=(0, lastFrame), hierarchy="none",
                                                  controlPoints=0, shape=1)
                        if numCurves:
                            mayac.pasteKey(bodyPart.AnimData_Joint, option=mixMethod, connect=1,
                                           timeOffset=newStartTime, valueOffset=0)
        ...
        if not directConnect_:
            ...
            for bodyPart in self.bodyParts:
                if mayac.objExists("%s%s" % (New_joint_Namespace, bodyPart.nodeName)):
                    mayac.rename("%s%s" % (New_joint_Namespace, bodyPart.nodeName),
                                 "Original_Animation_%s" % (bodyPart.nodeName))
            if self.ExtraJoints:
                for extraJoint in self.ExtraJoints:
                    if mayac.objExists("%s%s" % (New_joint_Namespace, extraJoint.nodeName)):
                        mayac.rename("%s%s" % (New_joint_Namespace, extraJoint.nodeName),
                                     "Original_Animation_%s" % (extraJoint.nodeName))
    ...
    def bakeAnimationToControls(self, bodyPart_="all"):
        ...
        for bodyPart in self.bodyParts:
            if bodyPart.FK_CTRL:
                mayac.filterCurve('%s_rotateX' % (bodyPart.FK_CTRL), '%s_rotateY' % (bodyPart.FK_CTRL),
                                  '%s_rotateZ' % (bodyPart.FK_CTRL))
            if bodyPart.nodeName == "LeftHand" or bodyPart.nodeName == "RightHand" or bodyPart.nodeName == "LeftFoot" or bodyPart.nodeName == "RightFoot":
                mayac.filterCurve('%s_rotateX' % (bodyPart.IK_CTRL), '%s_rotateY' % (bodyPart.IK_CTRL),
                                  '%s_rotateZ' % (bodyPart.IK_CTRL))
        ...
        # IK Toe Tap
        if self.rigType == "AutoRig":
            mayac.copyKey(self.LeftToeBase.FK_CTRL, time=(startTime, endTime), hierarchy="none", controlPoints=0,
                          shape=1, attribute="rotateX")
            mayac.pasteKey(self.LeftFoot.IK_CTRL, connect=1, attribute="ToeTap")
            mayac.copyKey(self.LeftToeBase.FK_CTRL, time=(startTime, endTime), hierarchy="none", controlPoints=0,
                          shape=1, attribute="rotateY")
            mayac.pasteKey(self.LeftFoot.IK_CTRL, connect=1, attribute="ToeRotate")
            mayac.copyKey(self.LeftToeBase.FK_CTRL, time=(startTime, endTime), hierarchy="none", controlPoints=0,
                          shape=1, attribute="rotateZ")
            mayac.pasteKey(self.LeftFoot.IK_CTRL, connect=1, attribute="ToeSideToSide")
            mayac.copyKey(self.RightToeBase.FK_CTRL, time=(startTime, endTime), hierarchy="none", controlPoints=0,
                          shape=1, attribute="rotateX")
    ...
    def clearAnimationControls(self, bodyPart_="all"):
        ...
        for bodyPart in self.bodyParts:
            if bodyPart.AnimData_Joint:
                mayac.filterCurve('%s_rotateX' % (bodyPart.AnimData_Joint), '%s_rotateY' % (bodyPart.AnimData_Joint),
                                  '%s_rotateZ' % (bodyPart.AnimData_Joint))
    ...
    def createExportSkeleton(self, keepMesh_=False, dynamicsToFK=0, reduceNonEssential=False, start=None, end=None,
                             removeEndJoints=False):
        ...
        for joint in (self.exportList + self.exportListDropFrames):
            if "Root" not in joint and "FacialAnim" not in joint and "Hips" not in joint and joint not in translateOpenList:
                DJB_LockNHide(joint, tx=True, ty=True, tz=True, rx=False, ry=False, rz=False, s=True, v=True,
                              other=("jointOrientX", "jointOrientY", "jointOrientZ", "lockInfluenceWeights", "liw"))
            elif joint in translateOpenList:
                DJB_LockNHide(joint, tx=False, ty=False, tz=False, rx=False, ry=False, rz=False, s=True, v=True,
                              other=("jointOrientX", "jointOrientY", "jointOrientZ", "lockInfluenceWeights", "liw"))
            else:
                DJB_LockNHide(joint, tx=False, ty=False, tz=False, rx=False, ry=False, rz=False, s=True, v=True,
                              other=("lockInfluenceWeights", "liw"))
                ...
                if self.ExtraJoints:
                    for node in self.ExtraJoints:
                        if node.Options_CTRL:
                            mayac.setKeyframe(node.Options_CTRL, attribute='FK_Dyn',
                                              t=[lowestTime + dynamicsToFK, highestTime - dynamicsToFK])
        ...
        if anythingToBake and self.exportList:
            mayac.bakeResults(self.exportList + self.exportListDropFrames, simulation=True, time=(startTime, endTime),
                              sampleBy=1.0)
        ...
        if keepMesh_:
            ...
            for i in range(len(self.mesh)):
                print self.mesh[i]
                oldSkin = mayac.listConnections(self.characterNameSpace + self.mesh[i], destination=True,
                                                type="skinCluster")
                if oldSkin:
                    oldSkin = oldSkin[0]
                ...
                if not isBlendShape:
                    ...
                    connections = mayac.listConnections("%s.instObjGroups[0]" % (shapeNode), destination=True,
                                                        plugs=True)
                    if connections:
    ...
    def writeInfoNode(self):
        ...
        pyToAttr("%s.LeftHand_grandparent_Constraint_Reverse" % (self.infoNode),
                 self.LeftHand_grandparent_Constraint_Reverse)
        pyToAttr("%s.RightHand_grandparent_Constraint" % (self.infoNode), self.RightHand_grandparent_Constraint)
        pyToAttr("%s.RightHand_grandparent_Constraint_Reverse" % (self.infoNode),
                 self.RightHand_grandparent_Constraint_Reverse)
        pyToAttr("%s.LeftForeArm_grandparent_Constraint" % (self.infoNode), self.LeftForeArm_grandparent_Constraint)
        pyToAttr("%s.LeftForeArm_grandparent_Constraint_Reverse" % (self.infoNode),
                 self.LeftForeArm_grandparent_Constraint_Reverse)
        pyToAttr("%s.RightForeArm_grandparent_Constraint" % (self.infoNode), self.RightForeArm_grandparent_Constraint)
        pyToAttr("%s.RightForeArm_grandparent_Constraint_Reverse" % (self.infoNode),
                 self.RightForeArm_grandparent_Constraint_Reverse)
        pyToAttr("%s.origAnim" % (self.infoNode), self.origAnim)
        pyToAttr("%s.origAnimation_Layer" % (self.infoNode), self.origAnimation_Layer)
    ...
    def makeExtraJointsInfoNode(self, joints):
        ...
        for i in range(len(joints)):
            ...
            if i == 0:
                ...
                newExtraJoints.append(
                    DJB_CharacterNode(joints[i], parent=ParentBodyPart, twistJoint_=True, translateOpen_=True))
                jointIndex = len(newExtraJoints) - 1
            else:
                newExtraJoints.append(DJB_CharacterNode(joints[i], parent=newExtraJoints[jointIndex], twistJoint_=True,
                                                        translateOpen_=True))
                jointIndex = len(newExtraJoints) - 1
    ...
    def makeDynamicChainRig(self, joints, dynamic_="ZV", control_="FK"):
        ...
        for i in range(len(joints)):
            ...
            if i == 0:
                ...
                newExtraJoints.append(
                    DJB_CharacterNode(joints[i], parent=newExtraJoints[jointIndex], dynamic_=dynamic_))
                jointIndex = len(newExtraJoints) - 1
            ...
            if i < len(joints) - 1 and len(joints) > 1:
                newExtraJoints[jointIndex].createControl(type="FK", rigType="Dyn",
                                                         style="circle",
                                                         scale=(self.proportions["depth"] * 0.10,
                                                                self.proportions["depth"] * 0.10,
                                                                self.proportions["depth"] * 0.10),
            ...
            if i == len(joints) - 1:
                newExtraJoints[jointIndex].createControl(type="options",
                                                         style="options",
                                                         scale=(self.proportions["depth"] * 0.06,
                                                                self.proportions["depth"] * 0.06,
                                                                self.proportions["depth"] * -0.06),
                                                         rotate=(-90, 0, 90),
        ...
        if len(joints) == 1:
            ...
            newExtraJoints[0].IK_Handle = DJB_createGroup(pivotFrom=newExtraJoints[0].Bind_Joint,
                                                          fullName="%s_Dyn_NULL" % (FullName))
            ...
            mayac.parentConstraint(newExtraJoints[0].parent.Bind_Joint, newExtraJoints[0].IK_CTRL_POS_GRP,
                                   maintainOffset=True)
            newExtraJoints[0].Dyn_Node = nParticleMethod(newExtraJoints[0].IK_Handle, weight=0.7, conserve=1.0,
                                                         transfShapes=False)
        ...
        if dynamic_ == "ZV" and jointIndex > 0:  # more than one dynamic joint
            for i in range(1, endJointIndex + 1):
                temp = mayac.ikHandle(sj=newExtraJoints[i].parent.Dyn_Joint, ee=newExtraJoints[i].Dyn_Joint,
                                      n="%s_DYN_IKHandle" % (newExtraJoints[i - 1].nodeName))
                ...
                if i == 1:
                    mayac.parent(newExtraJoints[i].IK_CTRL_POS_GRP, self.global_CTRL)
                    mayac.parentConstraint(newExtraJoints[0].parent.Bind_Joint, newExtraJoints[i].IK_CTRL_POS_GRP,
                                           maintainOffset=True)
                else:
                    mayac.parent(newExtraJoints[i].IK_CTRL_POS_GRP, newExtraJoints[i - 1].IK_Handle)
                newExtraJoints[i].Dyn_Node = nParticleMethod(newExtraJoints[i].IK_Handle, weight=0.7, conserve=1.0,
                                                             transfShapes=False)
            newExtraJoints[endJointIndex].createControl(type="Dyn",
                                                        style="box1",
                                                        scale=(self.proportions["depth"] * 0.1,
                                                               self.proportions["depth"] * 0.1,
                                                               self.proportions["depth"] * 0.1),
                                                        offset=(0, 0, 0),
                                                        color_="white",
        ...
        for i in range(len(joints)):
            if newExtraJoints[i].Dyn_Node:
                mayac.connectAttr("%s.weight" % (newExtraJoints[len(newExtraJoints) - 1].Dyn_CTRL),
                                  "%s.weight" % (newExtraJoints[i].Dyn_Node))
                mayac.connectAttr("%s.conserve" % (newExtraJoints[len(newExtraJoints) - 1].Dyn_CTRL),
                                  "%s.conserve" % (newExtraJoints[i].Dyn_Node))
            if newExtraJoints[i].Dyn_Joint:
                mayac.connectAttr("%s.multiplier" % (newExtraJoints[len(newExtraJoints) - 1].Dyn_CTRL),
                                  "%s.input2X" % (newExtraJoints[i].Dyn_Mult))
                mayac.connectAttr("%s.multiplier" % (newExtraJoints[len(newExtraJoints) - 1].Dyn_CTRL),
                                  "%s.input2Y" % (newExtraJoints[i].Dyn_Mult))
                mayac.connectAttr("%s.multiplier" % (newExtraJoints[len(newExtraJoints) - 1].Dyn_CTRL),
                                  "%s.input2Z" % (newExtraJoints[i].Dyn_Mult))
                if newExtraJoints[i].translateOpen:
                    jointPosHolder = mayac.shadingNode('multiplyDivide', asUtility=True,
                                                       name="%s_JointPos" % (newExtraJoints[i].Dyn_Mult1))
                    jointPosHolder1 = mayac.shadingNode('multiplyDivide', asUtility=True,
                                                        name="%s_JointPos" % (newExtraJoints[i].Dyn_Mult1))
                    ...
                    subtract1 = mayac.shadingNode('plusMinusAverage', asUtility=True,
                                                  name="%s_Subtract" % (newExtraJoints[i].Dyn_Mult1))
                    ...
                    mayac.connectAttr("%s.translate" % (newExtraJoints[i].Dyn_Joint), "%s.input3D[1]" % (subtract1),
                                      force=True)
                    mayac.connectAttr("%s.output3D" % (subtract1), "%s.input1" % (newExtraJoints[i].Dyn_Mult1),
                                      force=True)
                    add1 = mayac.shadingNode('plusMinusAverage', asUtility=True,
                                             name="%s_Add" % (newExtraJoints[i].Dyn_Mult1))
                    ...
                    mayac.connectAttr("%s.output3D" % (add1), "%s.translate" % (newExtraJoints[i].DynMult_Joint),
                                      force=True)
                    mayac.connectAttr("%s.multiplier" % (newExtraJoints[len(newExtraJoints) - 1].Dyn_CTRL),
                                      "%s.input2Y" % (newExtraJoints[i].Dyn_Mult1))
                    mayac.connectAttr("%s.multiplier" % (newExtraJoints[len(newExtraJoints) - 1].Dyn_CTRL),
                                      "%s.input2Z" % (newExtraJoints[i].Dyn_Mult1))
        # Contraint Reverses
        ...
        mayac.connectAttr("%s.%s" % (newExtraJoints[len(joints) - 1].Options_CTRL, typeName),
                          "%s.inputX" % (reverseNode))
        for i in range(len(joints)):
            if newExtraJoints[i].Constraint:
                mayac.setAttr("%s.interpType" % (newExtraJoints[i].Constraint), 2)
                mayac.connectAttr("%s.%s" % (newExtraJoints[len(joints) - 1].Options_CTRL, typeName),
                                  "%s.%sW1" % (newExtraJoints[i].Constraint, newExtraJoints[i].DynMult_Joint))
                mayac.connectAttr("%s.outputX" % (reverseNode),
                                  "%s.%sW0" % (newExtraJoints[i].Constraint, newExtraJoints[i].FK_Joint))
            DJB_Unlock_Connect_Lock("%s.%s" % (newExtraJoints[len(joints) - 1].Options_CTRL, typeName),
                                    "%s.visibility" % (newExtraJoints[i].DynMult_Joint))
            ...
            if newExtraJoints[i].Dyn_CTRL:
                DJB_Unlock_Connect_Lock("%s.%s" % (newExtraJoints[len(joints) - 1].Options_CTRL, typeName),
                                        "%s.visibility" % (newExtraJoints[i].Dyn_CTRL))
        # infoNode stuff
        for i in range(len(joints)):
            newExtraJoints[i].writeInfoNode()
            mayac.parent(newExtraJoints[i].infoNode, self.Misc_GRP)
...
class MIXAMO_AutoControlRig_UI:
    def __init__(self):
        ...
        mayac.menuItem(label='Bugs, Feature Requests, Confusions, Praise, Support',
                       command=lambda *args: goToWebpage("community"))
        ...
        self.layout = mayac.formLayout(self.form, edit=True, attachForm=(
        (self.tabs, 'top', 90), (self.tabs, 'left', 0), (self.tabs, 'bottom', 0), (self.tabs, 'right', 0)))
        ...
        mayac.text(
            label='  If the character has unusual proportions or large appendages, the button below will create a cube that you may scale to',
            align='center')
        ...
        mayac.text(label='WARNING!!!!: The following changes the rig.  Skeleton will no longer match original fbx.',
                   align='center')
        mayac.text(label='For best results with animation, export from the Export tab once rigging is complete and',
                   align='center')
        ...
        mayac.separator(height=15, style='in')
        autoSkinnerText = mayac.text(label='  Import your downloaded MIXAMO character and then press the button below.',
                                     align='left')
        ...
        mayac.text(label='', align='left')
        mayac.text(
            label="**Please note that the animation data functionality is only designed to work with animations retargeted to your character's skeleton**",
            align='left')
        mayac.text(label='', align='left')
        mayac.text(label='', align='left')
        ...
        mayac.text(label='', align='left')
        mayac.text(label='  Select the "Hips" joint of the imported motion and then press the button below.',
                   align='left')
        mayac.button(label='Copy Animation to Rig (for imported animation)', w=100, c=self.copyAnimationToRig_function)
        mayac.text(label='', align='left')
        mayac.button(label='Direct Connect Animation to Rig (for referenced animation)', w=100,
                     c=self.connectAnimationToRig_function)
        mayac.text(label='', align='left')
        mayac.text(label='', align='left')
        mayac.text(label='Here you can bake the animation to the controls and/or revert to clean controls at any time.',
                   align='center')
        controlRigText = mayac.text(
            label="For more details see the documentation or www.mixamo.com/c/auto-control-rig-for-maya",
            align='center')
        ...
        mayac.text(label='', align='left')
        self.bakeAnimation_button = mayac.button(label='Bake Animation to Controls (imported anim only)', w=100,
                                                 c=self.bakeAnimation_function)
        mayac.text(label='', align='left')
        self.bakeAnimation_button = mayac.button(label='Clear Animation Controls (imported anim only)', w=100,
                                                 c=self.clearAnimation_function)
        mayac.text(label='', align='left')
        mayac.text(label='', align='left')
        mayac.text(label='  Note: The original animation resides in the scene on its own layer until deleted.',
                   align='left')
        mayac.text(label='', align='left')
        self.deleteOrigAnimation_button = mayac.button(label='Delete Original Animation (imported anim only)', w=100,
                                                       c=self.deleteOrigAnimation_function)
        mayac.text(label='', align='left')
        mayac.setParent('..')
        ...
        self.reduceNonEssentialJointsCheckBox = mayac.checkBox(label='Reduce Keyframes on Non-Essential Joints',
                                                               align='left')
        ...
        self.dynamicsFadeFramesSlider = mayac.intSliderGrp(field=True, label='Dynamics Pose-Match Frame', minValue=0,
                                                           maxValue=50, value=0, columnAttach3=("both", "both", "both"))
        mayac.separator(height=15, style='in')
        self.exportBakedSkeleton_button = mayac.button(label='Export Baked Skeleton', w=100,
                                                       c=self.exportBakedSkeleton_function)
        mayac.text(label='', align='left')
        mayac.setParent('..')
        ...
        mayac.text(label='', align='left')
        self.batch_fileOptionRadio = mayac.radioButtonGrp(label="Animated File:",
                                                          labelArray3=["Open", "Import", "Reference"],
                                                          numberOfRadioButtons=3, sl=1)
        ...
        self.batch_replaceRigReference_textFieldButtonGrp = mayac.textFieldButtonGrp(label='New Rig Reference', text='',
                                                                                     buttonLabel='Browse',
                                                                                     buttonCommand=self.batch_replaceRigReferencePath_function)
        mayac.separator(height=15, style='in')
        self.batch_playblast_checkbox = mayac.checkBox(label="Playblast", v=True)
        self.batch_playblastPath_textFieldButtonGrp = mayac.textFieldButtonGrp(label='Playblast Save Path', text='',
                                                                               buttonLabel='Browse',
                                                                               buttonCommand=self.batch_playblastPath_function)
        mayac.separator(height=15, style='in')
        ...
        self.batch_savePath_textFieldButtonGrp = mayac.textFieldButtonGrp(label='File Save Path', text='',
                                                                          buttonLabel='Browse',
                                                                          buttonCommand=self.batch_savePath_function)
        mayac.text(label='', align='left')
        ...
        self.batch_startFileOption = mayac.button(label='Remake Mesh Infonode', w=100,
                                                  c=self.remakeMeshInfoNode_function)
        mayac.text(label='', align='left')
        ...
        self.makeExtraJointsInfoNode_button = mayac.button(label='Make Extra Joints Infonode from selection', w=100,
                                                           c=self.makeExtraJointsInfoNode_function)
        mayac.text(label='', align='left')
        ...
        self.makeDynamicChainRigFromSelection_button = mayac.button(label='Make Dynamic Chain Rig from selection',
                                                                    w=100,
                                                                    c=self.makeDynamicChainRigFromSelection_function)

        mayac.setParent('..')

        mayac.tabLayout(self.tabs, edit=True, tabLabel=(
        (self.child1, 'Rig'), (self.child2, 'Animate'), (self.child3, 'Export'), (self.child4, 'Batching'),
        (self.child5, 'Utilities')))
    ...
    def showAboutWindow(self, arg=None):
        ...
        about_layout = mayac.formLayout(about_form, edit=True, attachForm=(
        (about_tabs, 'top', 0), (about_tabs, 'left', 0), (about_tabs, 'bottom', 0), (about_tabs, 'right', 0)))
        # About Tab
    ...
    def createOverrideBB_function(self, arg=None):
        ...
        if not DJB_CharacterInstance:
            ...
            mayac.move(BoundingBox[0], BoundingBox[1], BoundingBox[5],
                       "%s.vtx[0]" % (DJB_Character_ProportionOverrideCube), absolute=True)
            mayac.move(BoundingBox[3], BoundingBox[1], BoundingBox[5],
                       "%s.vtx[1]" % (DJB_Character_ProportionOverrideCube), absolute=True)
            mayac.move(BoundingBox[0], BoundingBox[4], BoundingBox[5],
                       "%s.vtx[2]" % (DJB_Character_ProportionOverrideCube), absolute=True)
            mayac.move(BoundingBox[3], BoundingBox[4], BoundingBox[5],
                       "%s.vtx[3]" % (DJB_Character_ProportionOverrideCube), absolute=True)
            mayac.move(BoundingBox[0], BoundingBox[4], BoundingBox[2],
                       "%s.vtx[4]" % (DJB_Character_ProportionOverrideCube), absolute=True)
            mayac.move(BoundingBox[3], BoundingBox[4], BoundingBox[2],
                       "%s.vtx[5]" % (DJB_Character_ProportionOverrideCube), absolute=True)
            mayac.move(BoundingBox[0], BoundingBox[1], BoundingBox[2],
                       "%s.vtx[6]" % (DJB_Character_ProportionOverrideCube), absolute=True)
            mayac.move(BoundingBox[3], BoundingBox[1], BoundingBox[2],
                       "%s.vtx[7]" % (DJB_Character_ProportionOverrideCube), absolute=True)
            ...
            mayac.move(pivotPointX, pivotPointY, pivotPointZ, "%s.scalePivot" % (DJB_Character_ProportionOverrideCube),
                       "%s.rotatePivot" % (DJB_Character_ProportionOverrideCube), absolute=True)
    ...
    def copyAnimationToRig_function(self, arg=None):
        ...
        if not DJB_CharacterInstance:
            ...
            if DJB_CharacterInstance[0].Hips.Bind_Joint:
                isCorrectRig = DJB_CharacterInstance[0].checkSkeletonProportions(selection[0])
                if isCorrectRig:
                    DJB_CharacterInstance[0].transferMotionToAnimDataJoints(selection[0], newStartTime=0,
                                                                            mixMethod="insert")
                else:
                    OpenMaya.MGlobal.displayError("Imported Skeleton does not match character!")
        else:  # more than one character, spawn a choice window
    ...
    def exportBakedSkeleton_function(self, arg=None):
        ...
        if DJB_CharacterInstance:
            ...
            DJB_CharacterInstance[0].createExportSkeleton(keepMesh_=keepMesh, dynamicsToFK=dynamicsFadeFrames,
                                                          reduceNonEssential=reduce, removeEndJoints=removeEndJoints)
            if arg:
            ...
            if version == 2010.0:
                OpenMaya.MGlobal.displayInfo(
                    "You may delete the newly created geometry and joints after exporting is complete")
        else:
            OpenMaya.MGlobal.displayError("No Character Found!")
        ...
        if DJB_CharacterInstance:
            if sel:
                for cur in sel:
                    ...
                    DJB_CharacterInstance[0].makeDynamicChainRig(chain, dynamic_="ZV",
                                                                 control_="FK")  # need to add checks for eligible joint chain
            else:
    ...
    def batch_function(self, arg=None):
        ...
        for file in filesShort:
            ...
            if references:
                ...
                newRef = mayac.textFieldButtonGrp(self.batch_replaceRigReference_textFieldButtonGrp, query=True,
                                                  text=True)
                if ".ma" in newRef:
            ...
            if playblast:
                ...
                mayac.modelEditor("modelPanel4", edit=True, camera="persp", rnm="base_OpenGL_Renderer",
                                  nurbsCurves=False, joints=False, cameras=False, grid=False, ikh=False,
                                  deformers=False, dynamics=False, nParticles=False, follicles=False, locators=False,
                                  activeView=True)
                mel.eval(
                    'playblast  -format avi -filename "%s" -sequenceTime 0 -clearCache 1 -viewer 1 -showOrnaments 0 -offScreen  -fp 4 -percent 100 -compression "XVID" -quality 70 -widthHeight 1280 720;' % (
                    blastFile))
    ...
    def batch_File1_browse_function(self, arg=None):
        self.file1Dir = DJB_BrowserWindow(filter_="Maya", caption_="Browse for character files directory",
                                          fileMode_="directory")
    ...
    def batch_replaceRigReferencePath_function(self, arg=None):
        replaceRigReferencePath = DJB_BrowserWindow(filter_="Maya", caption_="Browse for rig reference replacement",
                                                    fileMode_="Maya")
        if replaceRigReferencePath:
            mayac.textFieldButtonGrp(self.batch_replaceRigReference_textFieldButtonGrp, edit=True,
                                     text=replaceRigReferencePath)
        else:
...
class ACR_copyAnimationToRigWindow():
    ...
    def copyFunction(self, arg=None):
        ...
        selection = mayac.ls(sl=True)
        selectedIndex = mayac.textScrollList(self.characters_ScrollList, query=True, selectIndexedItem=True)
        DJB_CharacterInstance[selectedIndex[0] - 1].transferMotionToAnimDataJoints(selection[0], newStartTime=0,
                                                                                   mixMethod="insert")
